...
Le triangle regtangle de hauteur $\sqrt {a_0}=\sqrt{wz}$ s'inscrit dans un cercle de rayon $(w + z)/2 = {a_1}^{1/4}^$, et présente des propriétés très particulières liées au nombre d'or, voir fig4. La valeur de $a_1$ est très particulière:

\begin{equation}
a_1 \approx \pi_1^4\sqrt2    ~~~~   \rightarrow ~~~~   \pi_1 \approx (\sqrt{pn}/6): 3, 7, \sqrt{H\sqrt{nH}/3p}
\end{equation}

Cette double caractérisation $\pi_1$ de montre que les masses des particules principales sont effectivement liée à la théorie de Wyler.

...
Cette suite fractionnaire de $\pi$ est un problème non résolu des mathématiques actuelles. Cependant, on note que la partie entière de $\pi_5$, le nombre $292$, est la moyenne des nombres de symétries cristallines paires (419) et impaires (265) dans l'espace $10D$ des supercordes \cite{Weigel} \cite{Veysseyre} (table3). 
...
Avec la valeur de $G$ optimisée par une précédente étude de corrélation \cite{Sanchez2}, on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (table 2):
...
\begin{table}
\label{tab:8:table8}
\caption[Table \ref{tab:2:table2}: 15 Hubble radius formulas]{15 ppb precise formula for $R \approx 13.8119768$ Gly}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{3}{c}{}                   \\
    \cmidrule(r){1-3}

...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 48 Hubble radius formulas]{48 formula for Hubble radius $R$}
\label{tab:6:table6}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
    %\multicolumn{3}{c}{}     \\
    \cmidrule(r){1-3}
    Formule     & Valeur (Gal)  & Remarks \\
    \midrule
...
The maximal Euler's suitable number $s_0 = 1848$, is very close to the Eddingtons's prediction \cite{Eddington} for the proton/electron mass ratio, $1847.599459$, as the ratio of the roots of the equation $10x^2 - 136 x + 1 = 0$. The integer part of this ratio is 1836, which obeys, 
with $\Pi_0 = \lambdabar_e/ \lambda_{\Pi_0}$, and the associated term $\Pi_+$ for the charged Pion, and $ p_{hol} = (4 (r_H/\lambdabar_e)^3/3)^{1/2}$:
 \begin{equation}
 \left\{{
    \begin{array}{ll}
        E[p] = 1836 = S_{12} = s_0 - 12 \\
        s_0 \Pi_+ \Pi_0 \approx m_{cd}/m_e \approx (q^2a/4)^2 H^{0}/p_{hol}\\
        s_0 + 1/2 \approx (4\pi)^2 \sqrt a \approx F_{137}/96 a_w^2 \\
          (s_0 +1/2)/2 = \binom{12}{6} + 1/4 \approx(210^{210}/\mu^{\mu})^{1/3}\\
    \end{array}
\right.
\end{equation}

The last formula is deduced from the relation with the Monster Group (section 5.7) confirming the connection $\mu \approx 210$. The $+1/2$ term comes from taking account of the dimension $0$ in the half sum of symmetry numbers, see below. The involved precise value for $\pi$ is very particular, opening further study.  
...
\begin{equation}
 ...
 The order of the Monster group connects with the Lepton masses:
 
 \begin{equation}
 O_M^9 \approx \tau^{137},     
 \end{equation}
 
 The Koide tau value \cite{Koide} is confirmed to 20 ppb by the following formula, where $n_t/p$ is the mass ratio neutron-proton:
 
\begin{equation}
 (\tau/\tau_0 )^{137} \approx \pi^{\pi}d_e n_t/p   
 \end{equation}

 Confirming the approximation $\tau_0 = (2+3+5+7)2\times 2\times 3\times 5\times 7 $, resulting from $\mu_0 = 2\times 2\times 3\times 5\times 7$.

...
\label{tab:2:table2}
  ...
  %This work "weak-mixing angle" & $(\sin \theta)^2(m_Z)$    & 0.231475 &  \\
   ...
   Optimized SU(2) gauge coupling &{$q'$}     & 0.6421390 & 100 this work \\
   Optimized U(1) gauge coupling & {$q''$}    & 0.3436256 & 100 this work \\
...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 35 Hubble radius formulas]{35 formula for $R$}
\label{tab:6:table6}
 ...
 $ R/\lambdabar_e  \approx 6H (2\pi_{hol} a^3)^5 $& 13.81197676    &  Holographique, avec     $\pi_{hol} = 3 + 1/(7+1/(16+2/e^{2e}))$\\
  ...
  $ R_N (sin\theta_Z)^2/ q$  & 13.81197676    & Identification, à partir de la charge $q$ précisant $(sin\theta_Z)^2$   \\
 ...
 $r_H 2^{110} (a\beta/\pi_A)^2$  & 13.81197    & Extrapolation d'ordre 16 de la table périodique, avec $\pi_A$ = 22/7 \\
  ...
  $ R_N (\sqrt f(2)/exp(exp(-q''))^{128}(137 p_G/a\sqrt{pn_t})$  & 13.81192    & Symétrie $R_N/R$ suivant parallèle Axe Topologique-charge de jauge $q''$    \\
  
  
 $ \lambdabar_{e} (6/\pi_R)^{r_H/\lambdabar_e}$  & 13.8120    & $137,a,a_w$ : bases de calcul, avec : $3,7,\sqrt a_w/2\times 137^2$ \\  
 ...
 $ \lambdabar_{e} exp(2a_w/21(137a)^2) $ & 13.85 & Confirme que l'univers est un boson de jauge, par sa puissane 7 \\ 
 ...
 $ (l_P/2)(\lambdabar_{CMB}^2\lambdabar_p/\lambdabar_{CNB}r_e^2)^6$  & 13.7    & Complémentarité des fonds micro-ondes etneutrinos \\
...
\begin{table}
\caption[Table \ref{CosmosTable}: 38 Cosmos radius formula]{38 formula for $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ R_N^2/2l_P $ & 9.07577338 & Rayon Holographique à partir de $R_N$ \cite{Sanchez2}   \\
 
 $ 2r_H 3^{210}/1830$& 9.075770 & Holique base 3, avec $ 1830 = (60\times 61)/2 = p_0-1$   \\
...
$ \lambdabar_P (H~210^{210}/p\pi_q^2sin \theta)^{1/4}$ & 9.07578 & Confirme le nombre Holique $210^{210}$   \\
 ...
 $  \lambdabar_P ~ (\sqrt a_w (a_s/d_e)^2)^{16}$  & 9.07568 & Empirique  \\
  ...
  $  \lambdabar_e ~ (a^2/137\pi \beta)^2(210^{210/ln a}/q \tau$  & 9.07579 & Empirique, utilisant le Nombre Holique $210^{210}$  \\
   ...
   $  (12d_e)^2 (\lambdabar_e^2/r_H) ~ exp(exp(exp(exp(exp(-q')))))  $ & 9.07588 & Liaison directe avec la charge de jauge $q'$ \\
   ...
   $ \lambdabar_eg(7) (H/p)^2  P/6 $ & 9.0763 & Pertinence de la fonction topologique réduite $g(7) = f(30)/7$ \cite{Sanchez2}  \\
    ...
    $  \lambdabar_e ~  \mu^{(1+\mu+\tau)/2 -1/\xi(3)}  $ & 9.077 & Empirique quasi-holique  \\
      ...
      $  \lambdabar_e ~  ((3/4\pi)(R_1/\lambdabar_e)^7){1/3})   $ & 9.078 & Relation du nombre cosmique de photons avec le rayon mono-électronique $R1$  \\
...
$  \lambdabar_e ~ cotg\theta ~ 495^{\pi \sqrt a}  $ & 9.03 & Base 495, avec exposant $ \pi \sqrt a\approx \pi^{\pi}$  \\ 
 ...
 $ \lambdabar_e g(7) (\lambdabar/r_H)^3 $ & 9.1 & Pertinence du rayonnement de fond\cite{Sanchez2}  \\
 
 $  (Rl_{nl})^{3/2}/r_e^2 $ & 9.2 & Provient de l'holographie non-locale \cite{Sanchez2}  \\
 
  $  \lambdabar_P ~ \mu^{\mu a^3/3pH}  $ & 9.0 & $\mu$  base de calcul, voisine de la base holique $210$   \\
...
Le nombre de « paramètres libres » dans le modèle standard des particules est considérable, de l'ordre d’une vintaine. Les paramètres-clefs sont ces nombres purs (dits "sans dimension") qui interviennent le plus directement dans les théories physiques (Table \ref{tab:2:table2}). En particulier, on considère le rapport des masses des particules les plus importantes avec celle de l'électron, ce qui s'est révélé déterminant dans l'Axe Topologique ci-dessus (Table \ref{AxeTopologique}). 
...
A noter que $ 4n_t/\Gamma \approx 292.108$, voisin de $\pi_5$ ci-dessus, le cinquième terme fractionnaire de $\pi$.

\subsection {Le modèle électrofaible}
Le modèle standard de la physique des particules (table) a unifié les forces électriques et nucléaires faibles dans le modèle "électrofaible". La théorie de jauge invoquée, introduit les charges de jauge {$q'$} (groupe SU(2)) et {$q''$} (groupe U(1)), reliés à la constante électrique $q$ par \cite{Taylor}:

\begin{equation}
 a/4 \pi_q = 1/q^2 = 1/{q'}^2  + 1/{q''}^2      
 \end{equation}
 
La notation officielle $g$, {$g’$}, moins symétrique, n'est pas utilisée. La théorie prévoit un photon de masse nulle et trois bosons massifs $W_{+}$, $W_{-}$ et $Z$, caractérisés par un angle de couplage $\theta$, et liés au boson scalaire massif $H^{(0)}$ (Brout-Englert-Higgs), voir le triangle de Weinberg (Figure \ref{WeinbergTriangle}):
...
\begin{equation}
 \left\{
    \begin{array}{ll}
        \cos\theta = W/Z \\
        q' = W/H^0 ~~~~\approx H^0/S^{(26)} \\
        \sqrt{{q'}^2  + {q''}^2} = Z/H^0
    \end{array}
\right.
\end{equation}

où $S^{(26)} = 381540$ est la somme cristallographique d'ordre 26.
  
%\begin{equation}
 %a/4 \pi_q = 1/q^2 = 1/{q'}^2  + 1/{q''}^2 ~~~~ \rightarrow ~~~~ 6\pi_q^5 \approx (9/2)^5 ~~~~\rightarrow ~~~~ H^{(0)} = W/{q'} \approx %{q'}~S^{(26)}
% \end{equation}
 
L'analyse conduit à la double ppb-relation suivante confirmant la valeur de Z: 
...
\textit{Comme le groupe SU(2) ne concerne que les rotations dans un plan, il est naturel de compléter le triangle de Weinberg par un troisième axe portant l'unité.} (figure 6: Weinberg-Sanchez cuboid). On obtient un vecteur de module directement lié à la "clef holique" ci-dessus $2a^3/pH $, où $H$ est remplacé par la masse lucasienne du proton ci-dessus $p_G$. \textit{Cette hypothèse unifie la théorie életrofaible avec la gravitation:}

\begin{equation}
 1 + {q'}^2  + {q''}^2 = 1 + (m_Z/m_{BEH})^2 = 2a^3/pp_G 
 \end{equation}
...
\begin{equation}
 a (sin \theta)^2  = {a'}  \approx 30.505998 \approx 2e^e ,    ~~~~         a (cos \theta)^2  = {a''}  \approx 106.530011 \approx 4\pi a_s \approx e^e /q sin \theta 
 \end{equation}
...
où l'on reconnaît le terme $2e^e$ de l'Axe Topologique. L'ordinateur indique, à 15 ppb près:

\begin{equation}
2\times 30\times 30.5 /  a e^e \approx \Gamma \sqrt 2 (H\sin\theta/3p)^2
 \end{equation}

\textit{La constante d'Atiyah $\Gamma$ confirme donc la valeur de $\sin~\theta$, donc de la masse du boson scalaire et la valeur de $G$ dans le domaine du ppb. Le programme d'Atiyah \cite{Atiyah} est ainsi en cours de réalisation}.

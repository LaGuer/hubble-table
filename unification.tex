...
\begin{equation}
 a (sin \theta)^2  = {a'}  \approx 30, 50 \approx 2e^e ,    ~~~~         a (cos \theta)^2  = {a''}  \approx 106.53 \approx 4\pi a_s
 \end{equation}
...
où l'on reconnaît le terme $2e^e$ de l'Axe Topologique. L'ordinateur indique les corrélations suivantes, à 70, 20 ppm, 0.6 ppm et 15 ppb près: 
...
\begin{equation}
 \left\{
    \begin{array}{ll}
        ...
        2\times 30\times 30.5 /  a e^e \Gamma \sqrt 2 (H\sin~\theta/3p)^2
    \end{array}
\right.
\end{equation}

\textit{La constante d'Atiyah $\Gamma$ confirme donc la valeur de $\sin~\theta$ dans le domaine du ppb.}

%Ces relations sont illustrées dans le « triangle électrique » figure \ref{ElectricTriangle}. 

Le modèle standard fait apparaître une quantité voisine de $\sin~\theta$. En identifiant $(qR/R_N)^{1/2}$ avec $\sin \theta(m_Z)$, on observe la corrélation suivante, à 0.8 ppm près:

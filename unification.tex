...
As seen above, conservation is tied to both symmetry and computation, So, this section look for connections with the Multi-Dimensional Crystallography.
...
Here $\mu$ and $\tau$ are the leptons relative masses, $\cos \theta = W/Z$, and $d_e \approx 1.00116$ is the electron magnetic excess. 
 ...
 \textit{The two fist terms are close for $d = 32$, which specifies the Topological Axis symmetry, from $k = 0$ to $k = 7$, and the characteristics of the string dimension SO(32), whose dimension is the perfect number 496:}
...
\textit{It is shown \cite{Weinberger} that a single Euler suitable number could exist beyond $s_0$, and if no, i.e if $s_0$ is really the maximal one, then Riemann conjecture would be confirmed. So the proton-electron ratio is at the heart of Number Theory.}
...
Moreover the number 1839, which is close to the neutron/electron masse ratio, appears also in the Table 5, and so implies the following:
...
1839 = \sum_{10}^{12} K_d = \sum_1^{12} (N_d/2) + 3   ~~~~ \Rightarrow~~~~ \sum_2^9K_d = (K_{12}-9)/2 = 496  
 ...
 This identifies the partial sum up to the 9D space with the above dimension of the canonic group SO(32), central in string theory. They could be identified with the above 9 dimension reduction. 

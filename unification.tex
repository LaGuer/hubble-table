...
Carl Hermann \cite {Hermann} a calculé le nombre de symétries ponctuelles cristallographiques pour des espaces de dimensions entre de 1 à 8 (table 6.7). C'est le nomnbre de polynomes dont le terme principal est $x^d$, à coefficients entiers dont toutes les racines sont sur le cercle unité. On remarque que \textit {le terme de degré 7 est 118, correspondant au numéro atomique de l'Organesson dans le tableau périodique à 7 rangées}, voir ci-dessous l'analyse qui prouve que cela n'est pas dû au hasard. Cette analyse est basée sur les termes spectroscopiques de types $2 \times (2k +1)$, où $k$ est le nombre quantique orbital, qui s'identifie avec la série spéciale des cordes de l'Axe Topologique, en correspondance avec les bosons de jauge.

En particulier, les nombres 2, 6, et 10, correspondant à k = 0, 1 et 2, figurent directement dans la série d'Hermann. Les dimensions 1, 2 et 3 paraissent ainsi privilégiées. De plus les nombres 6 et 10 sont également les nombres de composants indépendants dans les matrices symétriques d'ordre 3 et 4. 
...
Dans l'approche des bitstrings de Noyes \cite{Noyes}, l'approximation $p \approx 1836$ est centrale. 

...
où l'on reconnaît le terme $2e^e$ de l'Axe Topologique. L'ordinateur indique les corrélations suivantes, à 70, 20 ppm, 0.6 et 0.5 ppm près, la dernière provenant de $\tau/mu\approx 9 \cotg~\theta \approx \Gamma \sqrt2 \sin~\theta (H/p)^2$ :
\begin{equation}
 \left\{
    \begin{array}{ll}
        137 \approx {a'} a^3/\sqrt a_w \approx \tau/{a'} \sin~(2\theta)\\
        2\times 30\times 30.5 \approx a e^e \cos~\theta\\
        \Gamma \approx (\cos~\theta/\sqrt 2)(3p/H\sin~\theta)^2
    \end{array}
\right.
\end{equation}
...
En identifiant $(qR/R_N)^{1/2}$ avec $\sin \theta(m_Z)$, on observe la corrélation suivante, à 0.8 ppm près:

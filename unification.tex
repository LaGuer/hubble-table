...
\tkzInterLC[R](A,B)(C,-0.3cm) \tkzGetSecondPoint{B}
\tkzInterLC[R](B,B')(E,-1cm) \tkzGetSecondPoint{F}
%\tkzDefLine[orthogonal=through D](A,D)
\tkzDefLine[perpendicular=through F](O',B') \tkzGetPoint{G}
\tkzDefLine[perpendicular=through B](O,B) \tkzGetPoint{J}
\tkzDefMidPoint(F,G) \tkzGetPoint{Q'}
\tkzDefMidPoint(J,B) \tkzGetPoint{Q}
%\tkzInterLC(D,tkzPointResult)(O,A) \tkzGetSecondPoint{C}
%\tkzInterLC(F,tkzPointResult)(B,E) \tkzGetSecondPoint{G}
%\tkzDrawPolygon(A,E,H)
\tkzDrawSegment[color=blue](Q',B')
\tkzDrawSegment[color=blue](Q',Q)
\tkzDrawSegment[color=blue](B,B')
\tkzDrawSegment[color=red](A,B)
\tkzDrawSegment[color=red](Q,A)
\tkzDrawSegment[color=red](B,Q)
\tkzDrawPoints(A,B,Q,O,O',B',Q')
\tkzLabelPoints(A,B,O,B',Q')
...
\tkzMarkRightAngle(A,Q',B')
\tkzMarkRightAngle(A,Q,B)
\tkzDefMidPoint(Q',B') \tkzGetPoint{I}
\tkzLabelSegment[left,text=blue](Q',B'){\large $\tau$}
\tkzLabelSegment[right,text=red](Q,B){\large $q$}
%\tkzMarkAngle[fill=blue!20,size=0.4cm,opacity=.5](E,A,H)
%\tkzLabelAngle[pos=1.4](E,A,H){$\theta$}
    \begin{pgfinterruptboundingbox}
        \tkzMarkAngle[fill = gray, size=0.633cm, opacity = .3](B,A,Q)
        \tkzLabelAngle[pos = 1.](Q,A,B){$\theta$}
...
\label{tab:1:table1}
  ...
  Codata "weak-mixing angle" & $(\sin\theta_{meas})^2$    & 0.2229(1) & $3\times 10^5$ \\

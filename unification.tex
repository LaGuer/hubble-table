...
La Table \ref{tab:6:table6} présente 16 formules simples qui confirment ce rayon de Hubble. 

\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 16 Hubble radius formulas]{14 formula for $R$}
\label{tab:6:table6}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{3}{c}{}                   \\
 ...
 $\sqrt {2l_{nD}^3/r_e}$  & 13.75    & Relation holographique avec la longueur non-Doppler $l_{nD}$ \\
 ...
 $ (2\lambdabar_{e}/3) (\lambdabar_{CMB}/\lambdabar_{H_2})^2$ & 13.90 &  Holographie dans la molécule gravitationnelle d'hydrogène \\
   ...
   $(2/3)^{1/3}R_1 d_e^2$  & 13.812    & Relation empirique avec le rayon mono-électronique $R_1$ \\
 
 $\sqrt {2R_1R_N/3}$  & 13.80    & Relation empirique avec le rayon mono-électronique $R_1$ \\
...
\begin{equation}
(R/\lambdabar_e)^{1/210} \approx 2R/R_N \approx (2/\phi_R)^2
 \end{equation}
...
Or, on note que la partie entière de $\pi_5$, le nombre $292$, déjà rencontré ci-dessus en connexion avec le nombre d'or $\Phi_a$, est la moyenne des nombres de symétries cristallines paires (419) et impaires (265) dans l'espace $10D$ des supercordes \cite{Weigel} \cite{Veysseyre} (Table \ref{tab:4:table4}).

\subsection{les nombres 137, 1836 et 1839}
Dans cette table  \ref{tab:4:table4}, figurent les nombres canoniques 137, 1836 et 1839, qui sont respectivement les parties entières de \textbf{a, p, n}  montrant que \textit{la théorie ultime est liée à l'arithmétique des symétries cristallines en dimensions supérieures}. En particulier "la relation cristallographique" 
...
\subsection{Les connexions avec la table périodique des élements chimiques}

Les dimensions de la série anormale des cordes $d = 2 + 4k$, qui apparaissent dans l’Axe Topologique sont les nombres $2, 6, 10, 14, 18, 22, 26, 30$. Les quatre premiers s’identifient avec les nombres atomiques de la série spectroscopique s,p,d,f. La table périodique contient 19 de ces séries, aboutissant à l’Oganesson, de numéro atomique $7s + 6p + 6d + 2f = 118 = 2 \times 59$, de sorte que, tenant copte de la relation cristalloraphique ci-dessus $137 = 2 \times 59 + 19  = 7(s+1)+6(p+1)+4(d+1)+2(f+1)$. Effectivement, les périodes se distinguent des nombres quantiques principaux, de telle façon que les périodes, à partir de la deuxième, sont doubles, de sorte que le nombre ci-dessus se décompose en $118/2 = 59 =  1+3s+3p+2d+f$.

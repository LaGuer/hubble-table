...
The Holic Principle, which is the simplified form of the Holographic Principle and based on the simplest degenate Diophantine equations, induces an “Hyper-symmetry” between the constants $G$ and $\hbar$ which leads to a new expression of Bohr’s orbits. With the very precise quasar non-local period, this establishes a connection between the Hubble and Cosmos radius of the Permanent Holographic Oscilatory Bang model and their common CMB temperature. The c-observable Universe appears as a Cosmos “immergence”, where the mean DNA bi-codon mass is central, connecting to $10^{-7}$ with the mean dimension 16 of the Topological Axis. The later defines a scalar boson mass of $495^2m_e$, connected to the natural geometric extension of the Weinberg triangle, confirming the $G$ value to $10^{-7}$, at $+ 7.7~ \sigma$ of the official value. Associating computation with symmetry, whose common point is invariance, multi-dimensional crystallographic symmetries shows the arithmetical origin of the term $495^2$, and establishes tight connections between the physical parameters, the string theory dimension reduction, the periodic table and the sporadic groups. The Continuum, Infinity, Locality, Zero mass Particle and Multiverse are excluded, replaced by the search towards a Science-Arithmetic Unification. Very strigent predictions result for the very large telescopes in preparation.  
...
%from the Combinatorial HierarchyLa Table \ref{tab:6:table6} shows 31 simple formula for $R$. This recall the 14 molecular formula of Jean Perrin that established definitely the existence of atoms.
...
% $ \lambdabar_{e} (6/\pi_R)^{r_H/\lambdabar_e}$  & 13.8120    & $137,a,a_w$ : bases de calcul, avec : $\pi_R: 3,7,\sqrt a_w/2\times 137^2$ \\  
 ...
 $4o_2\sqrt Z \lambdabar_{e}\lambda_{CMB}/l_P     $  & 13.8129    &Confirms the couple GC with mass o_2 m_H    \\
...
showing the 11 supergravity dimensions and the usual 4.

One observes the double apparition of this "arithmetic monster" 137:
...
This results from the symmetry around d = 16, indeed 32 = 2 + 30 = 6 + 26 = 10 + 16 = 18 + 22, illustrating the association symmetry-computation. 

The height numbers are all of the form "prime - 1", except $d_k = 14$ and $26$, the later being the critical dimension which verifies: $ d_{26} = d_{d_6} = 106 $, so justifying the reduced Atiyah sum, with the octonion term ($2^7$) and the quaternion one ($2^3$): 

\begin{equation}
 \sum_{k=0}^{k=7} (d_k+1)= 2^7 + 2^3 = 136 = 30 + 106 = d_7 + d_{d_6}
 \end{equation}

This identifies with the reduced "gauge separation", where 136 is the initial Eddington's value, the number of elements in the symmetrical matrix $16\times16$.  

The Periodic Table uses only the $d$ values inferior or equal to the above 14, corresponding to the famous spectroscopic numbers, called by Friedrich Hund "sharp" (s = 2), "principal" (p = 6), "diffuse" (d = 10) and "fundamental" (f = 14). The $7^{th}$ row of the Periodic Table terminate in the Oganesson, of atomic number 118, which is precisely the Herman number for d = 7. The involved coefficients, ruling the particular natural distribution of the spectroscopic groups s,p,d,f are the following:
...
which recovers the complete Atiyah sum, including the "real algebra" term $2^0$, and, since the last term is $2\times 15 = 30$, coming back to the gauge separation.

...
The Keplerian Diophantine equation symmetries gravitation and quantum physics, connecting the Bohr’s orbitals, the quasar non-local period, the CMB temperature, the Hubble and the Cosmos radius of the Permanent Holographic Oscillatory Bang model. The mean DNA bi-codon mass is central and connects to 0.1 ppm with the Topological Axis, which induces a scalar boson mass confirmed both by the geometric extension of the Weinberg triangle and by the 9D crystallographic symmetry, identified to the string dimension reduction. The running number of the string dimension series is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This confirms Eddington, Atiyah and the Combinatorial Hierarchy, leading to the gauge partition. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, leading to stringent predictions. 
...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 47 Hubble radius formulas]{47 formula for Hubble radius $R$}
\label{tab:6:table6}
...
\begin{equation}
3; 11; 5; 137; 7; 11   ~~~~~~~~ \Rightarrow 137 = 11^2 + 4^2 ~~~~~~~2\times 137 - 1 = 273 = 2^8 + 2^4 + 2^1 
 \end{equation}

showing the 11 supergravity dimensions and the usual 4. The number 273 is one of the Euler's suitable numbers, the closest associated to the prime 137. This number 137 has been unambigously connected with the Lucas-Lehmer series \cite{Sanchez3}. One observes the double apparition of this "arithmetic monster" 137:
...
The maximal Euler's suitable number is $s_0 = 1848$, very close to the Eddingtons's prediction \cite{Eddington} for the proton/electron mass ratio, $1847.599459$, as the ratio of the roots of the equation $10x^2 - 136 x + 1 = 0$. The integer part of this ratio is 1836, which is related by: 


 \begin{equation}
 \left\{
    \begin{array}{ll}
        E[p] = 1836 = S_{12} = s_0 - 12 \\
        s_0 + 1/2 \approx (4\pi)^2 \sqrt a \\
          s_0/2 = \binom{12}{6} \approx(210^{210}/\mu^{\mu})^{1/3}\\
    \end{array}
\right.
\end{equation}

Confirming the connection $\mu \approx 210$.

\textit{The Riemann conjecture would imply that $s_0$ is really the maximal suitable number, and that only one more can exist, very larger than $s_0$ \cite{Weinberger}. So the proton-electron ratio is at the heart of Number Theory.}

Moreover, the numbers 1839, which are is the closest whole value of the proton and neutron/electron masse rtio, appear also in the Table 5.5, and so iplies the folowing:

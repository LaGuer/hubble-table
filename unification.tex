...
The main hypothesis of this article is that the Cosmos, considered as a black hole of horizon $R_c$ is a super-computer \cite{Sanchez3}. This subject is much debated today. In particular, Y. Ng \cite{Ng} has introduced a “quantic foam”, with a scale much greater than the Planck’s length $l_P$, so potentially measurable by the gravitational wave detectors. But, this length $(Rl_P^2)^{1/3} \approx 10^{-15} $ is variable in standard cosmology because the Universe horizon $R$ is supposed to increase. On the opposite, in the Permanent Cosmology \cite{Sanchez2}, where the horizon $R$ is invariable, this distance can be linked to the classical radius of the electron $r_e$. Hence, we define the “Nambu radius” $R_N$ attached to the Nambu mass $nN = a me$ [13], important in physics of particles, (the Schwarzschild’s relation with the critical mass $M_N$  brings the factor 2: $R_N  = 2GM_N /c^2$):

...
Dans le cadre de la Cosmologie de précision, le réexamen des corrélations entre grands nombres \cite{Sanchez2} conduit à l'Axe Topologique (Fig. \ref{tab:9:table9}), qui montre que le rayon de Hubble est invariant et confirme la théorie bosonique des cordes. 
...
Considérer l'espace-temps comme une propriété \textit{secondaire} signifie que \textit{les concepts de masse, longueur et temps sont en final réductibles à des nombres purs}. Cela justifie la relation purement numérique observée entre le Cosmos et l'Univers limité par la constante $c$ (Figure/Table \ref{tab:1:table1}). 
...
où la relation $a^2 \approx 137^2 + \pi^2$ a été soulignée par Sir M. Atiyah. Une autre proriété liée à la théorie de Wyler est basée sur 
l'approximation $\pi \approx 4/\sqrt \Phi$ :

\begin{equation}
(4/\sqrt \Phi_a)^5/6 \approx (4\pi^2)^2 sqrt a
\end{equation}

\textit{Ainsi $a$ et $\Phi_a$ sont reliés par la théorie de Wyler.}
...
\label{tab:4:table4}
  \hskip-0.0cm\begin{tabular}{llllllllllllll}
    \toprule
    \multicolumn{14}{c}{Crystallographic $PSO_{Cr}$ Table.}                  \\
    \cmidrule(r){1-14}
...
où $m_P$ est la masse de Planck. Avec la valeur du boson scalaire $ m_{BEH} = 495^2 m_e $, on obtient une valeur de $m_Z$ à $10^{-7}$ de la valeur précédemment optimisée  \cite{Sanchez2} (Table \ref{tab:1:table1}). 
...
Le rapport $\tau/(2\times (1+\mu+\tau) = \sin\theta_{\tau} $ est très voisin du $\sin\theta$ du modèle électro-faible rappelé ci-dessous (Table \ref{tab:1:table1}). Il corrèle directement avec les couplages $a_w$ et $P$ (12 ppb):

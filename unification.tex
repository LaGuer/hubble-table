...
définit le $\pi$ mathématique à mieux que le ppb. La suite fractionnaire de $\pi$ est un problème non résolu des mathématiques actuelles. Cependant on note que le nombre $292$ est la moyenne des nombres de symétries paires (419) et impaires (265) dans l'espace des supercordes $10D$ \cite{Weigel} (table 3).


Avec la valeur de $G$ optimisée par une étude de corrélation \cite{Sanchez2} (tableau 2), on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (table 3):
...
De plus, la masse gravitationnelle du proton $p_G = P/\sqrt {N_L}\approx 1831.531182$, où et le grand nombre de Lucas, le terme terminal de a Hiérarchie Combinatoire \cite{Bastin}, écrite sous la forme de Lenz-Wyler \cite{Wyler} présente un développement singulier: 
...
Cette valeur de $R$ connecte avec la fonction topologique réduite : $g(k) = exp(2^{k+1/2})/k$, pour la valeur $k = 6 (d = 26)$ de l'Axe Topologique, où 26 est la dimension privilégiée de la thèorie bosonique des cordes: 

\begin{equation}
R/\lambdabar_e \approx g(6)
 \end{equation}

où $\lambdabar_e = \hbar/m_ec$ est la longueur d'onde réduite de l'électron. Les multiples relations holographiques, impliquant en particulier le rayonnement de fond, qui est donc aussi invariant, se résument à:
...
où ce terme $2a^3/pH$ est lui-même central dans les relations holographiques.

...
où $m_P$ est la masse de Planck. Avec la valeur du boson scalaire $ m_{BEH} = 495^2 m_e $, on obtient une valeur de $m_Z$ à $10^{-7}$ de la valeur précédemment optimisée  \cite{Sanchez2}, et donc des valeur précises à $10^{-7}$ de $q$, $q$' et $q''$ (Table \ref{tab:2:table2}). 

La relation entre la charge électrique adimensonnelle $q$ et le couplage électrique $a$ est \cite{Taylor}, montrant une corrélation directee avec la some cristallographique $S^{(26)} = 381540$:
\begin{equation}
 a/4 \pi_q = 1/q^2 = 1/{q'}^2  + 1/{q''}^2 ~~~~ \rightarrow ~~~~ 6\pi_q^5 \approx (9/2)^5 ~~~~ H^{(0)} = W/{q'} \approx {q'}~S^{(26)}
 \end{equation}

...
\section{Introduction: le Multivers en question}

La physique mathématique, malgré une série de succès dans l'unification, se heurte actuellement au blocage de la cosmologie, qui, malgré une optimisation de 6 paramètres, butte sur un écart de 10\% avec la mesure directe du rayon de Hubble. De plus, la théorie des cordes semble privilégier la thèse du Multivers, un renoncement à la recherche ancestrale de l'Unité. Le "réglage fin" observé, si propice à la Vie, est alors justifié par une utilisation dévoyée du "principe anthropique" : on se limite à proclamer qu'un monde vivable est, statistiquemet, possible. 

Mais \textit{le seul argument} pour justifier des fameuses corrélations entre les grands nombres est précisément du même type "anthropique"\cite{Rees}. Or, comme la cosmologie prétend atteindre la précision du \%, cette vague explication "anthropique" ne tient plus. Par contre, une étude directe montre un ordonnancement remarquable, l'Axe Topologique \cite{Sanchez2} liant Micro et Macro-physique, visiblement lié à la théorie bosonique des cordes et l'algèbre des octonions, annoncé par Atiyah \cite{Atiyah} comme déterminant.

Selon le mot de Russel, celui-là même qui a démontré que $1+1 = 2$ en 180 pages, \textit {"Le plus incroyable de la science moderne est son retour au pythagorisme"}\cite{Alcina}. En effet, le pythagorisme s’accorde avec la nature quantique du monde, en particulier les nombres quantiques de la physique des particules. 

Mais la Science se caractérise par l'existence de lois de conservation, aussi bien en physique qu'en biologie. Les théoriciens associent conservation et symétrie, mais plus fondamentalement, \textit{conservation implique calcul, ce qui justifie la primauté de l'Arithmétique}. Cela élimine l'hypothèse centrale de la physique classique, l'hypothèse du continu, et donc justifie l'apparition de phénomènes quantiques.
 
Cet article prétend montrer la voie vers l'unification Physique-Arithmétique-Biologie.
...
\textit {Un tel Cosmos calculateur devant utiliser des bases de calcul supérieures à l’unité}, les 4 constantes de couplages considérées (électrique, gravitationnelle, faible, forte), $a, a_G, a_w, a_s$ sont ici les inverses des quantités officielles. On y joint la constante $P = m_P/m_e$, le rapport de la masse de Planck sur celle de l'électron, voir en Appendice le bas de la Table \ref{tab:9:table9}). Les constantes \textit{dimensionnées} sont rassemblées dans la Table \ref{tab:2:table2}) \cite{Tanabashi}. 


\section{L'Axe Topologique et le rayon \textit{invariant} de Hubble}

Dans le cadre de la Cosmologie de précision, le réexamen des corrélations entre grands nombres \cite{Sanchez2} conduit à l'Axe Topologique (Fig. \ref{tab:1:table1}), qui montre que le rayon de Hubble est invariant et confirme la théorie bosonique des cordes. 

Cette théorie des cordes a suscité un espoir énorme quand elle a exhibé une particule de spin 2, qu'on a identifié au graviton, intégrant ainsi la gravitaton. De plus, elle a mis en évidence une correspondance holographique explicite entre des espaces-temps de dimensions différentes. \textit{Mais cette théorie bute sur le concept de l'univers en expansion}. Schwarz : "this turns out to be surprizingly dfficult" \cite{Schwarz}.  
...
Mais cela est fortement discuté actuellement, et remis en question par l'étude ci-dessus \cite{Sanchez2} qui a montré que les multiples corrélations observées entre les grands nombres de la physique mathématique pouvait être considérées comme une succession de relations holographiques 1D-2D, aboutissant à l'Axe Topologique (Fig  \ref{tab:1:table1}), confirmant la théorie bosonique des cordes, et sa dimension 26, qui correspond au rayon de Hubble donné par le modèle de la molécule gravitationnelle d'hydrogène, 


\begin{equation}
R =  2 \hbar^2/Gm_e m_p m_H \approx 13.81 \times 10^9 annés-lumière
 \end{equation}
...
\begin{equation}
R \approx \lambdabar_eg(6) = 13.82 \times 10^9 annés-lumière
 \end{equation}
...
Le concept d'Univers en expansion étant éliminé, la courbure de l'espace-temps n'est alors qu'une propriété locale, tandis que l'Univers dans son ensemble présente une géomérie dite "plate", c'est-à-dire euclidienne. C'est effectivement ce que les astrophysiciens constatent, et désignent sous le nom de 'condition critique'. L'application de la physique non-relativiste à l'Univers critique conduit alors à justifier directement la fraction $3/10$ de l'énergie totale, ce qui confirme le taux de masse noire. Par contre, la partie complémentaire, les $70\%$ restants, attribuée à la soi-diant "énergie sombre", n'est ainsi qu'un faux problème \cite{Sanchez2}. 
...
\begin{equation}
\underline{T^2 = L^3 = M^5 = Ch^7 = x^{210}}
 \end{equation}
...
\phi_9^{2\times 210} \approx 2^{292}
 ...
 où apparaît le nombre \doubleunderline{292}, caractéristique de $\pi$, voir ci-dessous. 
...
où \underline{1836} est la partie entière de $p$.
\newpage
...
Avec la valeur de $G$ optimisée par une précédente étude de corrélation précise au ppb \cite{Sanchez2}, on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (Table \ref{tab:9:table9} ):
...
Le nombre de « paramètres libres » dans le modèle standard est considérable, de l'ordre d’une trentaine. Les paramètres-clefs sont ces nombres purs (dits "sans dimension") qui interviennent le plus directement dans les théories physiques (Table \ref{tab:9:table9}). En particulier, on considère le rapport des masses des particules les plus importantes avec celle de l'électron, ce qui s'est révélé déterminant dans l'Axe Topologique ci-dessus (Fig. \ref{tab:1:table1}). Certains paramètres sont mesurés dans le domaine du ppb, mais les masses centrales dans la théorie électrofaible rappelée ci-dessous, ne sont définis qu'à $10^{-3}$ (boson scalaire massif de Brout-Englert-Higgs), $10^{-4}$ (boson chargé W), $10^{-5}$ (boson neutre Z).
...
où $m_P$ est la masse de Planck. Avec la valeur du boson scalaire $ m_{BEH} = 495^2 m_e $, on obtient une valeur de $m_Z$ à $10^{-7}$ de la valeur précédemment optimisée  \cite{Sanchez2} (Table \ref{tab:9:table9}). 
...
Le rapport $\tau/(2\times (1+\mu+\tau) = \sin\theta_{\tau} $ est très voisin du $\sin\theta$ du modèle électro-faible rappelé ci-dessous (Table \ref{tab:9:table9}). Il corrèle directement avec les couplages $a_w$ et $P$ (12 ppb):
...
\sin \theta^2 - \sin \theta_{\tau}^2  \approx 1/\sqrt a_w \sin_{eff}{\theta} (m_Z) \approx 1 / P^{1/4}
 ...
 faisant apparaître le terme $\sin_{eff}{\theta} (m_Z)$ appelé "effective weak mixing angle"  (Table \ref{tab:9:table9}).
...
\section{Conclusion: vers l'uification }
...
\begin{table}
\caption[Table \ref{tab:9:table9}: Adimensional primary constants.]{Adimensional primary constants}
\label{tab:9:table9}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    %\multicolumn{4}{c}{}                   \\

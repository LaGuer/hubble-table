...
\node [anchor = west,color=black!40!green] at (0.,0.0) {$\times4/\eta $};
...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 38 Hubble radius formulas]{38 formula for Hubble radius $R$}
\label{tab:6:table6}
...
\begin{equation}
 ...
 Note that 495 is the odd part of the first Mathieu group, and the couple 495- 496 is the third perfect couple. In such a couple, the first number is the sum of the \textit{non-trivial} divisors of the second. Since $496 = C_{32}^2$ is the dimension number of the group SO(32), and $495 = C_{12}^4$, this leads to the conjecture : \textit{the perfect couple 495-496 could be the single one implying non-trivial binomial numbers}. This calls for further study.
...
The heigth numbers are all of the form "prime - 1", except $d_k = 14 and 26$, the later being the critical dimension which verifies: $ d_{26} = d_{d_6} = 106 $, so justifying the reduced Atiyah sum, with the octonion term ($2^7$) and the quaternion one ($2^3$): 

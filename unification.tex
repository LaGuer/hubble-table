...
\textit {Un tel Cosmos calculateur devant utiliser des bases de calcul supérieures à l’unité}, les 4 constantes de couplages considérées (électrique, gravitationnelle, faible, forte), $a, a_G, a_w, a_s$ sont ici les inverses des quantités officielles. On y joint la constante $P = m_P/m_e$, le rapport de la masse de Planck sur celle de l'électron, voir en Appendice le bas de la Table \ref{tab:1:table1}). Les constantes \textit{dimensionnées} sont rassemblées dans la Table \ref{tab:2:table2}) \cite{Tanabashi}. 


\section{L'Axe Topologique et le rayon \textit{invariant} de Hubble}
Dans le cadre de la Cosmologie de précision, le réexamen des corrélations entre grands nombres \cite{sanchez2} conduit à l'Axe Topologique (Fig. 1), qui montre que le rayon de Hubble est invariant et confirme la théorie bosonique des cordes . 

Cette théorie des cordes a suscité un espoir énorme quand elle a exhibé une particule de spin 2, qu'on a identifié au graviton, intégrant ainsi la gravitaton. De plus, elle a mis en évidence une correspondance holographique explicite entre des espaces-temps de dimensions différentes. \textit{Mais cette théorie bute sur le concept de l'univers en expansion}. Schwarz : "this turns out to be surprinzingly dfficult" \textit{Schwarz}.  
...
\draw [-,color=blue,dashed] (-34.0,31.) node[left]{} -- (31.,31.) node[right]{};   % Cosmos horizontal line
%\draw[blue, dotted]
%      let \p1 =  ($(B)-(center)$),
%          \n0 = {veclen(\x1,\y1)}
%      in (center) circle(\n0);
...
\node [anchor = west,color=black!40!green] at (0.,0.0) {$\times \phi_1^2$};
...
\node [anchor = east,color=black] at (-10.25,24.250){\LARGE{MICROPHYSICS}};
\node [anchor = east,color=black] at (-10.25,22.250){$y=\ln\ln(\lambdabar_e(n)/\lambdabar_{MICRO})$};
...
La complexité de la structure de l'espace-temps est trahie par les phènomènes décrits dans les deux sections suivantes. En particulier, l'absence d'effet Doppler pour l'oscillation de lapuissance du rayonnement de période $9600.60$ s est tellement hors-norme qu'aucun physicien n'y a prété attention. Seul Pecker a fait publier les travaux de Kotov et Lyuty \cite{Kotov1}.
...
où $137^2/10$ est, au premier ordre, le rapport des racines dans l'équation d'Eddington $10x^2-136x+1=0$ \cite{Eddigton}. L'analyse montre alors qu'une approximation $\Phi_1$ du nombre d'or est utilisée, 
\begin{equation}
...
Considérer l'espace-temps comme une propriété \textit{secondaire} signifie que \textit{les concepts de masse, longueur et temps sont en final réductibles à des nombres purs}. Cela justifie la relation purement numérique observée entre le Cosmos et l'Univers limité par la constante $c$ (Figure/Table xyz). 
...
\node[color=green] at (1.,0.25,1.75) (l){$\mathbf UNIVERS$};
...
\textit{La cosmologie permanente est tachyonique, liée au couple $G,\hbar$, tandis la théorie quantique correspond au couple $\hbar,c$, et la gravitation au couple $G,c$, associés respectivement au "phtonde" et "gravitonde"}, voir le "trièdre des constantes" (fig. \ref{tab:10:table10}).

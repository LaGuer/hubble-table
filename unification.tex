...
%\cite{Sanchez3}. 
En particulier, Y. Ng \cite{Ng} a cru pouvoir conclure à une mousse ("foam") quantique dont la dimension est beaucoup plus grande que la longueur de Planck $l_P$, potentiellement mesurable par ls détecteurs d'ondes gravitationnelles. Mais cette longueur $(Rl_P^2)^{1/3} \approx 10^{-15} $m est variable en cosmologie standard, où $R$ est censé augmenter. Par contre, dans la Cosmologie Permanente\cite{Sanchez2}, où l'horizon $R$ est invariant, on peut lier cette distance avec le rayon classique de l'életron $r_e$. On définit ainsi le "rayon de Nambu" $R_N$, lié à la masse de Nambu $m_N = am_e$ (le facteur 2 venant de la relation de Shwarszchild $R_N  = 2GM_N /c^$): 

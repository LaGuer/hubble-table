...
This Lucas number appears in the ppb precise formula of Table \ref{tab:3:table3}, in liaison with 137. Moreover $32^2 - N_{32} =  496$ is the above dimension of the superstring gauge group SO(32), and \textit {the third perfect number}, see below the paramount importance of this fact, unrecognized up to this day.  
 ...
 The Euler series $e \approx (1+1/n)^n$ converges rather slowly as opposed to the following series:
...
\end{equation}
 ...
 The fractional development of $e$ shows unambiguously the central gravitational parameter, tied to the Lucas number \cite{Sanchez}  $p_G = P/2^{127/2}$ 
 
 \begin{equation}
e : 2,7/5, 3\sqrt{p_G}
\end{equation}
 
This was discovered starting at the approximation $e\approx 19/7$, tied to the Archimedes approximation $\pi_{Arch}= 22/7$ and the following property of the first terms of the Combinational Hierarchy  : $2^7/3 \approx \sqrt p$. 
...
\section {Le blocage actuel de la physique mathématique}



\subsection{La rationalisation symbolique de $\pi$} 
La physique mathématique fait face à une crise grave. Le modèle standard des particules est incapable d’inclure la gravitation, se heurtant à des problèmes de renormalisation, c’est-à-dire l’élimination de quantités infinies qui apparaissent dans les calculs. Nous proposons ici l’hypothèse d’une rationalisation symbolique de $\pi$, revenant à supprimer les décimales infinies de $\pi$ au-dela d’un cetrain nombre. La précision $10^{-9}$ est recherchée, laissant peu de prise au hasard.

Cette hypothèse a été présentée comme une exigence dans une conférence à Cambridge en 1994, introduisant le Principe Holographique. En effet, par opposition aux équations différentielles, les équations globales les plus simples sont les connexions entre variétés topologiques de dimensions différentes (périmètre, aire, volume…), donc utilisent la constante $\pi$.  

Dans l’hypothèse plus générale d’un Cosmos calculateur, de même qu’un ordinateur ne peut utiliser le $\pi$ mathématique "rigoureux", $\pi$ doit être rationalisé, et, dans un premier temps, de manière symbolique, c’est-à-dire que son développement fractionnaire doit montrer des paramètres-clefs.

\subsection{La question de l'espace-temps 4D}
La théorie des cordes a suscité un espoir énorme quand elle a exhibé une particule de spin 2, qu'on a identifié au graviton, intégrant ainsi la gravitaton. De plus, elle a mis en évidence une correspondance holographique explicite entre des espaces-temps 5D et 4D.  Mais cete téorie bute sur le concept de d'univers en expansion. Une étude précédente a montré que les multiples corrélations observées entre les grands nombres de la physique mathématique pouvait être considérées comme une succession de relations holographiques 1D-2D, aboutissant en final a l'Axe Topoogique, confirmant la théorie bosonique des cordes, qui favorise la dimension 26D. Ce résutat (Fig) implique que \textit{le concept d'Univers en expansion doit être remis en question}. La courbure de l'espace-temps n'est alors qu'une propriété locale, tandis que l'Univers dans son ensemble présente une géomérie plate, euclidienne. C'est effectivement ce que les astropphysiciens consatent, et désignent sous le nom de 'condition critique'.

Pour justifier ce caractère critique, les cosmologistes ont introduit une étape originelle d'inflation. Mais celle-ci devient inutile si l'on abandonne le concept d'expansion. En efet, avec un rayon de Hubble invariant, on peut appliquer sur la sphère correspondante le principe holographique de trou noir de Bekeinstein-Hawking, justifiant ainsi directement le caratère critique, mais avec pour conséquance que le mur de Planck est reculé d'un facteur $10^61$, en contradiction avec tous les modèles téoriques actuels. Cependant cette "cosmologie cohérente"  conduit enfin à une explication de l'énormité de l'énergie quantique du vide par rapport à celle de l'univers ($10^121$).

L'énormité du Cosmos, quant à elle, a été justifiée par une exigence de quasi-continuité, qui conduit, là encore, à utiliser des approximations rationnelles de $pi$. Cette démarche rappelle la méthode d'Archimède pour approximer $\pi$. D'aileurs Atiyah se réfère aussi à cette procédure historique.


\subsection{Les paramètres-clefs} 
Le nombre de « paramètres libres » dans le modèle standard est d’une trentaine. Heureusement, certains, appelés « paramètres-clefs »,  semblent plus importants et sont mesurés dans le domaine du ppb ($10^{-9}$ ). Cet article prétend confirmer que, loin d’être "libres", \textit{ces paramètres sont des bases optimales de calcul, inconnues des mathématiques actuelles}.

Les paramètres-clefs sont ces nombres purs (dits "sans dimension") qui interviennent le plus directement dans les théories physiques (tableau 1). En particulier, on considère le rapport des masses des particules les importantes avec celle de l'électron, ce qui s'est révélé déterminant dans l'Axe Topologique. Certains paramètres sont mesurés dans le domaine du ppb, mais les masses centrales dans la théorie électrofaible rappelée ci-dessous, ne sont définis qu'à $10^{-3}$ (boson scalaire massif de Brout-Englert-Higgs), $10{-4}$ (boson chargé W), $10^{-5}$ (boson neutre Z). 


\subsection{L’inversion des constantes de couplages}
Dans la théorie standard, on procède par approximations successives, c’est la méthode dite ’’des perturbations’’. Ainsi le couplage électrique est un  nombre inférieur à l’unité, de sorte qu’on puise négliger son influence à des puissances suffisantes. On a pris l’habitude de le désigner par la lettre $\alpha$, appelée ‘constante de structure fine’, car ce nombre aparaît dans l’étude des raies spectrales, jusqu’à des puissances cinquièmes. Il est lié à la charge électrique, mais sa notation courante $e$, peut entrainer des confusions avec la base optimale des logarithmes, qui est d’importance centrale dans le cadre d’un cosmos calculateur, lequel doit utiliser des bases de calcul supérieures à l’unité. Dans cet article, la charge électrique élémentaire est notée $q$ et les 4 constantes de couplages (électrique, gravitationnelle, faible, forte) notées $a, a_G, a_w, a_s$, sont les inverses des quantités officelles. 



\subsection{La constante électrique d’Eddington 137}
Comme rappelé ci-dessus, Atiyah a récemment fait le parallèle entre la méthode d’Archimède pour la détermination de $\pi$ et la détermination de la constante électrique $a = \alpha^{-1} ≈ 137.035999084(21)$, qu’il considère comme une renormalisation. Il insiste sur la première approximation de $a$, l’entier 137 d'Eddington, en l’associant aux nombres caractéristiques 0, 3 et 7, des algèbres des réels, des quaternions et des octonions $137 = 2^0 + 2^3 + 2^7$. 
...
Cela montre de façon nette que le cosmos utilise la base $137$. Cela est confirmé par la double relation suivante, impliquant les rapports de masse $p$ proton/électron, $H$ (hydrogène/électron), et $n$ (neutron/électron), impliquant visiblement la dimension spatiale 9D des supercordes:

\begin{equation}
 p_G/ \sqrt{pH} (a/ \pi) \approx  (4n/\Gamma)^3/\sqrt{a_w} \approx  (n\sqrt{a_w}/137^2 \Gamma^3)^3  
 \end{equation}

De plus, le rapport de masse optimal $W = m_W/m_e$, qui ressort de l’optimisation des corrélations entre paramètres, est :

\begin{equation}
W ≈ 137^2 \Gamma /3d_e
\end{equation}

où $d_e ≈ 0.00115965218128(30)$ est l’excès magnétique de l’électron de masse $m_e$, intiment associée à $\sqrt a$, mais au prix de calculs qu’Atiyah qualife justement d’herculéens. Noter que cette valeur de $W$ est en désaccord de 2 $\sigma$ avec la valeur mesurée.





\subsection{La période non-Doppler des quasars et la cosmologie cohérente}
Heureusement, l’observation astronomique fournit une indication précieuse. La constante électrofaible ci-dessus semble intervenir dans la grandeur astrophysique la plus précise, la période des oscillations non-Doppler des quasars $t_K ≈ 9600.06(2) s$.

En effet, le rapport de cette période avec celle de l’électron $t_e = \hbar/m_ec^2$ est donnée par la plus simple formule éliminant $c$ entre $a_w$ et la constante inverse de couplage gravitationnelle $a_G = \hbar c/G m_p m_H$, qui intervient dans le modèle de la molécule gravitationnelle d’hydrogène, où $m_p$ et $m_H$ sont les masses du proton et de l’hydrogène: 
...
Cela donne une valeur de $G$ précise à $10^{-6}$, compatible avec la mesure du BIPM, précise à $10^{-5}$. Une optimisation des corrélations a sélectionné une valeur de $G ≈ 6.67545375 kg^{-1} m^3 s^{-2}$, précise au ppb ($10^{-9}$) par la relation suivante, où $p$ et $H$ sont les rapports de masse proton-électron et Hydrogène-électron:

\begin{equation}
p_G^2  = \hbar c/2^{128} G m_e^2 \approx p^7/d_e^2 H^5
\end{equation}

et $Z$ est défini par la première partie de la relation suivante, généralisant la relation connue $W^8 \approx a_G$, qui constitue l’un des piliers de l’axe topologique:

\begin{equation}
(R/\sqrt{pH})^{1/4}  \approx  WZ \approx (R \lambdabar_e/\lambdabar_p \lambdabar_H W^8)^3 
\end{equation}

où $R =  2 \hbar^2/Gm_e m_p m_H$ est le rayon de Hubble invariant de la cosmologie cohérente, définit par le modèle de la molécule gravitationnelle d’hydrogène, où $\lambdabar_p =  \hbar/m_pc$ et $\lambdabar_H =  \hbar/m_Hc$. 

Ce modèle est compatible avec les mesures directes du rayon de Hubble qui contredisent la cosmologie standard.

La formule ci-dessus implique le terme $W^96$, qui est très particulier car très voisin de $Z^95$. En effet, à $10^{-7}$ près, l’ordinateur indique:

\begin{equation}
(W^{96}/Z^{95})^2  \approx  (ad_ep/137)^3/Hnp_G  
\end{equation}

voir ci-dessous deux autres corrélations à $10^{-9}$. 

Cela montre l’insuffisance de la cosmologie standard, qui suppose que $R$ est variable, empêchant ainsi d’appliquer le principe holographique, pourtant présenté comme l’espoir de la nouvelle physique. L’invariance de $R$ est confirmée par des conservations holographiques simples, qui conduisent à:

\begin{equation}
R/\lambdabar_e  \approx  (2\pi^2a^3)^5
\end{equation}

où $2\pi^2a^3$ est l’aire de la sphère 4D de rayon $a$, mais cette relation montre une imprécision de 565 ppm.
...
En introduisant la formule de Lenz-Wyler pour le rapport de masse proton-électron :
...
on constate que la déviaton ci-dessus est voisine de  $H/p_W$, à 1.5 ppm près. Et ce dernier écart est réduit au ppb si on considère la valeur suivante de $\pi$:
...
La formule de Wyler pour $a$ est:

\begin{equation}
(3\sqrt a/4)^8  \approx 120 ~\pi^{11} 
\end{equation}

qui s’écarte de la valeur mesurée de 0.608 ppm. Cet écart minime a suffit pour que la théorie de Wyler soit rejetée. En examinant quelle valeur $\pi_W$ conviendrait, on note que le dévelopement suivant rétablit la précision souhaitée de $10^{-10}: 3, 7, 16, -163/2, 4\pi \sqrt a + 1/sin\theta^2 $, où apparaît l’angle de mixage, rappelé ci-dessous. Le nombre 163 est le célèbre nombre terminal de la série de Heegner-Stark.

Le nombre $p_G$ ci-dessus, caractéritique de la gravitation, se décline sur une valeur de $\pi$ singulière, visiblement liée à l'algèbre des octonions. \textit{Sir Atiyah avait effectivement annoncé que la gravitation devait être liée à cet algèbre}:
...
Le modèle standard de la physique des particules a unifié les forces électriques et nucléaires faibles dans le modèle "électrofaible". Mais la théorie de jauge invoquée \cite{Taylor} \footnote{Taylor J., The New Physics, Gauge theories in particle physics, Ed. Paul Davies, C.U.P, p. 471.}, laisse des paramètres numériques indéterminés, comme les charges de jauge {$q'$} (groupe SU(2)) et {$q''$} (groupe U(1)), reliés à la constante électrique $a$ par :
\begin{equation}
 a/4 \pi = 1/q^2 = 1/{q'}^2  + 1/{q''}^2 
 \end{equation}
La notation officielle $g$, {$g’$} est moins symétrique. Cela correspond aux paramètres numériques:
\begin{equation}
 4\pi / {q'}^2  = {a'}  ≈ 30 ,    ~~~~         4\pi / {q''}^2  = {a''}  \approx 107
 \end{equation}
...
Ces relations sont illustrées par le "triangle de Weinberg" et son correspondant le « triangle électrique », voir figures 1 et 2.

La charge électrique $q = \sqrt{4\pi/a} $ montre une connection avec la forme trigonométrique $a \approx 44\pi - Arccos(1/e)$

\begin{equation}
a \approx 44\pi_a + ln(\sqrt {4\pi_a/a})     ~~~~ \rightarrow 6\pi_a^5 \approx p ~~~~ \pi_a : 3, 7, e^e + 1
 \end{equation}

Ainsi la charge électrique $q$  est intimement relié au couplage électrique $\sqrt a$. Le triangle représentatif de ce dernier montre une nette connexion avec les puissances 2 et 4 du nombre d'or, ce qui avait été également prédit dans la conférence de Cambridge.
...
\tkzLabelSegment[sloped,right,above,text=red](A,B){$m_Z/m_{BEH}$}
\draw[blue, dotted]
      let \p1 =  ($(B)-(center)$),
          \n0 = {veclen(\x1,\y1)}
      in (center) circle(\n0);
...
\end{tikzpicture}
\caption[Weinberg Triangle properties]{Weinberg Triangle.}
\end{figure}
...
\tkzDefPoint(10.,0.){E}
\tkzDefMidPoint(A,B) \tkzGetPoint{O}
\tkzInterLC[R](A,B)(B,-0.3cm) \tkzGetSecondPoint{D}
\tkzInterLC[R](B,E)(E,-1cm) \tkzGetSecondPoint{F}
%\tkzDefLine[orthogonal=through D](A,D)
\tkzDefLine[perpendicular=through F](O',E) \tkzGetPoint{G}
\tkzDefLine[perpendicular=through D](O,B) \tkzGetPoint{J}
\tkzDefMidPoint(F,G) \tkzGetPoint{H}
\tkzDefMidPoint(J,D) \tkzGetPoint{C}
...
\tkzDrawSegment[color=blue](H,C)
\tkzDrawSegment[color=blue](B,E)
\tkzDrawSegment[color=red](A,C)
\tkzDrawSegment[color=red](A,B)
\tkzDrawSegment[color=red](C,B)
\tkzDrawPoints(A,B,C,O,O',D,E,H)
\tkzLabelPoints(A,B,C,D,O,E,H)
...
\draw[blue, dotted]
      ...
      in (center1) circle(\n0);
%\draw [<->] (-1.8,1.7) node[right]{} -- (O) node[left]{};
\node [anchor = right,text=blue] at (3.,-3.5) {\large $1+\mu+\tau$};
\draw [<->] (O') node[left]{} -- (3.,-7.) node[right]{};
\node [anchor = left,text=red] at (-2.,-1.) {\large $q'/2$};
\draw [<->] (O) node[left]{} -- (-2.,-2.) node[right]{};
\tkzMarkRightAngle(A,H,E)
\tkzDefMidPoint(H,E) \tkzGetPoint{I}
\tkzLabelSegment[left,text=blue](H,E){\large $\tau$}
%\tkzMarkAngle[fill=blue!20,size=0.4cm,opacity=.5](E,A,H)
\tkzLabelAngle[pos=1.4](E,A,H){$\theta$}
\end{tikzpicture}
\caption[Weinberg-Koide Circles]{Weinberg-Koide similitude circles.}
\end{figure}
...
\subsection{Le boson de Brout-Englert-Higgs}
Le modèle standard de la physique des particules a recu une éclatante confirmation lors de la découverte du boson scalaire de Brout-Englert-Higgs. Le champs scalaire est rajouté au modèle de jauge pour produire une masse aux bosons, selon le potentiel :


\begin{equation}
V(\Psi) = \mu_0^2 \Psi\Psi_1 + (\lambda^2/2) (\Psi\Psi_1)^2
\end{equation}

 et, avec la valeur moyenne dans le vide $v \approx 246$ GeV :


\begin{equation}
(\Psi\Psi_1)_{mean} = v/\sqrt{2} = \mu_0/\lambda
\end{equation}
 

Au premier ordre de la théorie de perturbation, les masses sont : 


\begin{equation}
m_{BEH} = \lambda m_v,  ~~~~ m_W = {q'}m_v/2
\end{equation}
 
On observe que $m_v$ est compatible avec le double de la masse du boson scalaire, laquelle est compatible avec $495^2 m_e$, où 495 est l’antécédent du troisième nombre parfait 496, central en théorie des supercordes. De plus, 495 est la partie impaire de l’ordre $16 \times 495$ du premier groupe de Mathieu, et le triple du nombre de symétries impaires dans le groupe cristallographique $10D$. On suppose donc, avec la masse de l’électron comme unité :


\begin{equation}
\lambda  = 1/2,  ~~    v_0 =  495^{2}, ~~~~ W / 495^{2} =  {q'} =  \sqrt(4\pi_e/{a'}) 
\end{equation}
   

La précision actuelle sur la mesure des masses $W$ et $Z$, de $1.5 \times 10^{-4} et 2.3 \times 10^{-5}$ est insuffisante pour caractériser les approximations de $v$ et $\pi_e$ réalisés dans cette formule. Avec les valeurs optimisées $W \approx 157340.1093$ et $Z \approx 178451.7402$  rappelées ci-dessus, on observe que le nombre $495$ corréle avec le facteur gravitationnel $P$ et la constante d'Atiyah $Gamma$ dans la formule rès symétrique suivante (domaine ppb):

\begin{equation}
(495 W a/ 137 Z)^3 \approx P (H/np)^4/\Gamma 
\end{equation}
 
La valeur $\pi_e$ connecte avec une approximation $\Phi_e$ du nombre d'or, et manifeste une rationalisation symbolique singulière de $\pi$, :

\begin{equation}
\pi_e = ({q'} \sin\theta)^{2} a/4 = 4/\sqrt{\Phi_e}   \approx   3 + 2 \pi^2 a^3 /(137 a_0)^{2}   
\end{equation}

où $2 \pi^2a^{3}$ est l’aire de la sphère 4D de rayon $a$, centrale en cosmologie, comme rappelé ci-dessus, et $a_0 = (a_w/WZ)^{2}$ est la valeur brute de $a$, en connection directe avec 137 (3.4 ppm):

\begin{equation}
a_0 = (a_w/WZ)^{2} \approx 137^{8}/a^{7}
\end{equation}
 
Avec les valeurs officielles de $W$ et $Z$, l’élégance de cette formule, \textit{visiblement liée à l’algèbre des octonions}, pourrait faire croire qu’elle donne la valeur précise de $WZ$.  Mais l’ordinateur confirme les valeurs individuelles ci-dessus de $W $ et $Z$ par la formule suivante, dans le domaine du ppb:

\begin{equation}
(137^3W^2a^5/\sqrt{a_w})^{7} \approx (Z^{6} \sqrt{a_w})^{3} \approx 
(1837p/16)^{3 \times 7} \approx (4 \sqrt{a_s})^{3 \times 5 \times 7} 
\end{equation}

Cette relation, dont le type a été prévue par la théorie holique, relie donc les paramètres électrofaibles avec la masse du proton et le couplage fort $a_s$, Il y a, de plus, une correlation avec le paramètre gravitationnel $P$ car le grand nombre ci-dessus est voisin de:

\begin{equation}
P^5 / (1+sin\theta^22) \approx (4\pi/3) e^{256}
\end{equation}

De plus, $a_0$ s’inscrit dans la relation directe suivante entre gravitation et force électro-faible (-1.1 et 1.5 ppm):

\begin{equation}
P/n^3 \approx aWZ(H/n)^{3/2}\beta^4 \approx \sqrt{aa_0}~\beta^{-7/2}
\end{equation}
  
où $\beta = 1/(H-p)$ est la correction de masse dans l’atome d’hydrogène.
 
La valeur spéciale ci-dessus $\pi_e$ implique une valeur spéciale  $\Phi_e$  du nombre d’or, qui est très voisine (32 ppm) du nombre d’or défini par (0.8 ppm):

\begin{equation}
\Phi_{WZ} = (WZ)^{1/50} \approx (w+z)^{1/4}
\end{equation}


où $w = \sqrt a_w/W$ et $z = \sqrt a_w/Z$, avec $wz = \sqrt a_0$. Ainsi qu’il avait été prédit, le nombre d’or joue une place centrale dans la future théorie ultime. L’exposant 50 doit être lié à l’espace 5D, voir ci-dessous. L’ordinateur indique les corrélations avec le rapport de masse des leptons tau et mu, à 0.2 ppm et 4 ppm :


\begin{equation}
z (w+z)  \approx p^4n/H(\mu \tau)^2
\end{equation}

\begin{equation}
(n^2/\mu \tau)^2 \approx  (d_e{a'})^5/{a''}^3
\end{equation}

Particulièrement pertinente est la corrélation à 23 ppm près :

\begin{equation}
{a'}/{a''} = cotg~\theta  \approx  \tau/9\mu
\end{equation}

où $9\mu$ est voisin du rapport proton/electron, conformément à \textit{la prédiction d’Eddington : une symétrie proton/tau complètement oubliée, malgré qu’Eddington ait prévu le tau, 30 ans avant sa surprenante découverte, avec une bonne estimation de sa masse}. 


On constate que $1/sin\theta - 1/cos\theta$ est voisin de 1, et en particulier:

\begin{equation}
i^{\pi/i} \approx \sqrt a + d_e/\sqrt a \approx   137(1/sin\theta - 1/cos\theta) 
\end{equation}

où $i^{\pi/i} = e^{\pi^2/2}$ implique le nombre $i^{1/i}$, extrapolé du terme central $e^{1/e}$, qui perment de défnir la base optimale $e$. Il est étonnant que les mathématiciens orientés vers les applications physiques, tels que Sir Atiyah, n'aient pas insisté sur ce point. Ce nombre se découvre dans l'ordre du groupe monstre:

\begin{equation}
lnlnO_M \approx  i^{1/i}
\end{equation}
...
\subsection {La relation gravito-électrofaible }
En complétant le triangle de Weinberg ci-dessus par un troième axe portant l'unité, on obtient un vecteur de module diretement lié à la constante gravitationnelle $p_G$ ci-dessus:

\begin{equation}
 1 + {q'}^2  + {q''}^2 = 1 + (m_Z/m_{BEH})^2 \approx 2a^3/pp_G
 \end{equation}
 
 La précision de cette relation ($10^{-7})$ confirme la valeur ci-dessus du boson scalaire $ m_{BEH} = 495^2 m_e $. Rappelons que le terme $2a^3/pH \approx (R/\lambdabar_e)^{1/2\times 3\times 5\times 7}$ est la clef holique du rayon de Hubble. 
...
\subsection{La connexion avec le générateur de Lucas-Lehmer}
...
\subsection{La validation de la relation de Koide}
La valeur optimale de $\tau$ est déduite de la formule empirique de Koide et de la relation $\mu \approx a\sqrt{(a_w/pH}$ précisant, au ppb près, la mesure, déjà à 10 ppb, du rapport de masse muon-électron. A noter que la simplicité de cette formule de Koide ne reçoit aucune explication dans le modèle standard, qui manifeste, là encore, son insuffisance. De plus, l’écriture la plus symétrique de cette relation de Koide présente aussi une rationalisation symbolique de $\pi$:

\begin{equation}
(1+\mu+\tau)/2 = (1+\sqrt\mu+\sqrt\tau)^2/3 = p_K ≈ 6\pi_K^5 (1+(\mu+\tau)^2)   ~~~~ \rightarrow   \pi_K: 3,7,16,-(2×137)2/3 
\end{equation}
...
%une interaction entre la matrice d’espace-temps $4 \times 4$ et la matrice $5 \times 5$. The later was indeed predicted by Eddington, who showed that the Dirac algebra corresponds to a ﬁve-dimensional base space. 


%%En effet, d’après Salingaros: \textit{Indeed, it seems that Eddington was really ahead of his time : Eddington anticipated results of current interest. He discovered the Majorana spinors, and was responsible for the standard \gamma^5$ notation as well as the notion of chirality. Furthermore, Eddington deﬁned Cliﬀord algebras in eight and nine dimensions which are now appearing in grand uniﬁed gauge and supersymmetric theories. A point which Eddington cleared up, yet is still misunderstood, is that the Dirac algebra corresponds to a ﬁve-dimensional base space}.

L’analyse montre les deux relations à $10^{-7}$ près:

\begin{equation}
(2 + \sqrt 3) \approx \tau (137^2 +e)^2/a_w
\end{equation}

\begin{equation}
1+cos\theta  \approx  \tau/(4\pi_{\tau})^2 \sqrt a    ~~~~   \rightarrow ~~~~   \pi_{\tau} : 3, 7, \sqrt a
\end{equation}

La pertinence de la rationalisation symbolique de $\pi$ est confirmée. Le terme symétrique $1-cos\theta$ est visin du couplage fort $1/a_s$

\subsection {Le triangle d'or}
 
Le triangle regtangle de hauteur $\sqrt {a_0}=\sqrt{wz}$ s'inscrit dans un cercle de rayon $(w + z)/2 = {a_1}^{1/4}$, et présente des propriétés très particulières liées au nombre d'or, voir fig4. La valeur de $a_1$ vérifie:

\begin{equation}
a_1 \approx \pi_1^4\sqrt2    ~~~~   \rightarrow ~~~~   \pi_1 \approx (\sqrt{pn}/6)^{1/5}: 3, 7, \sqrt{H\sqrt{nH}/3p}
\end{equation}

Cette double caractérisation  de $\pi_1$  montre que les masses des particules principales sont effectivement liée à la théorie de Wyler. L'extension 3D untaire (fig 4) semble reliée à la matrice d'Eddington.

La pertinence de $a_1$ est confirmée par (60 ppb):

\begin{equation}
\sqrt P \approx (a_1/a)^2 137WZ/\Gamma
\end{equation}

une nouvelle confirmation de la pertinence du rôle unificateur de la constante d'Atiyah $\Gamma = \gamma a/\pi$.

\subsection{La rationalisation symbolique directe de $\pi$}

Les termes du développement fractionnaire de $\pi$ sont : $3, 7, 15, 1, 292.63459$, où ce derner terme, d’amplitude fortement improbable, ne reçoit aucune explication mathématique. Or il s’identifie, à $3 × 10^{-6}$  près avec le rapport $n$ de masse neutron-électron, divisé par $2\pi$:

\begin{equation}
p_5 \approx  n/2\pi
\end{equation}

\subsection{Les nombres économiques naturels}
La relation la plus remarquable entre $\pi$ et la constante d’Euler $e$ est $lnlnln\pi \approx -2$. La valeur de $\pi$ correspondante exhibe une approximation du paramètre de jauge ci-dessus ${a'}$ dans le troisième terme du développement:

\begin{equation}
\pi_e  = e^(e^(e^(1/e^2))) : 3 + 1/(7 + 1/30.53186) 
\end{equation}

Il se confirme la pertinence des nombres économiques naturels, basés sur $e$. On le constate dans les relations suivantes à -1,69, -1,73 et 0.02 millièmes près :

\begin{equation}
P^4 = (\hbar c/Gm_e^2)^2 \approx e^(e^(e^e))^{1/(a-1)^2} \approx (9/2)^a \approx (a/137) (1/\sin\theta)^{ja}
\end{equation}

où $j = 2d_e$ est le moment magnétique de l’électron.

\subsection{La réhabilitaton de l'équation d'Eddington}

Il s'agit ci-dessus d'une "corrélation symbolique". La recherche de corrélation polynomiale débouche sur (ppb domaine), où $E = e\times e^e\times e^(e^e)$

\begin{equation}
2\times 137 WZF / 3\times pp_K  \approx Pp_E {q''} \sin\theta/495 \Gamma n\mu \tau  \approx 495 \Gamma E a{a'}/a_0 a_1 
\end{equation}

%La symétrie de cette double relation unificatrice, où les quatre forces sont rerésentées, garantit sa pertinence. On note le rôle central %de $495 \Gamma$, ce qui montre une liason intime enre le boson BEH et la constante d'Atiyah. 

Le terme ${q''} \approx a_s^{-1/2}$ est lié à l'interaction forte. Le terme $p_E^approx 1847.599459$ est le rapport des solutions de l'équation d'Eddington. La relation $Pp_E \approx (r_H/\lambdabar_e)^{12} = (aH/p)^{12}$, où $r_H$ est le rayon de Bohr, induit l'étude de ce dernier facteur. En effet l'expression symétrique $(r_H/\lambdabar_e)^{12}Rl_K/O_B^2$, où $O_B$ est l'ordre du Bébé Monstre, corrèle à 50 ppb avec les quantités ci-dessus au degre polynomial 2. 

\subsection{La connexion avec la table périodique des élements}
...
\subsection{Les quatre algèbres principales}
...
Or si on rétablit l’algèbre des nombres complexes qu’Atiyah avait ôté de la série, on aboutit au nombre 139, la partie entière de $a_{00} = i^{\pi/i} = e^{\pi^2/2}$. \textit{Il apparaît que 137 est la moyenne de ces deux nombres}. 
...
\label{tab:1:table1}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    $\pi$ fithth fractional term & $\pi_5$    & 292.63459101440 & "exact" \\ 
    ...
    Third coperfect number  & $n_{3}$    & 495 & exact \\ 
    
    
    Electric constant  & $a_{mes}$    & 137.035999084(21) & 0.15 \\
    
    Measured massive scalar boson/Electron mass ratio  & $H^0_{meas}$ & 245000  & $10^6$ \\
    Optimized massive scalar boson/Electron mass ratio  & $H^0$ & $n_{3}^2 = 495^2$  & exact \\
    
    Measured W boson/Electron mass ratio  & $W_{meas}$ & 157297  & $1.5 \times 10^5$ \\     
    Optimized  W boson/Electron mass ratio  & $W$ & 157340.1093  & 1 \\     
    Measured Z boson/Electron mass ratio  & $Z_{meas}$ & 178450  & $2.3 \times 10^4$ \\     
    Optimized  Z boson/Electron mass ratio  & $Z$ & 178451.7402  & 1 \\     
   
   
   Optimized "weak-mixing angle" $1-(W/Z)^2$ & $(\sin\theta)^2$    & 0.2226129168 & 1 \\
  Codata "weak-mixing angle" & $(\sin\theta_{meas})^2$    & 0.22290 & $3\times 10^5$ \\
  
  Particle Data Group "weak-mixing angle" & $(\sin\theta_{meas}(m_Z))^2$    & 0.23122(4) & $1.7\times 10^5$ \\
  Particle Data Group strong coupling constant & $1/a_s$    & 0.1179(10) & $8.5\times 10^6$ \\
  
   Optimized coupling & $ q_1 $    & 0.6421390034 & 0.15 \\
    Optimized gauge coupling & $ q_2 $    & 0.3436256462 & 0.15 \\
   Optimized electric charge & $ q $    & 0.302973211 & 0.15 \\
     
    Excess Electron magnetic moment / Bohr magneton  & $d_e$    & 1.00115965218128 & 0.15 \\    
    Atiyah constant & $\Gamma$    & 25.17809724196  & 0.15 \\          
    Eddington's proton-electron mass ratio  & $x_1 = 1 / 10 x_2$    & 1847.599459   & "exact"\\         
    ...
    Rydbergh correction constant $(H-p)^{-1}$ & $\beta$  & 1.000026597 & 0.3   \\ 
...
Optimised Strong coupling constant $a_w/2\pi(pH)^{3/2}$ & $a_s$   & $8.434502906$ & this work ppb  \\
...
\section{The connection with the Periodic Table of Elements}
 ...
 Now the above numbers 19 and 59 are the Crystalline Ponctual Symmetry Operation numbers ($PSO_{Cr}$), respectively negative and positive in 6D crystallography \cite{Weigel} \cite{Veysseyre} (Table \ref{tab:10:table10}) : $k_{6-} = 19$, $k_{6+} = 59$. Note that this dimension d = 6 corresponds to k = 1 in the Topological Axis (Fig. \ref{tab:9:table9}). So the above definition of 137 writes:
  \begin{equation}
...
%Note that the roots of the crystallographic algebraic equation of degree $n$ are of type $exp(i2\pi m/l)$, where $l$ and $m$ are whole numbers such that $l \leq n$ and $l \leq m \leq l$,  : this is similar to the above spectroscopic series. Such an unexpected connection needs also further study.
    ...
    %{ Eddington predicted the tau fermion, 35 years before its surprising discovery, calling it «heavy Mesotron», with a right order prediction of its mass [13]. This was very surprising because the Eddington theory, accused of pythagorism, was rejected. But he also predicted the importance of the $N{_16} = 136 elements of a symmetrical matrix $16\times 16$, giving 137 by adding unity, whose pertinence is confirmed by the above Central relation $a_w = (2Ã—137 G)^3$. Atiyah presented this number by the form $137 = 2^0 + 2^3 + 2^7$.} 
...
\bibitem{Weigel} Veysseyre R., Veysseyre H., and Weigel D. "Counting, types and symbols of crystallographic Point Symmetry Operations of space E n" AAECC 5, 53--70 (Nov.1994) DOI: 10.1007/BF01196625 ISBN: 0938-1279.\\

...
\textit{Cette Holographique Toponique explique enfin l'énormité ($10^{121}$) de l'énergie quantique du vide par rapport à celle de l'univers La matière noire serait une oscilation matiére-antimatiére en quadrature}\cite{Sanchez2}.
...
La table 5 présente 24 formules pour le rayon du Cosmos. On y remarque le terme $210^{210}$ prévu par la théorie holique rappelée ci-dessous, ainsi que le terme $\mu^{\mu}$. La proximité $\mu \sim 210$ est ainsi jusifiéee. Noter que cette approximation est centrale dans la théorie des bit-strings \cite{Noyes}.
...
où $\Pi_{par}$ est le produit des ordres des 6 groupes sporadiques dits "parias" \cite{Sanchez2}, et $4181$ est le 19ième terme de la suite de Fibonacci, qui se singularise par le fait qu'il est divisible ($4181 = 37 \times 113$), alors que tous les termes précédents d'ordre premier sont aussi premiers. 
...
L'analyse montre qu'une approximation $\Phi_1$ du nombre d'or apparaît: 
\begin{equation}
...
montrant que \textit{le Cosmos utilise à la fois, et de manière symétrique, $\pi$ et la base néperienne $e$}. Noter que les séries de Riemann d'ordre paires utilisent effectivement des puissances de $\pi$, lequel se comporte donc bien comme une base de calcul. L'une des 22 formules donnant R (table 3) implique effectivement des puissances de $\pi$.
...
\textit{La série d'Hermann privilégie donc les espaces de dimensions 3 et 4, répondant à une interrogation séculaire des théoriciens} 
...
tandis que, avec $ d_{26} = d_{d_6} = 106 $, la relation caractéristique de la dimension critique 26: 

\begin{equation}
 \sum_{k=0}^{k=7} (d_k+1)= 2^7 + 2^3 = 136 = d_7 + d_{d_6}
 \end{equation}

justifiant la "séparation de jauge", rappelée ci-dessous  $136 = d_7 + d_{26} = 30 + 106$ et la somme canonique d'Atiyah, intégrant les octonions ($2^7$), et les quaternions ($2^3$). Ce nombre primaire d'Eddington 136 est le nombre d'éléments indépendants dans la matrice symétrique $16\times16$.
...
où $5$ et $3^3$ sont les deux premiers nombres co-parfaits, précédant 495. Si on rétablit l’algèbre des nombres complexes qu’Atiyah avait ôté de la série, on aboutit au nombre $2^0 +2^1+2^3+2^7 = 139$, la partie entière de $a_{00} = i^{\pi/i} = e^{\pi^2/2}$. \textit{Il apparaît que 137 est la moyenne de ces deux nombres 135 et 139}. 
...
L'analyse conduit à la double-ppb relation suivante confirmant la valeur de Z: 
 ...
 \begin{equation}
 (aZ)^2pH/p_G^3 \approx a_w + (a_w/Z^2)^3 \approx (\sqrt a_w + \pi_q/3)^2
 \end{equation}
...
\begin{equation}
 ...
 L'analyse conduit à la double-ppb relation suivante confirmant la valeur de $Z$ dans le domaine du ppb: 
...
\textit{La constante d'Atiyah $\Gamma$ confirme donc la valeur de $\sin~\theta$, donc de la masse $495^2$ du boson scalaire et la valeur de $G$ dans le domaine du ppb. Le programme d'Atiyah \cite{Atiyah} est ainsi en cours de réalisation}.
...
%\end{equation}
 ...
 Avec $u_0$ = 5, $d_0$ = 6, les masses représentatives des quarks up et down \cite{Sanchez1}, et $f(8) = e^4$, situé à mi-chemin du terme central $f(16)=e^{16}$  on observe que les combinaisons $u_0-u_0-d_0$(proton) et $u_0-d_0-d_0$(neutron) apparaissent dans:
...
Les bases entières fondamentales 2,3, et 5 de la sensation musicale, respectivement liés aux spins 1/2, 1 et 2, sont reliés au Principe Holique. Mais comme le Nombre holique $210^{210}$ joue un rôle central dans cette étude, il faut s'attendre à une particule de spin 3. 

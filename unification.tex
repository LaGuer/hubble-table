...
1839 = \sum_{10}^{12} K_d = \sum_1^{12} (N_d/2) + 3   ~~~~ \Rightarrow~~~~ \sum_2^9K_d = (K_{12}-9)/2 = 496  
 ...
 This identifies the partial sum untill the 9D space with the above $496= (^{32}_2)$, the dimension of the canonic group SO(32), central in string theory. They could be identified with the above 9 dimension reduction. ${\substack{\\ (2)~\mathrm{even}}}^{32}$
 %% replaced C_{32}^2 by (^32_2)  {\substack{n=0\\ (n+k-i)~\mathrm{even}}}^{k-i}
 .

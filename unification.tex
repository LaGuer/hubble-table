...
\label{tab:1:table1}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    %\multicolumn{4}{c}{}                   \\
    \cmidrule(r){1-4}
    $m_G$ & $m_{\hbar}^2$    & $r = \hbar^2/Gm_G m_{\hbar}^2$  & property \\
    \midrule
    
    
    $m_N $ & $m_N^2$   & $R_N/2$ demi-rayon de l'hologramme du Cosmos & $R_N/\lambdabar_e \approx (3^3)^{3^3}$\\
    $m_{bc} $ & $m_{bc}^2$   & $2l_{cc}$ double longueur non-Doppler & $ l_{cc}/\lambdabar_e \approx \pi^{50}$\\
    $m_e $ & $m_pm_H$   & $R/2$ demi-rayon de l'Univers & $R/\lambdabar_e \approx g(6) \approx 2^{128}$\\
    

    $m_Pa^3 $ & $m_pm_H$   & $\lambda_{Wn}$ longueur d'onde de Wien du rayonnement de fond & $\lambda_{Wn}/l_P \approx \pi^{64}$\\
    
    $m_{bc}R/R_N $ & $m_{ph}m_{gr}$   & $R_C$ rayon du Cosmos & $ R_C/\lambdabar_e \approx 3^{128}$\\

...
Les masses brutes n'impliquant que les isotopes proncipaux $(~^{1}_{1}H^{(0)} = H,~ ^{6}_{12}C = C^{(0)}?,~ ^{7}_{14}N = N^{(0)}, ~^{8}_{16}O = O^{(0)},~^{15}_{31}P= P^{(0)})$ des 4 nucléotides de la molécule d'ADN, sont, rapportées à la mase de l'hydrogène \cite{Huang}:
...
\begin{equation}
 \left\{
    \begin{array}{ll}
        ...
        Couple AT~masse~brute~~ o_2 \approx  613.299802
    \end{array}
\right.
\end{equation}
...
La masse moyenne du bi-codon, composé de six nucléotides $m_{bc}/m_H = (6/4)(o_1 + o_2) \approx 1838.418122$  est donc très voisine de $p_W = 6 \pi^5$, le rapport de masse proton-electron de Lenz-Wyler \cite{Wyler}. A 0.1 ppm près:

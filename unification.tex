...
The Topological Axis rehabilites the string theory and the Atiyah octonion approach. It refutates the current cosmology, but confirms the Coherent Cosmology. The extreme form of the Calculation Principle, the Holic Principle, implies an “Hyper symmetry” between $G$ and $\hbar$, leading to a new expression of Bohr’s orbits, showing a connection between fundamental masses and the Hubble and Cosmos radiuses and their common CMB temperature (to 0,03 \%). Among the fundamental masses, the mean DNA bi-codon mass appears as entral. It is directly associated with the very precise quasar no-local wave-length, which is associated with the photon and graviton masses. The Universe appears then as a c-local cosmic “immergence” of the Cosmos. The mean DNA bi-codon confirms the Topological Axis, to $10^{-7}$. The physical parameters and the periodic table of chemical elements are related to the crystallographic symmetries in higher dimensions without ambiguity, leading to a Higgs mass $495^2 m_e$. So the Multiverse model must be replacd by the research towards a Science-Arithmetic Unification.
...
\section{Introduction: The Topological Axis}\label{AxeTopologique}
...
In spite of a set of successes towards unification, the mathematical physics strikes itself to the obstructing of the current standard cosmology. Indeed, despite the optimisation of six parameters, this model finds 67,74(46) km/s by Megaparsec for the present-day Hubble’s constant \cite {Zyla}; meanwhile, the team of A. Riess finds the value 75,8(5) i.e. a damning difference of $9 \%, i.e. 13 \sigma$ \cite{Riess}. The most recent precise measure is the one of the Freedman’s team: $69,8(8)$ \cite{Freedman}, an intermediary value between the previous two values and this result agrees with the prediction $70,79$ of the Coherent Cosmology [3] \cite{Sanchez4}.
...
In parallel, the string theory favor the Multiverse thesis, a renunciation to the ancestral research for Unity. The observed “fine-tuning”, so favourable to Life, is then justified by the “anthropic principle”, borning to say that the existence of a world where we can live is statically possible. Moreover, the \textit {single} argument to justify the famous correlations between the great numbers observed by Eddington and Dirac \cite{Sanchez4} is precisely of the same “anthropic” type: in the frame of the evolutive standard Cosmology, we would live a particular cosmic moment where the supposed age of the Universe should be \textit {comparable} with a star lifetime able to spread vital atoms on the space \cite{Rees}.
...
But this is clearly refutated by the Topological Axis (Table \ref{AxeTopologique}). Indeed, this Axis shows that the multiple observed correlations between great numbers of the mathematical physics can be considered as a sequence of holographic relations 1D-2D, confirming the bosonic string series $d = 2+4k$ and its favoured dimension $26 ( k = 6)$, \textit{which corresponds, within 0,1 \%, to the Hubble radius given by the model of the gravitational hydrogen molecule \cite{Sanchez2}}.
...
Hence, the accuracy of these relations excludes any “anthropic” explanation, as well as the concept of "Universe age".  Indeed, this Axis advocates the worldly invariance of the parameters, therefore restablishing the “steady-state” model of Hoyle \cite{Hoyle} and Bondi and Gold \cite{Bondi2}. Moreover, the limiting value k = 7 (d= 30) Topological Axis favors the Atiyah's octonion algebra approach \cite{Atiyah}.
...
According to the word of B. Russell, the one who has proved that 1 + 1 = 2 in 180 pages \cite{Russell}, “The most incredible in the modern science is its return to the Pythagorism” \cite{Alcina}. Indeed, the Pythagorism agrees with the quantic nature of the world, especially with the quantic numbers of the particle physics and with the dimensions of the special series $d = 2 + 4k$ of the string theory emphasised by the Topological Axis. \textit{In particular, this paper show the strong correlation between the Topological Axis and the periodic table of the elements}.
...
In fact, more generally, the Science is characterised by conservation laws, in physics as well as in biology. The theoreticians associate conservation and symmetry but, more fundamentally,  \textit {conservation implies calculation, justifying the pre-eminence of the Arithmetic. Therefore, the classical hypothesis of the continuum is eliminated}.

\textit{This paper supports the existence of an Ultimate Theory which unifies Arithmetics, Physics, Chemistry and Biology}.
...
\section{Le Computing Principle}
...
The main hypothesis of this article is that the Cosmos, considered as a black hole of horizon $R_c$ is a super-computer \cite{Sanchez3}. This subject is much debated today. In particular, Y. Ng \cite{Ng} has introduced a “quantic foam”, with a scale much greater than the Planck’s length $l_P$, so potentially measurable by the gravitational wave detectors. But, this length $(Rl_P^2)^{1/3} \approx 10^{-15} m $ is variable in standard cosmology because the Universe horizon $R$ is supposed to increase. On the opposite, in the Permanent Cosmology \cite{Sanchez2}, where the horizon $R$ is invariable, this distance can be linked to the classical radius of the electron $r_e$. Hence, we define the “Nambu radius” $R_N$ attached to the Nambu mass $n_N = a m_e$ [13], with $a\approx 137$, central in physics of particles, the Schwarzschild’s relation $R_N  = 2GM_N /c^2$ with the critical mass $M_N$ bringing the factor 2:
...
This radius $R_N$ is slightly greater (30\%) than the horizon $R$ of the Universe. \textit {It is considered as the holographic reduced radius of the Cosmos through its Bekenstein-Hawking entropy }: $\pi (R_N/l_P)^2 = 2\pi R_C/l_P$. Hence the Cosmos radius would be:
...
R_c = R_N^2/2l_P \approx 9.075773376 \times 10^{86} m 
 ...
 textit {So, infinity, as well as continuity, are rejected by the Computing Principle}.

%\textit{Une hypothèse complémentaire de cet article est que les constantes adimensionnées de la physique sont des bases privilégiées de calcul.} C'est pourquoi la constante $a$ ci-dessus est l'inverse de la ‘constante de structure fine’ $\alpha$, car les bases de calcul doivent être des nombres plus grands que l'unité. Ce paramètre $a$ est lié à la charge électrique, mais la notation courante de celle-ci, $e$, peut entrainer des confusions avec la base optimale $e$ des logarithmes néperiens, qui est d’importance centrale dans le cadre d’un cosmos calculateur, comme cet article le confirme. La charge électrique élémentaire, \textit{un nombre pur}, est donc notée \textbf{$q$}.


\textit{An additional hypothesis of this article is that the adimension constants of the physics are privileged basis of calculation. It is the reason why the constant $a$ above is the inverse of “fine structure constant” $\alpha$, because a calculation basis must be greatest than unity.
This parameter $a$ is connected to the electric charge but the usual notation $e$ can be confused with the optimal basis of the natural logarithms, very important for a computeing cosmos. Then, the primary electric charge, a pure number, is denoted $q$.
...
Here, the four coupling constants (electric, gravitational, weak, strong) $a, a_G, a_w, a_s$, are the inverses of the standard quantities. We add the constant $P = m_P/m_e$, the ratio of the Planck and electron masses, see the bottom of table \ref{tab:2:table2} which resumes the main parameters. Some of these constants are measured to within $10^{-9}$ but other constants are not so well definite: $10^{-3}$ for the scalar boson $H^{(0)}$, and $10^{-4}$ (boson W) and $10^{-5}$ (boson Z). The fine-tuning is interpreted as the hypothesis that the adimensional parameters are \textit{strongly linked} calculation basis. So, thanks to a set of very precise auto-correlations, it was possible to assign to these constants the optimal values listed table 2, within to a greater precision: $10^{-7}$ \cite{Sanchez4}. Table \ref{tab:3:table3} lists the physical dimensional quantities, determined experimentally \cite{Zyla}.

...
\keywords{Quantum Physics \and Quantum Gravity \and String Theory \and Number theory \and Cosmology \and Holographic Principle \and Holic Principle \and Multi-dimensional Crystallography}
...
montrant que \textit{le Cosmos utilise à la fois, et de manière symétrique, $\pi$ et la base néperienne $e$}. Noter que les séries de Riemann d'ordre paires utilisent effectivement des puissances de $\pi$, lequel se comporte donc bien comme une base de calcul. D'ailleurs, la théorie de Wyler \cite{Wyler} aboutit à une formule approchant $p$ à 18 ppm près, repérée initialement par Lenz, dans un article célèbre d'une seule ligne \cite{Lenz}:

\begin{equation}
p_W = 6\pi^5
\end{equation}
 
Noter que cette expression est le produit aire-volume d'un cube de coté $\pi$. De plus, la masse gravitationnelle du proton $p_G = P/\sqrt {N_L}\approx 1831.531182$, où $N_L$ est le grand nombre de Lucas, le nombre premier le plus célèbre des l'histoire des mathématiques et le terme terminal de la Hiérarchie Combinatoire \cite{Bastin}, écrite sous la forme ci-dessus de Lenz-Wyler, présente un terme $\pi$ ayant un développement singulier: 

\begin{equation}
p_G = 6\pi_G^5~~~~ \rightarrow  ~~~~ \pi_G : 3, 7, 7, 7\sqrt 7
\end{equation}

Cette formule, précise à 5 ppb réhabilite la théorie, trop vite écartée, de Wyler, qui, précisément, implique un espace de dimension 7, caractéristique de l'algèbre des octonions. \textit{Sir Atiyah avait effectivement prédit que la gravitation devait être liée à cet algèbre}\cite {Atiyah}.
 
A noter que ce nombre $p_G$ se trouve aussi dans le développement suivant de la base naturelle des logarithmes, basé sur l'approximation $e \approx 19/7$, qui connecte avec l'approximation d'Archimède: $\pi \approx 22/7$. $e : 2, 7/5, -128$. Or $128/3 \approx sqrt p$. On observe que le développement:
...
ce qui confirme la température cosmique dans le domaine du ppb. 

\textit{A moins d'admettre que nous vivons un siècle très particulier, force est d'admettre l'invariance de la température cosmique. Ceci est contraire aux publications officielles, mais seules les observations conformes au paradigme cosmologique dominant sont publiées.}
...
De plus, l'écart ci-dessus $e-2$ apparaît dans, où 1836 est la partie entière de $p$, à 30 ppb:
...
Cette suite fractionnaire de $\pi$ est un problème non résolu des mathématiques actuelles. Cependant, on note que la partie entière de $\pi_5$, le nombre $292$, est la moyenne des nombres de symétries cristallines paires (419) et impaires (265) dans l'espace $10D$ des supercordes \cite{Weigel} \cite{Veysseyre} (Table \ref{tab:4:table4}). 
...
Avec la valeur de $G$ optimisée par une précédente étude de corrélation précise au ppb \cite{Sanchez2}, on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (table 1):
...
où $\lambdabar_e = \hbar/m_ec$ est la longueur d'onde réduite de l'électron. Les multiples relations holographiques, impliquant en particulier le rayonnement de fond, qui doit donc aussi être invariant, conduisent à:
...
L'Axe Topologique réhabilite la théorie \textit{bosonique} des cordes qui a le défaut apparent de faire intervenir des tachyons. En fait, c'est plutôt un avantage pour expliquer le caractère non-Doppler de l'oscillation \cite{Kotov2} de période $t_K \approx 9600.06(2)$ s, qui apparaît à la fois dans le rayonnement solaire et de plusieurs quasars. 
...
Considérer l'espace-temps comme une propriété \textit{secondaire} signifie que \textit{les concepts de masse, longueur et temps sont en final réductibles à des nombres purs}. Cela justifie la relation purement numérique observée entre le Cosmos et l'univers visible (figure 2). 
 ...
 \left\{
    \begin{array}{ll}
        m_G = m_e~~ m_{\hbar}^2 = m_pm_H ~~~~~~~~~~~~~~~~~~ \rightarrow r_1 = R/2 \\
         ...
         m_G = m_{bc} = m_{\hbar}   ~~~~~~~~~~~~~~~~~~~~~~~~~~ \rightarrow r_1 = 2l_K \\
...
\textit{La cosmologie permanente est tachyonique, liée au couple $G,\hbar$, tandis la thèorie quantique corresond au couple $\hbar,c$, et la gravitation au couple $G,c$, voir le "trièdre des constantes"} (fig).
...
Un autre phénomène \textit{totalement incompréhensible} par le modèle standard est la périodicité $v_T = 75 km/s$ des décalages Doppler \cite{Tifft}. Or cette vitesse montre une double singularité :

\begin{equation}
c/v_T \approx \sqrt a_w/a \approx 4181
 \end{equation}

où $4181$ est le 19ième terme de la suite de Fibonacci, qui se singularise par le fait qu'il est divisible ($4181 = 37 \times 113$), alors que tous les termes précédents d'ordre premier le sont aussi. Cette singularité n'est pas unique: elle implique que le nombre de premiers dans la suite de Fibonnaci pourrait ne pas être infini: c'est un problème non résolu.
...
De plus, le nombre co-parfait 495 est la partie impaire de l'ordre du groupe de Mathieu: $7920 = 16 \times 495$ et le triple de 165, le nombre de symétries cristallines impaires de dimension 10 (table 3).
...
\caption[Table \ref{tab:1:table1}: Adimensional primary constants.]{Adimensional primary constants}
\label{tab:1:table1}
  \hskip-2.0cm\begin{tabular}{llll1}
    \toprule
    %\multicolumn{4}{c}{}                   \\
    \cmidrule(r){1-4}
    ...
    $m_N $ & $m_N^2$   & $R_N/2$ demi-rayon de l'hologramme du Cosmos & $R_N/\lambdabar_e \approx (3^3)^{3^3}$& masse associée $m_P^4/m_G m_{\hbar}^2$ \\
    $m_{bc} $ & $m_{bc}^2$   & $2l_{cc}$ double longueur non-Doppler & $ l_{cc}/\lambdabar_e \approx \pi^{50}$& masse associée $m_P^4/m_G m_{\hbar}^2$ \\
    $m_e $ & $m_pm_H$   & $R/2$ demi-rayon de l'Univers & $R/\lambdabar_e \approx g(6) \approx 2^{128}$& masse associée $m_P^4/m_G m_{\hbar}^2$ \\
    

    $m_Pa^3 $ & $m_pm_H$   & $\lambda_{Wn}$ longueur d'onde de Wien du rayonnement de fond & $\lambda_{Wn}/l_P \approx \pi^{64}$& masse associée $m_P^4/m_G m_{\hbar}^2$ \\
    
    $m_{bc}R/R_N $ & $m_{ph}m_{gr}$   & $R_C$ rayon du Cosmos & $ R_C/\lambdabar_e \approx 3^{128}$& masse associée $m_P^4/m_G m_{\hbar}^2$ \\
...
\draw[axis] (0, 0, 0) -- (0, 3, 0) node [above] {$\hbar$};
\draw[axis] (0, 0, 0) -- (0, 0, 3) node [above] {$c$};
...
%}}
\node at (0.,0.5,-0.5) (l){$\mathbf Cosmology$};
\node at (2.,0.0,1.5) (l){$\mathbf Gravitation$};
\node at (1.,1.5,1.5) (l){$\mathbf Quantum$};
%\tkzLabelSegment[sloped,below,text=blue](A,C){$Gravitation$}
\end{tikzpicture}
\caption[The Triedre]{Triedre of Cosmology / Quantum / Gravitation connections}
\end{figure}

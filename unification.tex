...
% $ R/\lambdabar_e  \approx 6H (2\pi_{hol} a^3)^5 $& 13.81197676    &  Holographique, avec     $\pi_{hol} = 3 + 1/(7+1/(16+2/e^{2e}))$\\
 ...
 $(H/p_W)(2\pi^2 a^3)^5 \lambdabar_{e} $ & 13.811956 & with $p_W = 6\pi^5$, 5D holography in the gravitational Hydrogen molecule \cite{Sanchez3}  \\
  ...
  $ \lambdabar_{e} (N_{ph}  (n_t/p_t)^2/\pi \sqrt{g_0})^{1/7} $ & 13.81197 & with $N_{ph}$ the Cosmos photon number, confirms that the Universe is a cosmic gauge boson, acting by its power 7  \\
...
We introduce the "Weinberg-Sanchez" natural geometric extension of the Weinberg triangle \cite{Taylor}, $1/g_0 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2$. With the BE-Higgs scalar boson mass ratio, by respect to electron:  $H^{(0)} = 495^2$ (125.208 GeV)\cite{Sanchez3}, the radius ratio $R_e/R$ obeys the following $10^{-7}$ precise relations, involving $G$ through $p_G = P/2^{127/2}$ (from the Combinatorial Hierarchy \cite{Bastin}) and the Wien factor $w_i$. So $1/g_0$ is identified as $g_2/g_1g_3$ where $g_3$ is the inverse SU(3) coupling, while $g_1 = W/H^{(0)}$ is the U(1) coupling and $g_2 = Z sin~\theta/H^{(0)}$ is the SU(2) coupling, with $cos\theta = W/Z$: 
...
In this manner, in conformity with the standard model, $g_3$ is connected with the strong coupling constant, but through the Wien factor which is not pertinent in the present standard model, but must be central in cosmology, due to the thermal character of the Cosmic Microwave Background (CMB). 

%1/g_0 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2 \approx (R_e/2R) (p_G/H) = g_2/g_1g_3 \approx 2(H/p_t)(\beta sqrt a_s/w_i)^{1/2}
 %\end{equation}


Note that the small difference between $g_0$ and $g_2$ induces one formula in the Hubble Table. The radius $R_e$ is about 30\% larger than $R$, so \textit {$R_e$ was identifed to the holographic reduced Cosmos radius \cite{Sanchez3}, defined by the Bekenstein-Hawking entropy of the sphere with radius $R_e$} \cite{Bekenstein}: $\pi (R_e/l_P)^2 = 2\pi R_C/l_P$, so:   

 \begin{equation}\label{Eq6}
R_c = R_e^2/2l_P \approx 2^{128} l_P (Rg_0/ \lambdabar_H)^2 \approx 9.075773 \times 10^{86} m 
 \end{equation}
 
 \textit{The Table \ref{CosmosTable} presents 24 formula confirming this Cosmos radius.}
...
\begin{table}
\caption[Table \ref{CosmosTable}: 27Cosmos radius formula]{27 formula for the Cosmos radius $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ R e^{210} (f(16) e^{e^6})^{-1/6}  $ & 9.07577 & confirms the role of the central topologic term $f(16) = e^{16}$   \\
...
Note that, while $137 = l_{16}$, the $16^{th}$ Lazy Caterer number (maximal number of zones in a plane defined by n straight lines), $11 = l_4$ and $4 = l_2$. The corresponding Pythagorean triangle has the sides 8,105,137, i.e. the number of partitions with elements greater than 1 of 18, 19 and 20. Its perimeter is $K_9$ and the area 10 $s_0$:

...
R_N = 2r_e^3/l_P^2 = 2 \hbar^2/Gm_N^3 ~~~~~~~~~~~~ M_N = m_P^4/m_N^3
 ...
 Ce rayon $R_N$ est légèrement supérieur (30\%) à l'horizon $R$ de l'Univers, et est considéré comme le "rayon holographique réduit" du Cosmos de rayon $R_C$, comme rappelé ci-dessous. 
...
Les masses des 4 nucléotides de la molécule d'ADN, sont, rapportées à la mase de l'hydrogène \cite{Huang}:

\begin{equation}
 \left\{
    \begin{array}{ll}
        Cytosine : ~masse~brute ~~^{6}_{12}C_{9}~^{1}_{1}H_{12}~^{7}_{14}N_3~^{8}_{16}O_6~^{15}_{31}P : ~ C_y^{(0)} \approx 286.8021362\\
        Thymine : ~masse~brute~~ ^{6}_{12}C_{10}~^{1}_{1}H_{13}~^{7}_{14}N_2~^{8}_{16}O_7~^{15}_{31}P :  T_h^{(0)} \approx301.68553403\\
        Adenine : ~masse~brute~~ ^{6}_{12}C_{10}~^{1}_{1}H_{12}~^{7}_{14}N_5~^{8}_{16}O_5~^{15}_{31}P:~  A_d^{(0)} \approx310.6269397\\
        Guanine : ~masse~brute~~ ^{6}_{12}C_{10}~^{1}_{1}H_{12}~^{7}_{14}N_5~^{8}_{16}O_6~^{15}_{31}P :   G_u^{(0)} \approx326.4976654 
    \end{array}
\right.
\end{equation}

Ces masses sont voisines de la masse de Fermi $m_F/m_H \approx 311.8997032 $. Avec $o_1$ et $o_2$ les masses brutes des couples AT et GC, la masse brute moyenne est $ (o_1 + o_2)/4 \approx 306.4032199$, très voisin de $\pi^5 \approx 306.019684$. 


La masse moyenne du bi-codon, composé de six nucléotides $m_{bc}/m_H = (6/4)(o_1 + o_2) \approx 1838.418122$  est donc voisine de $p_W = 6 \pi^5$, le rapport de mass proton-electron de Lenz-Wyler. A 0.1 ppm près:
...
La masse $b$ de bicodon, relativement à l'électron, donnée par l'Eq. (12), présente un écart $ \approx 5 \times 10^{-4}$, qui est \textit{l'incertitude actuelle des masses isotopiques effective}{} \cite{Wichers}. Plus précisément, à 10 ppm près

...
The Keplerian Diophantine equation shows a symmetry between gravitation and quantum physics, connecting the Bohr’s orbitals with the Permanent Holographic Oscillatory Bang model. The mean DNA bi-codon mass is central and connects to 0.1 ppm with the Topological Axis. This induces a scalar boson mass confirmed both by the geometric extension of the Weinberg triangle and by the 9D crystallographic symmetry, associated to the string dimension reduction. This is tied to binomial Euler's suitable numbers implying the Riemann hypothesis. The running number of the string dimension series is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This confirms Eddington, Atiyah and the Combinatorial Hierarchy, leading to the gauge partition. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, and leads to stringent predictions. 
...
Here $\mu$ and $\tau$ are the leptons relative masses, $\cos \theta = W/Z$, and $d_e$ is the electron magnetic excess. 

 \textit{The two fist terms are equivalent for $d\approx 32$, which specifies the Topological Axis symmetry, from $k = 0$ to $k = 7$, and the characteristics of the string dimension SO(32), whose dimension is the perfect number 496:}
 ...
 \left\{
    \begin{array}{ll} 
            d_k + d_{7-k} = 32  \\ 
            \\
       d_{SO(32)}  \binom{32}{2} = 496  
    \end{array}
\right.
\end{equation}

From the above double relation for B, the following property of the scalar boson emerges, showing a special recurrence relation between the dimensions 10 and 9:

\begin{equation}
 \left\{
    \begin{array}{ll} 
            H^{(0)} = 495^2 = K_{9}\textbf{N}_9 = K_{10} N_{10} + N_{9} - 1 \\ 
            \\
       495 =  \binom{12}{4} = 496 - 1  
    \end{array}
\right.
\end{equation}

where  $\textbf{N}_9 = 9K_9$ is the total number of positive zeros on the unit circle for the central string reduction dimension 9, and $N_{9}-1$ the number of non-trivial $9D$ symmetries. 
...
The equivalent relation for dimension 4 shows up a relation between the brut proton-electron mass ratio 1836 with the Euler maximal suitable number 1848: 

\begin{equation}
 \left\{
    \begin{array}{ll} 
            K_4 \textbf{N}_4 = 1836 - 12 \\          
       ~~~~s_0  = 1836 + 12 = 2 \binom{12}{6} = (N_4 + K_4)^2 - 1 = 1848\\
       1836 = S_{12}
    \end{array}
\right.
\end{equation}
...
%\label{tab:4:table4}
  \hskip-0.0cm\begin{tabular}{llllllllllllll}
    ...
    $K_d$ (positive $PSO_{Cr}$)  & 1 & 1 & 5 & 5 & 19 & 19 & 59 & 59 & 165 & \underline{165} & 419 & 419 & 1001 \\
...
The overwhelming connections between the HVW crystallographic series, the string theory, and the sporadic groups confirms that \textit{the pure mathematics must now pursue unification, by concentrating on the mathematical properties of physical parameters, looking for the connection with the Riemann hypothesis through the maximal Euler suitable number}.

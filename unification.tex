...
\label{tab:6:table6}
 ...
 $r_H 2^{110} (a\beta/\pi_A)^2$  & 13.81197    & Extrapolation d'ordre 16 de la tabe périodique, avec $\pi_A$ = 22/7 \\
...
où est le produit des ordres des 6 groupes sporadiques dits "parias" \cite{Sanchez2}, et $4181$ est le 19ième terme de la suite de Fibonacci, qui se singularise par le fait qu'il est divisible ($4181 = 37 \times 113$), alors que tous les termes précédents d'ordre premier sont aussi premiers. 
...
\begin{equation}
\lambda_{Wn}/l_P \approx \pi_0^{64}~~~~ \rightarrow  ~~~~ \pi_0 : 3, 7, 16, 1, (W_+ + W_-)/Z
\end{equation}
...
Dans l'approche des bitstrings de Noyes \cite{Noyes}, l'approximation $p \approx 1836$ est centrale. La pertinence des entiers 1836 et 1839 est congirmée par l'ordinateur, qui indique, \textit {à 70 ppb près}:
...
Considérant les sommes $S^{(26)}$ et $S_3{(26)}$, leur rapport est très voisin de $3/\sqrt2\approx 1/2sin~\theta \approx 2H/\tau$, et plus précisément, à 3 ppm près:

\begin{equation}
S^{(26)}/S_3^{(26)} \approx (a/137)^2 (1+\mu +\tau)/\tau 
 \end{equation}

où $q$ est la charge électrique, $\theta$ l'angle de couplage, $mu$ et $\tau$ les rapports leptoniques, avec pour unité la masse de l'électron. Ces nombres sont rappelés dans le chapitre suivant qui détaille comment le terme $(1+\mu +\tau)$ joue un rôle central, quoique \textit{non reconnu par le modèle standard des particules}.
...
L'importance arithmétique du nombre 495 sera détaillée dans le chapitre suivant. En identifiant $H^0 = 495^2$, cela définit la charge de jauge $q' = q/sin\theta$, qui est en relation approchée simple avec $S^{(26)}$:

\begin{equation}
495^2 = H^0 = W/q' \approx q'S^{(26)}   
 \end{equation}
 
 De plus, on constate la liaison suivante de $S{(26)}$ avec le couplage fort et le rapport canonique $aq^2/4\pi$, à 6.2 ppm près:
...
Noter que des travaux récents ont confirmé la pertinence de la corrélation dite "Monstruous Moonshine" faisant intevenir la dimension $D = 196883$ du groupe monstre \cite{Conway}. On constate, à 22 ppm près:
...
Le modèle standard a été complété et conforté par la prévision et la découverte d'un boson scalaire massif $H^0$. Cet article prétend préciser ces valeurs cruciales à partir de l'hypothèse que le rapport de masse principal est:
...
\textit{La charge élecrique relie donc la fonction topologique à la physique des particules.}

...
\begin{table}
\caption[Table \ref{CosmosTable}: 20 Cosmos radius formula]{20 formula for $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ Rl_{nl}^2c^2a_w/R_NGm_{bc} $ & 9.07529 & Principe Holique, Eq.(15), avec  $m_{bc} \approx 1838.419 ~m_H$ \cite{Sanchez3}   \\
...
%$  \lambdabar_e ~ cotg\theta ~ 495^{\pi \sqrt a}  $ & 9.03 & Base 495, avec exposant $ \pi \sqrt a\approx \pi^{\pi}$  \\ 
 ...
 $\lambdabar_e  ((\ln(M_N/m_e)^2 + (ln(R/\lambdabar_e)^2 + ((\ln(t/t_e))^2)^{1/2} $ & 9.12 & couple Cosmos/c-Universe $(R/\lambdabar_e = t/t_e)$ , fig    \\

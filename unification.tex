...
%The Holic Principle is the simplified form of the Holographic Principle and based on the simplest degenate Diophantine equations. It induces an “Hyper-symmetry” between the constants $G$ and $\hbar$ which leads to a new expression of Bohr’s orbits. With the cosmic most precise measurement ($10^{-6}$), the quasar non-local period, this shows a connection between the Hubble radius, the Cosmos radius and their common CMB temperature. This confirms the Permanent Holographic Oscilatory Bang model, the c-observable Universe appearing as a cosmic gauge boson. The mean DNA bi-codon mass is central, connecting to $10^{-7}$ with the mean dimension 16 of the Topological Axis. The later defines a scalar boson mass of $495^2 m_e$, connected to the natural geometric extension of the Weinberg triangle, confirming the $G$ value to $10^{-7}$ precision, at $+ 7.7~ \sigma$ of the official value. The association of computation with symmetry, whose common point is invariance, leads to consider multi-dimensional crystallographic symmetries. This is characterised by a function which shows the origin of the term 495^2, tied to the 9D string dimension reduction. The string principal dimension series $d = 2 + 4k$ are identified with the dimensions of the Topological Axis for the Bott "octonion" series 0 to 7. For the partial series, from 0 to 3, $k$ is identified with the classical orbital quantum numbers of the Periodic Table. This leads to confirm both the triple algebra Atiyah series $137 = 2^7 + 2^3 + 2^0$ and the Combinational Hierarchy $137 = 3 + 7 + 127$, leading to the brute gauge separation 137 = 30 + 107. So, the electron spin 1/2 is associated with the string dimension 2. The underlying Computation Principle excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, to be replaced by the search towards a Science-Arithmetic Unification. Stringent predictions are deduced, in particular for the very large telescopes in preparation.  


The Holic Principle, the simplified form of the Computation and Holographic Principles is based on the simplest degenate Diophantine equations. It induces an “Hyper-symmetry” between the universal constants $G$ and $\hbar$ which leads to a new expression of Bohr’s orbits. With the cosmic most precise measurement ($10^{-6}$), the quasar non-local period, the Holic Principle shows a connection between the Hubble radius, the Cosmos radius and their common CMB temperature. This confirms the Permanent Holographic Oscilatory Bang model, the c-observable Universe appearing as a cosmic gauge boson in the Topological Axis. The mean DNA bi-codon mass is central in the cosmic-holic formulation, connecting to $10^{-7}$ with the topologic function f(16), where 16 is the mean Topological Axis dimension. 


%The later defines a scalar bE-Higgs boson mass of $495^2 m_e$, connected to the natural geometric extension of the Weinberg triangle, confirming the $G$ value to $10^{-7}$ precision, at $ + 7.7~ \sigma $ of the official value. The association of Computation with Symmetry, whose common point is invariance, leads to consider multi-dimensional crystallographic symmetries. This is characterised by a function which shows the origin of the term 495^2, tied to the 9D string dimension reduction. The string principal dimension series $d = 2 + 4k$ are identified with the dimensions of the Topological Axis for the Bott "octonion" series 0 to 7. For the partial series, from 0 to 3, $k$ is identified with the classical orbital quantum numbers of the Periodic Table. This leads to confirm both the triple algebra Atiyah series $137 = 2^7 + 2^3 + 2^0$ and the Combinational Hierarchy $137 = 3 + 7 + 127$, leading to the brute gauge separation 137 = 30 + 107. So, the electron spin 1/2 is associated with the string dimension 2. The underlying Computation Principle excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, to be replaced by the search towards a Science-Arithmetic Unification. Stringent predictions are deduced, in particular for the very large telescopes in preparation.
...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 43 Hubble radius formulas]{43 formula for Hubble radius $R$}
\label{tab:6:table6}
...
% $ \lambdabar_{e} a'^{26} /(1+1/2\pi) $ & 13.83 & Proximité de  $ a' \approx d_7 avec d_6 = 26 $ \\
 ...
 $ \lambdabar_{e} exp(2a_w/3\times 7 \times(137a)^2) $ & 13.85 & Confirme that the Universe is a cosmic gauge boson, acting by its power 7 \\ 
...
The classical theory associates conservation law with symmetry. More generally, a conservation law can be seen as the result of a calculation. So, the main hypothesis is that the Cosmos, considered as a black-hole of radius  $R_C$ is a super-computer \cite{Sanchez2}. This is a very discussed domain. In particular, Y. Ng \cite{Ng} introduces wrongly a "quantum foam" of dimension $(Rl_P^2)^{1/3} \approx 10^{-15} $ m, a length independantly found through the Computation Principle\cite{Sanhez2}. With the above hypothesis of an \textit{invariant} Hubble radius $R$, this can be rather associated with the \textit{invariant} electron classical radius. So, the "classical radius" of the Universe was defined as \textit{the radius which is the simplest elimination of $c$ between the classical electron radius and the Planck length formula. Indeed the speed $c$ is dramaticaly too small a speed to inter-connect a so vast Cosmos}: 
...
This radius $R_e$ connects with $R$ by the folowing $10^{-7}$ precise relation, implying the "Weinbergh-Sanchez" natural geometric extension of the Weinberg triangle \cite{Taylor}, $\eta = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2$, with the SU1 coupling $g_1 = W/H^{(0)}$ and the SU2 coupling $g_2 = Z sin~\theta/H^{(0)}$, with $cos\theta = W/Z$ and the BE-Higgs scalar boson $H^{(0)} = 495^2m_e$ (125.208 GeV)\cite{Sanchez3} and $p_G = P/2^{127/2}$ (from the Combinatorial Hierarchy \cite{Bastin}), so to $10^{-7}$:
...
\begin{equation}
 \left\{
    ...
    q = g_1 cos \theta = g_2 sin \theta = 4\pi/a \\
...
This radius $R_e$ is about 30\% larger than $R$, so it was considered as the holographic reduced Cosmos radius \cite{Sanchez3}, defined by Bekenstein-Hawking entropy of the sphere with radius $R_e$ \cite{Bekenstein}: $\pi (R_e/l_P)^2 = 2\pi R_C/l_P$, so:   
...
The Topological Axis rehabilitates the bosonic part of the string theory which has the apparent imperfection it includes tachyons. In fact, it is rather an advantage in order to explain the quasar non-Doppler oscillation, introducing a non-local period $t_{nl} \approx 9600,06 (2) $ s \cite{Kotov}.
...
This gives a $G$ value precise to $10^{-6}$, compatible with the BIPM $10^{-5}$ precise measurement \cite {Quinn}. This implies that the the official value of $G$, \textit{the incongruous mean between incompatible measurements}, is too weak by 7.7 $\sigma$.
...
Moreover, such arithmetic-physical synthesis has been anticipated by the Holic principle [34], a simplified form of the holographic principle. In any Diophantine equation, this principle allows to discriminate a temporal ratio $T$, acting by its square, from a spatial ratios $L$, acting by its cube (due to 3D space). Indeed, {\textit the simplest Diophantine Equation, which implies a 2-dimensional Time}, $T^2 = L^3 = n^6$ is the Diophantine form of the third law of Kepler, it implies: $L_n = r_n /r_1 = n^2$ and $T_n = t_n/t_1 = n^3$. Hence, with $v_n = r_n/t_ n$:
...
These masses are close to the Fermi mass ratio. The mean masses of the effective couples are close to $H/3$: 
...
Note that the Wyler formula $6\pi^5$ shows a geometric property: it is the product of the area by the volume of a cube of side $\pi$
    
There is a $10^{-7}$ precision relation with $f(16)$, the mean term of the Topological Axis:

\begin{equation}
f(16) = e^{16} \approx 2(d_em_{cd}^{(0)}/m_e)^2 /a^3  
 \end{equation}
...
Carl Hermann \cite {Hermann} calculated the number of crystallographic point symmetries $N_d$ for dimensions from 1 to 8. This number $N_d$ is the number of monic polynomials (i.e. first term $1x^d$) with roots on the unit cercle.
...
\end{equation}
 ...
 \textit{The two fist terms are equivalent for $d\approx 32$, which specifies the Topological Axis symmetry, from $k = 0$ to $k = 7$:}
...
where  $\textbf{N}_9 = 9K_9$ is the total number of positive zeros on the unit circle for the central string reduction dimension 9, and $N_{9}-2$ the number of non-trivial symmetries. The equivalent relation for dimension 4 is particular: $K_{4}\textbf{N}_4/2 = K_5^2$, and one finds: 
...
\begin{equation}
 ...
 Note that 495 is the odd part of the first Mathieu group, and the couple 495- 496 is the third perfect couple. In such a couple, the first number is the sum of the \textit{non-trivial} divisors of the second. Since $496 = \binom{32}{2}$ is the dimension number of the group SO(32), and $495 = \binom{12}{4}$ \footnote{Number of groups of 4 elements taken in a 12 elements grouping}, this leads to the conjecture : \textit{the perfect couple 495-496 could be the single one implying non-trivial binomial numbers}. This calls for further study.
...
Strangely enough, mathematicians seems to have not looked for the prime numbers that appear in the harmonic series, which is the single pole of the Rieman series, precisely known to inform about the distribution of prime numbers. The six first prime numbers appearing are showing a symmetry of 11 around 137:

\begin{equation}
3; 11; 5; 137; 7; 11   ~~~~~~~~ \Rightarrow 137 = 11^2 + 4^2
 \end{equation}

showing the 11 supergravity dimensions and the usual 4. This number 137 has been unambigously connected with the Lucas-Lehmer series \cite{Sanchez3}. One observes the double apparition of this "arithmetic monster" 137:
...
1839 = \sum_{10}^{12} K_d = \sum_1^{12} (N_d/2) + 3   ~~~~ \Rightarrow~~~~ \sum_2^9K_d = (K_{12}-9)/2 = 496  
 ...
 This identifies the partial sum up to the 9D space with the above $496= \binom{32}{2}$, the dimension of the canonic group SO(32), central in string theory. They could be identified with the above 9 dimension reduction. 
 %% replaced C_{32}^2 by \binom{32}{2}
 .
 \subsection{The \textit{precise} gauge partition}
 Taking account of the dimension zero, the above sum becomes $S_{12} = 1836.5$, close to the mean proton-Hydrogen mean, and the gauge separation could imply rather $n_7+1/2 = 30.5$, which is close to 196 ppm with the real gauge partition term $a (\sin~\theta)^2\approx 30.505983$, and more precisely:
...
The string dimensions special series $d= 2+4k$ identifies both with the Topological Axis one and with the spectroscopic one, \textit {so, the string dimension 2 identifies with the spin 2 degeneracy, where $k$ identifies with the orbital number, running in the octonion series, between 0 and 7.} 

Their sum is particular, illustrating the association symmetry-computation where the central dimension is 16: indeed 2 $\times$ 16 = 32 = 2 + 30 = 6 + 26 = 10 + 22 = 14 + 18:  

\begin{equation}
 \sum_{k=0}^{k=7} d_k = 2^7~~~~~~~~  \sum_{k=0}^{k=7} d_k + \sum_0^1{d_k + 1} = 137
 \end{equation}

 where the term 137-127 = 3 + 7 shows the first terms of the Combinatorial Hierarchy \cite{Bastin}. The height numbers are all of the form "prime - 1", except $di = 14$ and $26$, the later being the critical dimension which verifies: $ d_{26} = d_{d_6} = 106 $, so justifying the "reduced" Atiyah sum, with the octonion term ($2^7$) and the quaternion one ($2^3$). This identifies with the reduced "gauge separation", where 136 is the initial Eddington's electric coupling, the number of elements in the symmetrical matrix $16\times16$. 
...
The Periodic Table uses only the $d$ values inferior or equal to the above term 14, corresponding to the famous spectroscopic numbers, called by Friedrich Hund "sharp" (s = 2), "principal" (p = 6), "diffuse" (di = 10) and "fundamental" (f = 14). The $7^{th}$ row of the Periodic Table terminate in the Oganesson, recently synthetised, of atomic number 118, \textit{which is precisely the Herman number for d = 7}. The involved coefficients, ruling the particular natural distribution of the spectroscopic groups s,p,di,f are the following:
...
which recovers the complete Atiyah sum, including the "real algebra" term $2^0$, and, since the last "fundamental" term is $2\times 15 = 30$, coming back to the above brute gauge separation.
...
The Cosmology must be completely re-interpreted, with the unifying concept of "Holographic Permanent Matter-Antimatter Bang Oscillation". The future giant telescopes must observe an invariant background (CMB) temperature, as well as an invariant value 3/10 for the baryon+dark matter density, the later being an anti-phase oscillation of normal baryons.

...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 45 Hubble radius formulas]{45 formula for Hubble radius $R$}
\label{tab:6:table6}
...
% $ R/\lambdabar_e  \approx 6H (2\pi_{hol} a^3)^5 $& 13.81197676    &  Holographique, avec     $\pi_{hol} = 3 + 1/(7+1/(16+2/e^{2e}))$\\
 ...
 $ l_{P} \sqrt a (\mu^{\mu}~Wp/ZH)^{1/8}$  & 13.81198 & Shows the pertinence of the computational term $\mu^{\mu}$   \\
  ...
  $12 F_{137}^2 l_P(m_{bc}^{(0)}/m_e)^{3/2}/pH$ & 13.81194    & With the Fibonacci prime number $F_{137}$ and the bi-codon mass\\

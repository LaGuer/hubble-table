...
La masse moyenne du bi-codon, composé de six nucléotides $m_{bc}/m_H = (6/4)(o_1 + o_2) \approx 1838.418122$  est donc très voisine de $p_W = 6 \pi^5$, le rapport de masse proton-electron de Lenz-Wyler \cite{Wyler}. Plus précisément, avec $d_e$ l'excès magnétique de l'électron, à 0.15 et 0.2 ppm ppb:

\begin{equation}
 m_{bc}^{(0)}/m_H = (6/4)(C_y^{(0)} + T_h^{(0)} + A_d^{(0)} + G_u^{(0)}) \approx H\sqrt{n_t/p} \approx \sqrt {(f(16) a^3/2)}/Hd_e
 \end{equation}

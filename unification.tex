...
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{lllll}
    ...
    $m_e{red}$&$m_P/\sqrt a $& $r_H$ rayon de Bohr   & exact & $ r_H/\lmbdabar_e \approx 137 = 2^7+2^3+2^0 $\\
    ...
    $m_e $ & $\sqrt {m_pm_H}$   & $R/2$ demi-rayon de l'Univers &exact& $R/\lambdabar_e \approx g(6) \approx 2^{128}\approx (2R/R_N)^{210}$   \\
    
    $m_{bc}^{(0)}R/R_N $ & $\sqrt{ m_{ph}m_{gr}}$   & $R_C$ rayon du Cosmos & $4.7 \times 10^{-4} $& $ R_C/\lambdabar_e \approx e^{e^{2e}} \approx 6^{128}\approx (2R/R_N)^{64a_s}  $\\

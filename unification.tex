...
\textit{C'est la distribution des orbitales principales de Bohr}. Cette formule exhibe 6 autres cas particuliers:

\begin{equation}
 \left\{
    \begin{array}{ll}
    
    m_G = m_e;~~  m_{\hbar} = m_P/\sqrt a  ~~~~~~~~~~~~~~~~~ \rightarrow r_1 = R_H \\
...
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{lllll}
    ...
    %\multicolumn{4}{c}{}                   \\
    \cmidrule(r){1-4}
    $m_G$ & $m_{\hbar}^2$    & $r_1 = \hbar^2/Gm_G m_{\hbar}^2$  & Imprécision &Propriété arithmétique \\
    \midrule
    
    
    $m_N $ & $m_N^2$   & $R_N/2$ demi-rayon holographique du Cosmos  &  exact & $R_N/\lambdabar_e \approx (3^3)^{3^3}$ \\
    $m_{bc} $ & $m_{bc}^2$   & $2l_{cc}$ double longueur non-locale & $6.3 \times 10^{-4}$ & $ l_{cc}/\lambdabar_e \approx \pi^{50}$  \\
    ...
    $m_{bc}R/R_N $ & $m_{ph}m_{gr}$   & $R_C$ rayon du Cosmos & $5 \times 10^{-4} $& $ R_C/\lambdabar_e \approx e^{e^{2e}} \approx 6^{128}\approx (2R/R_N)^{64a_s}  $\\

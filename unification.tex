...
définit le $\pi$ mathématique à mieux que le ppb. La suite fractionnaire est un problème non résolu des mathématiques actuelles. Cependant on note que le nombre $292$ est la moyenne des nombres de symétries paires (419) et impaires (265) dans l'espace des supercordes $10D$ \cite{Weigel} (tableau 1).


Avec la valeur de $G$ optimisée par une étude de corrélation \cite{Sanchez} (tableau 2), on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (tableau 3):
...
De plus, la masse gravitationnelle du proton $p_G = P/\sqrt {N_L}\approx 1831.531182$, écrite sous la forme de Lenz-Wyler \cite{Wyler} $p_G = 6\pi_G^5$ présente un développement singulier: 

\begin{equation}
p_G = 6\pi_G^5~~~~ \rightarrow  ~~~~ \pi_G : 3, 7, 7, 7\sqrt 7
\end{equation}

précise à 5 ppb, caractéristique de l'algèbre des octonions. \textit{Sir Atiyah avait effectivement annoncé que la gravitation devait être liée à cet algèbre}\cite {Atiyah}:
...
Les théories des supercordes actuelles tendent à considérer l'espace-temps comme une propriété émergente. Cela signifie que les concepts de masse, longueur et temps sont en final réductibles à des nombres purs. Cete hypothèse justifie la relation observée entre le Cosmos et l'univers visible (figure 2). 

Cette synthèse arithmético-physique a été annoncée par le principe holique \cite{Sanchez1} qui, dans une équation diophantienne, permet de distinguer des termes temporels, agissant par leur carré, des spatiaux, agissant par leur cube. La masse est associée à l'exposant 5, et le champ au 7. La résolution diophantienne la plus simple est donc:
...
\begin{equation}
R/\lambdabar_e)^{1/210} \approx 2a^3/pH
 \end{equation}

où ce terme $2a^3/pH$ est central dans les relations holographiques.

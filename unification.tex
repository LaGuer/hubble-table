...
La table 4 présente 14 formules au ppb pour ce rayon de Hubble. Cette démarche rappelle celle de Jean Perrin qui, en présentant 14 formules impliquant des nombes de molécules, a finalement convaincu la communauté de l'existence réelle des atomes.
...
La complexité de la structure de l'espace-temps est trahie par le phènomène décrit dans la section suivante, tellement hors-norme qu'aucun physicien, à part Pecker \cite{Kotov}, n'y a prété attention.
...
où 419 est le nombre de symétries positives 10D \ref{tab:10:table10}, 417 étant le nombre non trivial.
...
Considérer l'espace-temps comme une propriété émergente signifie que \textit{les concepts de masse, longueur et temps sont en final réductibles à des nombres purs}. Cela justifie la relation purement numérique observée entre le Cosmos et l'univers visible (figure 2). 

Une telle synthèse arithmético-physique a été annoncée par le principe holique \cite{Sanchez1} qui, dans une équation diophantienne, permet de distinguer des rapports temporels, agissant par leur carré, des termes spatiaux, agissant par leur cube. L'éqation $T^2 = L^3 = n^6$ est la forme diophantienne de la troisième loi de Kepler, qui implique $L = n^2$, la loi des orbites atomiques quantifiées, impliquant la quantification du moment cinétique.
...
\sin \theta^2 - \sin \theta_{\tau}^2  \approx 1/\sqrt a_w \sin_{eff}{\theta} (m_Z) \approx 1 / P^{1/4}
 ...
 faisant apparaître le terme appelé "weak mixing angle" effectif (table 1).

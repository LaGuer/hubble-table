 ...
 %Rydbergh reduced wavelength $\lambdabar_e(2aH/p)^2$ & $\lambdabar_{Ryd}$  & m  & $1.45190673 \times 10^{-8} $ & 0.3   \\
  ...
  %Official CMB temperature & $T_{CMBoff}$ & K & $2.7255(6)$ & $2 \times 10^5$ \\    
   ...
   Cosmos holographic radius & $R_N$ & m & $1.712894163 \times 10^{26} $ & \cite{Sanchez2} \\
    Cosmos radius & $R_{GC} = R_C$ & m & $9.075773376 \times 10^{86} $ & \cite{Sanchez2} \\
...
\label{tab:6:table6}
 ...
 $ (l_P/2)(\lambdabar_{CMB}^2\lambdabar_p/\lambdabar_{CNB}r_e^2)^6$  & 13.7    & Complémentarité des fonds microondes-neutrinos \\
 
 
 
 
 
  $ \lambdabar_{CNB}^2 \lambdabar_{CMB}^4 (ln\Phi)^8/l_P^4 R_C H_0 $  & 13.812   & Confirme le boson scalaire $H^0$ et le Role de $ln\Phi \approx (\sin \theta_{eff})(m_Z) $ \\
...
In a 3D Super-space, logarithms of physical ratios are considered vectors. The Grandcosmos radius $R_{GC} = R_C$ appears as the norm of the vector using for length and time projections the same value $R/\lambdabar_e = t/t_e$. For the mass projection it is $M_N/m_e$ where
%that of the Hubble radius, and for mass ratio $M_N/m_e$.
$M_N = M'$ is the critical mass in the Cosmos reduced spherical hologram. This is a dramatic geometrical confirmation (not dependant of the base for logarithms) of the Extended (2D-1D) Holographic Principle applied to the Bekenstein-Hawking Universe entropy. The Cosmos existence and radius is specified to $10^{-7}$.} 
\end{figure}
...
Avec $u_0$ = 5, $d_0$ = 6, les masses représentatives des quarks up et down \cite{Sanchez1}, et $f(8) = e^4$, situé à mi-chemin du terme central $f(16)=e^{16}$  on observe:
...
Avec $f(1) = e^{\sqrt2}$ le terme topologique fondamental, on observe, à 7 et 30 pm près:

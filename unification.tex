...
Cette hypothèse a été présentée comme une exigence dans une conférence à Cambridge en 1994, introduisant le Principe Holographique et sa forme diophantienne, le Principe Holique. En effet, le Principe Holographique est la connexion entre variétés topologiques de dimensions différentes (périmètre, aire, volume…), donc utilise la constante $\pi$. 
...
Le nombre de « paramètres libres » dans le modèle standard est d’une trentaine. Heureusement, certains, appelés « paramètres-clefs »,  semblent plus importants et sont mesurés dans le domaine du ppb ($10^{-9}$ ). Cet article prétend montrer que, loin d’être "libres", ces paramètres sont des bases optimales de calcul, inconnues des mathématiques actuelles.

Les paramètres-clefs sont définis comme étant ceux mesurés dans le domaine du ppb, ou s’ils corrèlent de manière significative avec des paramètres mesurés, comme c’est le cas pour les masses intermédiaires $W$ et $Z$. En effet ces masses jouent un rôle central dans le modèle électro-faible, comme rappelé ci-dessous.
...
\tkzLabelSegment[sloped,right,text=red](A,B){$m_Z/m_{BEH}$}
\draw[blue, dotted]
      let \p1 =  ($(B)-(center)$),
          \n0 = {veclen(\x1,\y1)}
      in (center) circle(\n0);
...
\node [anchor = east,text=red] at (2.5,-0.5) {$q'$};
...
\tkzDefPoint(4.,0.0){A} \tkzDefPoint(0.,0.0){B} %% \tkzDefPoint(5.,0.){C} \tkzDefPoint(3.3,2.5){D}
\tkzDefMidPoint(A,B) \tkzGetPoint{M}
\tkzInterLC[R](A,B)(A,1cm) \tkzGetSecondPoint{D}
\tkzDefLine[orthogonal=through D](A,D)
\tkzInterLC(D,tkPointResult)(M,A) \tkzGetSecondPoint{C}
\tkzDrawPolygon(A,B,C)
\tkzDrawSegment(C,D)
\tkzDrawPoints(A,B,C,D,M)
\tkzLabelPoints[above](A,B,C,D,M)
\coordinate (center) at (M);

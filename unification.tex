...
La physique mathématique fait face à une crise grave. Le modèle standard des particules est incapable d’inclure la gravitation, se heurtant à des problèmes de renormalisation, c’est-à-dire l’élimination de quantités infinies qui apparaissent dans les calculs. Nous proposons ici l’hypothèse d’une rationalisation symbolique de $\pi$, revenant à supprimer les décimales infinies de $\pi$ au-dela d’un cetrain nombre. 

Cette hypothèse a été présentée comme une exigence dans une conférence à Cambridge en 1994 \cite{Sanchez1}, introduisant le Principe Holographique. En effet, par opposition aux équations différentielles, les équations globales les plus simples sont les connexions entre variétés topologiques de dimensions différentes (périmètre, aire, volume…), donc utilisent la constante $\pi$.  

Dans l’hypothèse plus générale d’un Cosmos calculateur, de même qu’un ordinateur ne peut utiliser le $\pi$ mathématique "rigoureux", tout calcul doit être associé à une imprécision. Dans cet article, la précision maximale $10^{-9}$ (ou ppb) est recherchée, laissant peu de prise au hasard. Ainsi $\pi$ \textit {doit} être rationalisé, et, dans un premier temps, de manière symbolique, c’est-à-dire que son développement fractionnaire doit montrer des paramètres-clefs.

\subsection{La question de l'espace-temps 4D}
La théorie des cordes a suscité un espoir énorme quand elle a exhibé une particule de spin 2, qu'on a identifié au graviton, intégrant ainsi la gravitaton. De plus, elle a mis en évidence une correspondance holographique explicite entre des espaces-temps 5D et 4D.  Mais cete théorie bute sur le concept de d'univers en expansion. Une étude précédente a montré que les multiples corrélations observées entre les grands nombres de la physique mathématique pouvait être considérées comme une succession de relations holographiques 1D-2D, aboutissant en final a l'Axe Topologique, confirmant la théorie bosonique des cordes, qui favorise la dimension 26D. Cet Axe Topologique (Fig 1) implique que \textit{le concept d'Univers en expansion doit être éliminé}. La courbure de l'espace-temps n'est alors qu'une propriété locale, tandis que l'Univers dans son ensemble présente une géomérie plate, euclidienne. C'est effectivement ce que les astropphysiciens consatent, et désignent sous le nom de 'condition critique'.

Pour justifier ce caractère critique, les cosmologistes ont introduit une étape originelle d'inflation. Mais celle-ci devient inutile si l'on abandonne le concept d'expansion. En efet, avec un rayon de Hubble invariant, on peut appliquer sur la sphère correspondante le principe holographique de trou noir de Bekeinstein-Hawking \cite{Sanchez2}, justifiant ainsi directement le caratère critique, mais avec pour conséquence que le mur de Planck est reculé d'un facteur $10^61$, en contradiction avec tous les modèles théoriques actuels. Cependant, cette "Cosmologie Permanente"  conduit enfin à une explication de l'énormité de l'énergie quantique du vide par rapport à celle de l'univers ($10^121$).

L'énormité du Cosmos, quant à elle, a été justifiée par une exigence de quasi-continuité, qui conduit, là encore, à utiliser des approximations rationnelles de $pi$. Cette démarche rappelle la méthode d'Archimède pour approximer $\pi$. D'aileurs Atiyah se réfère aussi à cette procédure historique \cite{Atiyah}.

\subsection{Le Principe Holique}
Les théories de pointe actuelles tendent à considérer l'espace-temps comme une propriété émergente. Cela signifie que les concepts de masse, longueur et temps sont en final réductibles à des nombres purs. Cete hypothèse justifie la relation entre le Cosmos et l'univers visible (figure 1) et a conduit au principe holique qui dans une équation diophantienne permet de distinguer des termes temporels, agissant par leur carré, des spatiaux, agissant par leur cube. La masse est associée à l'exposant 5, et le champ au 7. La résolution diophantiene la plus simple est donc:

\begin{equation}
T^2 = L^3 = M^5 = Ch^7 = x^{210}
 \end{equation}
 
 En effet, la clef holique du rayon de Hubble est singulière: 

\begin{equation}
R/\lambdabar_e = (2a^3/pH)^{210}
 \end{equation}
 

\subsection{Le rayon de Hubble \textit {invariant}}
Le modèle de la molécule gravitaionnelle d'hydrogène donne le rayon de Hubble $R =  2 \hbar^2/Gm_e m_p m_H \approx 13.812 \times 10^3$ années-lumère compatible avec les mesures directes du rayon de Hubble qui contredisent la cosmologie standard de 10% \cite{Tanabashi}.
Cette valeur de $R$ connecte avec la valeur de $k = 6 (d = 26)$ de l'Axe Topologique décrit par sa foncion réduite : $g(k) = exp(2^{k+1/2})/k$:

\begin{equation}
R/\lambdabar_e \approx g(6)
 \end{equation}

où $\lambdabar_e = \hbar/m_ec$ est la longueur d'onde réduite de l'électron. Les ultiples relations holographiques, impliquant en patticulier le rayonnement de fond se résument à:
...
Le nombre de « paramètres libres » dans le modèle standard est d’une trentaine. Les paramètres-clefs sont ces nombres purs (dits "sans dimension") qui interviennent le plus directement dans les théories physiques (tableau 1). En particulier, on considère le rapport des masses des particules les importantes avec celle de l'électron, ce qui s'est révélé déterminant dans l'Axe Topologique. Certains paramètres sont mesurés dans le domaine du ppb, mais les masses centrales dans la théorie électrofaible rappelée ci-dessous, ne sont définis qu'à $10^{-3}$ (boson scalaire massif de Brout-Englert-Higgs), $10^{-4}$ (boson chargé W), $10^{-5}$ (boson neutre Z).

Cet article prétend confirmer que, loin d’être "libres", \textit{ces paramètres-clefs sont des bases optimales de calcul, inconnues des mathématiques actuelles}.


\subsection{L’inversion des constantes de couplages}
Dans la théorie standard, on procède par approximations successives, c’est la méthode dite ’’des perturbations’’. Ainsi le couplage électrique est un  nombre inférieur à l’unité, de sorte qu’on puisse négliger son influence à des puissances suffisantes. On a pris l’habitude de le désigner par la lettre $\alpha$, appelée ‘constante de structure fine’, car ce nombre aparaît dans l’étude des raies spectrales, jusqu’à des puissances cinquièmes et plus. Il est lié à la charge électrique, mais sa notation courante $e$ peut entrainer des confusions avec la base optimale des logarithmes néperiens, qui est d’importance centrale dans le cadre d’un cosmos calculateur. Dans cet article, la charge électrique élémentaire est notée $q$. Le Cosmos devant utiliser des bases de calcul supérieures à l’unité, les 4 constantes de couplages (électrique, gravitationnelle, faible, forte) notées $a, a_G, a_w, a_s$, sont les inverses des quantités officielles. 
...
où $d_e ≈ 0.00115965218128(30)$ est l’excès magnétique de l’électron de masse $m_e$, intimement associée à $\sqrt a$, mais au prix de calculs qu’Atiyah qualife justement d’herculéens. Noter que cette valeur de $W$ est en désaccord de 2 $\sigma$ avec la valeur mesurée (Table 1).





\subsection{La période non-Doppler des quasars}
...
\subsection{Les fermions primaires, le muon et le tau et la relation de Koide}
L'Axe Topologique est dévolu aux bosons, mais les deux fermions principaux le muon et le tau apparaisent par leur rapport, pour k = 1, en dimension 6:

\begin{equation}
\tau/mu \approx g(1) \approx 2 a_s
\end{equation}

La valeur optimale de $\tau$ est déduite de cette formule et de la relation $\mu \approx a\sqrt{(a_w/pH}$ précisant, au ppb près, la mesure, déjà à 10 ppb, du rapport de masse muon-électron. A noter que la simplicité de cette formule de Koide ne reçoit aucune explication dans le modèle standard, qui manifeste, là encore, son insuffisance:

\begin{equation}
(1+\mu+\tau)/2 = (1+\sqrt\mu+\sqrt\tau)^2/3 
\end{equation}

Cette formule s'est révélée prédictive à une époque où la mesure de $\tau$ était fausse de 3 sigmas. Voir ci-dessous son intégration directe avec le modèle électro-faible 
...
Ainsi la charge électrique $q$  est intimement relié au couplage électrique $\sqrt a$. Le triangle représentatif de ce dernier montre une nette connexion avec les puissances 2 et 4 du nombre d'or, ce qui avait été également prédit \cite{Sanchez1}.
...
La précision actuelle sur la mesure des masses $W$ et $Z$, de $1.5 \times 10^{-4} et 2.3 \times 10^{-5}$ est insuffisante pour caractériser les approximations de $v$ et $\pi_e$ réalisés dans cette formule. Avec les valeurs optimisées $W \approx 157340.1093$ et $Z \approx 178451.7402$  rappelées ci-dessus, on observe que le nombre $495$ corréle avec le facteur gravitationnel $P$ et la constante d'Atiyah $Gamma$ dans la formule très symétrique suivante (domaine ppb):
...
En complétant le triangle de Weinberg ci-dessus par un trosième axe portant l'unité, on obtient un vecteur de module diretement lié à la constante gravitationnelle $p_G$ ci-dessus:

\begin{equation}
 1 + {q'}^2  + {q''}^2 = 1 + (m_Z/m_{BEH})^2 \approx 2a^3/pp_G
 \end{equation}
 
 La précision de cette relation ($10^{-7})$ confirme la valeur ci-dessus du boson scalaire $ m_{BEH} = 495^2 m_e $. Comme rappelé ci-dessus, le terme $2a^3/pH \approx (R/\lambdabar_e)^{1/2\times 3\times 5\times 7}$ est la clef holique du rayon de Hubble. 

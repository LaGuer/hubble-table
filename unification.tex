...
The only known couple of Wieferich numbers: $p_{W1} $ = 1093 and 
%$\approx = e^7 \approx 8 a$,
$p_{W2}$ = 3511
%$\approx e^{3e}$

%is close to the \textit {supersymmetric} couple (meson $\eta$, fermion $\tau$). They connect with the only known couple of Mirimanoff numbers, which uses the base 3, instead of the Wieferich base  2: $p_{M1}$ = 11 and $p_{M2}$ = 1006003 = 1003^2 - 6, where 1003 = $K_9 + K_{10} + K_{11}$ shows up in the Crystallographic Table. One notes:
...
The first Mirimanoff number 11 means that $3^{11-1}-1$ is a multiple of $11^2$.This recalls that \cite{Sanchez5}:

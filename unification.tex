...
où $d_e ≈ 0.00115965218128(30)$ est l’excès magnétique de l’électron de masse $m_e$, intiment associée à √a, mais au prix de calculs qu’Atiyah qualife justement d’herculéens. Noter que cette valeur de $W$ est en désaccord de 2 $\sigma$ avec la valeur mesurée.
...
La formule ci-dessus implique le terme $W^96$, très particulier, car très voisin de $Z^95$. En effet, à $10^{-7}$ près, l’ordinateur indique:
...
où $2\pi^2a^3$ est l’aire de la sphère 4D de rayon $a$, mais cette relation montre une imprécion de 565 ppm.
...
on constate que la déviaton ci-dessus est voisine de  $H/p_W$, à 1.5 ppm près. Et ce dernier écart est éliminé si on considère la valeur suivante de $\pi$:
...
Ainsi la charge électrique $q$  est intimement relié au couplage électrique $\sqrt a$. Le triangle représentatif de cet dernier montre une nette connexion avec les puissances 2 et 4 du nombre d'or, ce qui avait été également prédit dans la conférence de Cambridge.
...
Le modèle standard de la physique des particules a recu une éclatante confirmation lors de la découverte du boson scalaire de Brout-Englert-Higgs. Le champs scalaire est rajouté au modèle de jauge pour produire des bosons massifs, selon le potentiel :
...
La précision actuelle sur la mesure des masses $W$ et $Z$, de $1.5 \times 10^{-4} et 2.3 \times 10^{-5}$ est insuffisante pour caractériser les approximations de $v$ et $\pi_e$p réalisés dans cette formule. Avec les valeurs optimisées $W \approx 157340.1093$ et $Z \approx 178451.7402$  rappelées ci-dessus, on observe que la la valeur $v_0 = 495$ corréle avec le facteur gravitationnel $P$ et la constante d'Atiyah $Gamma$ dansla formule rès symétrique suivante (domaine ppb):

\begin{equation}
(v_0 W a/ 137 Z)^3 \approx P (H/np)^4/\Gamma 
\end{equation}
 
La valeur $\pi_e$ connecte avec une approximation $\Phi_e$ du nombre d'or, et manifeste une rationalisation symbolique de $\pi$, :

\begin{equation}
\pi_e = ({q'} \sin\theta)^{2} a/4 = 4/\sqrt{\Phi_e}   \approx   3 + 2 \pi^2 a^3 /(137 a_0)^{2}   
\end{equation}

où $2 \pi^{2a^{3}}$ l’aire de la sphère 4D de rayon $a$, centrale en cosmologie, comme rappelé ci-dessus, et $a_0 = (a_w/WZ)^{2}$ est la valeur brute de $a$, en connection directe avec 137 (3.4 ppm):
...
On constate que $1/sin\theta - 1/cos\theta$ est voisin de 1. En particulier, on constate que
...
En complétant le triangle de Weinberg ci-dessus par un troième axe portant l'unité, on obtient un vecteur de module diretement lié à la constante gravitationnelle ci-dessus:
...
\begin{equation}
a_1 \approx \pi_1^4\sqrt2    ~~~~   \rightarrow ~~~~   \pi_1 \approx (\sqrt{pn}/6): 3, 7, \sqrt{H\sqrt{nH}/3p}
\end{equation}
...
une nouvelle confirmation de la pertinence de la constante d'Atiyah $\Gamma = \gamma a/\pi$

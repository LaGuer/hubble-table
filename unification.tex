...
Par contre, ce modèle standard est conforté par la relation suivante, précise à 48 ppm:

\begin{equation}
\sqrt{WS^{(26)} \approx 495^2  
 \end{equation}

compatible avec le rapport de masse $H^0$ du boson scalaire massif. L'importance arithmétique du nombre 495 sera détaillée dans le chapitre suivant. En identifiant $H^0 = 495^2$, cela définit la charge de jauge $q' = q/sin\theta$, qui est donc en relation approchée simple avec $S^{(26)}$:
...
ce terme $K_+^{(12)}$ = 1001 = $S_3^{(10)}-2$ établit donc la liaison entre les dimensions 26 des cordes et la dimension 10 des supercordes.

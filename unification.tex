...
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{lllll}
    ...
    %\multicolumn{4}{c}{}                   \\
    \cmidrule(r){1-4}
    $m_G$ & $m_{\hbar}^2$    & $r_1 = \hbar^2/Gm_G m_{\hbar}^2$  & Ecart &Propriété arithmétique \\
    \midrule
    
    
    $m_N $ & $m_N^2$   & $R_N/2$ demi-rayon holographique du Cosmos  &  exact & $R_N/\lambdabar_e \approx (3^3)^{3^3}$ \\
    $m_{bc} $ & $m_{bc}^2$   & $2l_{cc}$ double longueur non-locale & $-6.3 \times 10^{-3}$ & $ l_{cc}/\lambdabar_e \approx \pi^{50}$  \\
    $m_Pa^3 $ & $m_pm_H$  &$\lambda_{Wn}$ longueur d'onde de Wien du rayonnement de fond & $-3.2 \times 10^{-4}$ &$\lambda_{Wn}/l_P \approx \pi^{64}$\\
    $m_e $ & $m_pm_H$   & $R/2$ demi-rayon de l'Univers &exact& $R/\lambdabar_e \approx g(6) \approx 2^{128}\approx (2R/R_N)^{210}$   \\
    
    $m_{bc}R/R_N $ & $m_{ph}m_{gr}$   & $R_C$ rayon du Cosmos & $4.7 \times 10^{-4} $& $ R_C/\lambdabar_e \approx e^{e^{2e}} \approx 6^{128}\approx (2R/R_N)^{64a_s}  $\\
...
\textit {$f(16)$ est le terme central de l'Axe Topologique, qui est ainsi confirmé à 0.2 ppm près par l'ADN.} 
 
l'Eq. (15) donne une masse de bicodon qui présente un écart $ 5 \times 10^{-4}$, qui est \textit{l'incertitude actuelle de la masses effective, tenant compte de la distribution des isotopes, mal connue à cause de variations locales} \cite{Wichers}.

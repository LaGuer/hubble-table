...
\begin{table}
\caption[Table \ref{CosmosTable}: 39 Cosmos radius formula]{39 formula for $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ R_N^2/2l_P $ & 9.0757734 & Rayon Holographique à partir de $R_N$ \cite{Sanchez2}   \\
 
 $ Rl_{nl}^2c^2a_w/R_NGm_{bc} $ & 9.07574 & Principe Holique, avec $m_{bc} \approx 1839.3 m_H$ \cite{Sanchez3}   \\
...
$ \lambdabar_P (H~210^{210}/p\pi_q^2sin \theta)^{1/4}$ & 9.07578 & Confirme le nombre Holique $210^{210}$   \\
 ...
 $  \lambdabar_P ~ (\sqrt a_w (a_s/d_e)^2)^{16}$  & 9.07568 & Empirique  \\
  ...
  $  \lambdabar_e ~ (a^2/137\pi \beta)^2(210^{210/ln a}/q \tau$  & 9.07579 & Empirique, utilisant le Nombre Holique $210^{210}$  \\
   ...
   $  12^2d_e^{3/2}(\lambdabar_e^2/r_H) ~ exp(exp(exp(exp(exp(-q')))))  $ & 9.07588 & Liaison directe avec la charge de jauge $q'$ \\

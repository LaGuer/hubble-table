...
Le Principe Calculatoire est confirmé par une série de rationalisations symboliques de $\pi$. Celle liée au proton et à la théorie de Wyler permet de relier $G$ à la constante de Lucas, le dernier terme de la Hiérarchie Combinatoire. Une autre, liée aux bosons intermédiaires, confirme l'invariance de la température de fond, éliminant définitivement le concept d'expansion de l'Univers, au profit de la Cosmologie Permanente, dont le rayon de Hubble est donné par 15 formules précises à $10^{-7}$. La forme extrême du Principe Caclulatoire, le Principe Holique, implique une Hypersymétrie entre les constantes $G$ et $hbar$, permettant de définir le rayon de Hubble et celui du Cosmos à partir de masses fondamentales, où le bi-codon d'ADN est central. Le quantonde spatio-temporel quasi-local, lié à $c$, apparait alors comme émergent dans un Cosmos calculateur à deux étapes. Le Multivers est ainsi exclu au profit d'une Unification Science-Arithmétique. 
\end{abstract}


% keywords can be removed
\keywords{Quantum Physics \and Holographic Principle \and Holic Principle  \and Cosmology \and Number theory  \and Multi-dimensional Crystallography}
...
La Science se caractérise par l'existence de lois de conservation, aussi bien en physique qu'en biologie. Les théoriciens ont associé conservation et symétrie, mais plus fondamentalement, textit{conservation implique calcul, donc c'est l'Arithmétique qui gouvene le monde}. Cela élimine d'entrée l'hypothèse centrale de la physique classique, l'hypothèse du continu, et donc justifie l'apparition de phénomènes quantiques.

La théorie standard procède ssentiellement par approximations successives: c’est la méthode dite "des perturbations". Ainsi le "couplage électrique" standard est un  nombre inférieur à l’unité, de sorte qu’on puisse négliger son influence à des puissances suffisantes. On a pris l’habitude de le désigner par la lettre $\alpha$, appelée ‘constante de structure fine’, car ce nombre apparaît dans l’étude des raies spectrales, jusqu’à des puissances élevées. Il est lié à la charge électrique, mais la notation courante de celle-ci, $e$, peut entrainer des confusions avec la base optimale des logarithmes néperiens, qui est d’importance centrale dans le cadre d’un cosmos calculateur, où la charge électrique élémentaire est donc notée $q$. 

\textit {Un tel Cosmos calculateur devant utiliser des bases de calcul supérieures à l’unité}, les 4 constantes de couplages considérées (électrique, gravitationnelle, faible, forte), $a, a_G, a_w, a_s$ sont ici les inverses des quantités officielles. Très importante aussi est la constante $P = m_P/m_e$, le rapport de la masse de Planck sur celle de l'électron, voir en Appendice le bas de la (Table \ref{tab:1:table1}). Les constantes dimensionnées sont rassemblées dans la (Table \ref{tab:2:table2}) \cite{Tanabashi}. 


\section{L'Axe Topologique et le rayon \textit{invariant} de Hubble}
Dans le cadre de la Cosmologie de précision, le réexamen des corrélations entre grands nombres conduit à l'Axe Topologique (Fig. 1), qui montre que le rayon de Hubble est invariant et confirme la théorie bosonique des cordes. 

La théorie des cordes a suscité un espoir énorme quand elle a exhibé une particule de spin 2, qu'on a identifié au graviton, intégrant ainsi la gravitaton. De plus, elle a mis en évidence une correspondance holographique explicite entre des espaces-temps de dimensions différentes.  \textit{Mais cette théorie bute sur le concept de l'univers en expansion}. 

La plupart des physiciens ont opté pour la soluton de facilité, le "Multivers", évacuant le problème, pourtant ancestral, de la caractérisation précise de l'Univers. Ils s'appuient sur le fait que le nombre de solutions pour le repliement des dimensions cachées est énorme ($10^{500}$). 
...
\caption[The Topological Axis: figure \ref{tab:9:table9}.]
...
Mais cela est fortement discuté actuellement, et remis en question par une étude précédente \cite{Sanchez2} qui a montré que les multiples corrélations observées entre les grands nombres de la physique mathématique pouvait être considérées comme une succession de relations holographiques 1D-2D, aboutissant à l'Axe Topologique (Fig  \ref{tab:9:table9}), confirmant la théorie bosonique des cordes, et sa dimension 26, qui correspond au rayon de Hubble donné par le modèle de la molécule gravitationnelle d'hydrogène $R =  2 \hbar^2/Gm_e m_p m_H \approx 13.812 \times 10^9$ années-lumère, compatible avec les mesures directes du rayon de Hubble qui contredisent la cosmologie standard de 10\% \cite{Tanabashi}, ce qui est rédhibitoire dans le cadre d'une cosmologie de précision.


Cette valeur $R$ connecte avec la fonction topologique réduite : $g(k) = exp(2^{k+1/2})/k$, pour la valeur $k = 6 (d = 26)$ de l'Axe Topologique, où $26$ est la dimension privilégiée de la thèorie bosonique des cordes: 
...
La table \ref{tab:8:table8} présente 15 formules au ppb pour ce rayon de Hubble. Cette démarche rappelle celle de Jean Perrin qui, en présentant 14 formules impliquant des nombres de molécules en 1908, a finalement convaincu la communauté de l'existence réelle des atomes. Cette table devrait convaincre les indécis de \textit{l'inexistence de l'expansion de l'Univers}.

Rappelons que l'expansion de l'univers est avantageusement remplacée par une force répulsive entre galaxies, proportionnelle à la distance, correspondant à \textit{la constante cosmologique} que Hilbert avait suggéré à Einstein pour compléter ses équations. En effet, une telle récession galactique est naturellement accélérée, ce qui a été une totale surprise pour les cosmologues standards. La raison d'être de cette force est qu'elle autorise un renouvelement par des bébé-galaxies, effectivement observées par Halton Arp \cite{Arp}, empéchant ainsi l'Univers de finir dans un 'Big Child', comme c'est le cas dans la cosmologie standard actuelle.
...
%}}
\node[color=green] at (0.,0.5,-0.5) (l){$\mathbf COSMOS$};
%\node at (2.,0.0,1.5) (l){$\mathbf Gravitation$};
\node[color=green] at (1.,0.25,1.75) (l){$\mathbf QUANTONDE$};
\node[color=red] at (1.,1.5,1.5) (l){$\mathbf PHOTONDE$};
\tkzLabelSegment[sloped,below,text=blue](d1,M){$GRAVITONDE$}
\end{tikzpicture}
\caption[The cG$\hbar$ Triedre]{ Cosmologie Quantique : le Triedre $G\hbar c$}
\end{figure}
...
\tkzDefPoint[color=red](-12.0,-12.0){E}
\tkzDefPoint(-7.0,-7.0){F}
\tkzDefPoint(-4.25,-4.25){G}
\tkzDefPoint(4.0,4.0){H}
\tkzDefPoint(12.0,12.0){I}
\tkzDefPoint(-3.0,-3.0){J}
\tkzDefPoint(15.25,15.25){K}
\tkzDefPoint(17.25,17.25){L}
\tkzDefPoint(19.25,19.25){M}
\tkzDefPoint(25.25,25.25){N}
\tkzDefPoint(-10.0,-10.0){P}
\tkzDrawPoints(A,B,C,D,F,E,G,H,I,J,K,L,M,N,P)
%\tkzMarkRightAngle(A,D,C)
%\tkzLabelPoints[below](A,C)
%\tkzLabelPoints[above](STRING,GLUON)
%\tkzLabelPoints[left](BOSONX_GUT)
...
\draw[color=blue] (A) -- (Y);
\node [anchor = east,text=red] at (31.,31.0) {$COSMOS$};
\node [anchor = south] at (-28.,-32.0) {$2$};
\node [anchor = south] at (-28.,-34.0) {$0$};
\node [anchor = north] at (-28.,-28.0) {$STRING$};
\node [anchor = south] at (-20.,-32.0) {$6$};
\node [anchor = south] at (-20.,-34.0) {$1$};
\node [anchor = east] at (-20.,-20.0) {$GLUON$};
\node [anchor = south] at (-12.,-32.0) {$10$};
\node [anchor = south] at (-12.,-34.0) {$2$};
\node [anchor = east] at (-12.,-12.) {MEAN BOSON PI};
\node [anchor = west,color=red] at (-12.,-12.) {ATOM DIAMETER};
\node [anchor = east] at (-10.,-10.) {KAON};
\node [anchor = south] at (-4.25,-32.0) {$14$};
\node [anchor = south] at (-4.25,-34.0) {$3$};
\node [anchor = east] at (-4.0,-4.0) {HALF MEAN WEAK BOSON};
\node [anchor = west,color=red] at (-4.0,-4.0){HALF RYDBERG WAVELENGTH};
\node [anchor = south] at (-2.75,-32.0) {$\gamma\Gamma$};
\node [anchor = east,color=black] at (-3.0,-3.0){SCALAR BOSON};
\node [anchor = east,color=green!90!black] at (0.,0.0) {$BICODON$};
\node [anchor = west,color=black!40!green] at (0.,0.0) {$2R_N/R$};
\node [anchor = south] at (4.25,-32.0) {$18$};
\node [anchor = south] at (4.25,-34.0) {$4$};
\node [cross,anchor = west,color=red] at (4.25,4.25){THERMAL PHOTON};

\node [anchor = south] at (12.25,-32.0) {$22$};
\node [anchor = south] at (12.25,-34.0) {$5$};
\node [anchor = east,color=black] at (12.25,12.250){$BOSON X_{GUT}$};
\node [anchor = east,color=green!85!black] at (15.25,15.250){$PHOTONDE$};
\node [anchor = west,color=red] at (15.25,15.250){KOTOV length $*2\pi$a};
\node [anchor = west,color=red] at (17.25,17.250){Galaxy group};
\node [anchor = south,color=red] at (17.25,-32.0) {$\Gamma$};
\node [anchor = east,color=green!85!black] at (19.25,19.250){$GRAVITONDE$};
\node [anchor = west,color=red] at (19.25,19.250){HUBBLE Radius $/6$};
\node [anchor = east,color=green!85!black] at (25.25,25.250){$SUPER PERIOD$};
\node [anchor = east,color=black] at (26.25,26.250){$TOPON$\tiny{:Visible universe wavelength}};
\node [anchor = south] at (20.25,-32.0) {$26$};
\node [anchor = south] at (20.25,-34.0) {$6$};
\node [anchor = south] at (28.25,-32.0) {$30$};
\node [anchor = south] at (28.25,-34.0) {$7$};
\node [anchor = south] at (30.25,-34.0) {$e^2$};
\node [anchor = south] at (31.0,-31.10) {$d$};
\node [anchor = south] at (31.0,-36.0) {$k$};
%%% Y AXIS
\node [anchor = north] at (-31.55,-31.0) {$0$};
\node [anchor = north] at (-31.55,-18.0) {$1$};
\node [anchor = north] at (-31.55,-8.0) {$2$};
\node [anchor = north] at (-31.55, 2.0) {$3$};
\node [anchor = north] at (-31.55, 12.0) {$4$};
\node [anchor = north] at (-31.55, 22.0) {$5$};
\node [anchor = north] at (-31.55, 30.0) {$2e$};
%%% Y' AXIS
...
\node [anchor = north] at (-34.25,26.0) {$10^{80}$};
\node [anchor = north] at (-34.25,28.0) {$y=\ln\ln(y)$};
...
\end{tikzpicture}
\caption[Topological Axis]{Figure \ref{tab:9:table9}: The Topological Axis is the extrapolation toward smaller numbers of the Eddington's Double Larger Number correlation. The double natural logarithms y = ln(ln(Y)) of the main dimensionless physical quantities (Y) corresponds to the special string dimension series d = 4k + 2, from k = 0 to k = 7, characteristics of a Bott octonion sequence. This is the reunion of height 2D-1D holographic relations, hence the name `Topological Axis`.}
\end{figure}
...
\caption[MLT Adimensional cuboid]{\textit{Geo-dimensional Universe-Grandcosmos couple}, with unit length the Electron Compton reduced wavelength. 
...
\section{La question de l'espace-temps}

Les théories actuelles (théorie des cordes, modèle des boules, ...) débouchent sur la question de la structure de l'espace-temps.

Le concept d'Univers en expansion étant éliminé, la courbure de l'espace-temps n'est alors qu'une propriété locale, tandis que l'Univers dans son ensemble présente une géomérie plate, euclidienne. C'est effectivement ce que les astrophysiciens constatent, et désignent sous le nom de 'condition critique'. L'application de la physique non-relativiste à l'Univers critique conduit alors à justifier directement la fraction $3/10$ de l'énergie totale. La partie complémentaire, les $70\%$ restants, étant attribuée à l'énergie sombre, qui n'est ainsi qu'un faux problème \cite{Sanchez2}.


\subsection{La Cosmologie Holographique: l'immergence des particules}
...
Cette "Cosmologie Permanente" conduit enfin à une explication de l'énormité ($10^{121}$) de l'énergie quantique du vide par rapport à celle de l'univers.
...
De plus ce nombre $4181$ est l'hypothénuse du rectangle pythagoricien dont les côtés sont $21\times 89$ et $2\times 34\times55$, où $21;34;55;89$ sont les termes de la suite de numéros d'ordre (8;9;10;11) \cite{Corbalan}. Or:

\begin{equation}
34\times55  = 21\times 89 + 1 (137^2-138/2)/10 
 \end{equation}

où $137^2/10$ est, au premier ordre, le rapport des racines dans l'équation d'Eddington $10x^2-136x+1=0$. L'analyse montre alors qu'une approximation $\Phi_1$ du nombre d'or est utilisée, qui définit relie les paramètres $a, a_w, W, Z$ à 0.8 ppm:

\begin{equation}
(a_w /WZ)^4 /2 = \Phi_1^{19} \approx \sqrt 5 \times sqrta_w/a 
 \end{equation}
 
 
\subsection{L'inexplicable période non-Doppler des quasars}

L'Axe Topologique réhabilite la théorie \textit{bosonique} des cordes qui a le défaut apparent de faire intervenir des tachyons. En fait, c'est plutôt un avantage pour expliquer le caractère non-Doppler de l'oscillation \cite{Kotov1} de période $t_K \approx 9600.06(2)$ s, qui apparaît à la fois dans le rayonnement solaire et de plusieurs quasars. 
...
Cela donne une valeur de $G$ précise à $10^{-6}$, compatible avec la mesure du BIPM, précise à $10^{-5}$ \cite {Quinn}.
...
Considérer l'espace-temps comme une propriété \textit{secondaire} signifie que \textit{les concepts de masse, longueur et temps sont en final réductibles à des nombres purs}. Cela justifie la relation purement numérique observée entre le Cosmos et l'univers visible (Figure/Table xyz). 


\begin{table}
\caption[Table \ref{tab:5:table5}: Cosmologie Holographique.]{Cosmologie Holographique}
\label{tab:5:table5}
...
Une telle synthèse arithmético-physique a été annoncée par le principe holique \cite{Sanchez1}, forme simplifiée du principe holographique, qui, dans une équation diophantienne, permet de distinguer des rapports temporels, agissant par leur carré, des rapports spatiaux, agissant par leur cube. L'équation $T^2 = L^3 = n^6$ est la forme diophantienne de la troisième loi de Kepler, qui implique $L = r_n/r_1 = n^2$ et $T = t_n/t_1 = n^3$ : d'où 
...
des équations de l'atome gravitato-quantiques, qui symétrisent $G$ et $\hbar$, associées à des masses $m_G$ et $m_{\hbar}$:
...
\textit{La cosmologie permanente est tachyonique, liée au couple $G,\hbar$, tandis la thèorie quantique corresond au couple $\hbar,c$, et la gravitation au couple $G,c$, voir le "trièdre des constantes"} (fig. \ref{tab:10:table10}).
...
\textit{Contrairement à la thèse du Multivers, basée sur une indépendance entre lois physiques et biologique et sur une mauvaise interprétation du principe anthropique, la biologie s'intègre dans le Cosmos calculateur, la molécule d'ADN devant être un hologramme-ligne}.
...
\begin{equation}
(R/\lambdabar_e)^{1/210} \approx 2R/R_N
 \end{equation}
...
Une "exigence de quasi-continuité" a été présentée dans la conférence ANPA 1994 \cite{Sanchez1}, introduisant, par souci de simplicité, le Principe Holographique, de manière beaucoup plus simpe que celle d'autres théoriciens \cite{Hooft}. En effet, par opposition aux équations différentielles, qui sont fondamentalement inadaptées dans un monde quantique, comme Poincaré l'avait expressément souligné \cite{Poincare}, \textit{les équations globales les plus simples} sont les connexions entre variétés topologiques de dimensions différentes (périmètre, aire, volume…), donc utilisent la constante $\pi$, et pourraient être liées aux symétries cristallines de dimensions supérieures, comme constaté ci-dessous.  
...
D'ailleurs, la théorie de Wyler \cite{Wyler} aboutit à une formule approchant $p$ à 18 ppm près, repérée initialement par Lenz, dans un article célèbre d'une seule ligne \cite{Lenz}:

\begin{equation}
p_W = 6\pi^5
\end{equation}
 
Noter que cette expression est le produit aire-volume d'un cube de coté $\pi$. 
...
Cette formule, confirme à 5 ppb près unde étude d'optimisation antérieure \cite{Sanchez2}, et réhabilite ainsi la théorie de Wyler, trop vite écartée, , qui, précisément, implique un espace de dimension 7, caractéristique de l'algèbre des octonions. \textit{Sir Atiyah avait effectivement prédit que la gravitation devait être liée à cet algèbre}\cite {Atiyah}.
...
\subsection{Le Tau et $pi$}
On observe le développement singulier suivant:
...
De plus, l'écart ci-dessus $e-2$ apparaît dans,  à 30 ppb:
...
La suite fractionnaire ci-dessus de $\pi$ est un problème non résolu des mathématiques actuelles. Cependant, on note que la partie entière de $\pi_5$, le nombre $292$, est la moyenne des nombres de symétries cristallines paires (419) et impaires (265) dans l'espace $10D$ des supercordes \cite{Weigel} \cite{Veysseyre} (Table \ref{tab:4:table4}). 
Dans cette table, figurent les nombres canoniques 137, 1836 et 1839, qui sont respectivement les parties entières de $a, p, n$  montrant que \textit{la théorie ultime est liée à l'arithmétique des symétries cristallines en dimensions supérieures}.
...
Avec la valeur de $G$ optimisée par une précédente étude de corrélation précise au ppb \cite{Sanchez2}, on observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$ (Table \ref{tab:1:table1} ):
...
\section{Le boson scalaire et le nombre co-parfait 495} 
Le nombre de « paramètres libres » dans le modèle standard est considérable, de l'ordre d’une trentaine. Les paramètres-clefs sont ces nombres purs (dits "sans dimension") qui interviennent le plus directement dans les théories physiques (table 1). En particulier, on considère le rapport des masses des particules les plus importantes avec celle de l'électron, ce qui s'est révélé déterminant dans l'Axe Topologique ci-dessus. Certains paramètres sont mesurés dans le domaine du ppb, mais les masses centrales dans la théorie électrofaible rappelée ci-dessous, ne sont définis qu'à $10^{-3}$ (boson scalaire massif de Brout-Englert-Higgs), $10^{-4}$ (boson chargé W), $10^{-5}$ (boson neutre Z).
...
\section{La relation de Koide des trois fermions principaux}
L'Axe Topologique est dévolu aux bosons, mais les deux fermions exotiques principaux, le muon et le tau, apparaisent par leur rapport, pour k = 1, c'est-à-dire dans les 6 dimensions cachées des cordes: $\tau/\mu \approx g(1) \approx 2 a_s$. L'étude des déviations conduit à la relation (1 ppm):

\begin{equation}
2 a_s \approx \sqrt{g(1)\tau/\mu}~ (H-p)^{-4}  
\end{equation}

Déterminant est la simplicité de la formule empirique suivante de Koide, où $\mu$ et $\ tau$ sont les rapports de masse des fermions muon et tau par rapport à l'électron, qui sont les correspondants de l'électron dans les deux "générations inutiles" des particules. 
...
Cette formule s'est révélée prédictive à une époque où la mesure de $\tau$ était fausse de $3 \sigma$s. La valeur optimale de $\tau$ a été déduite \cite{Sanchez2} de cette formule et de la relation $\mu \approx a\sqrt{(a_w/pH}$ précisant, au ppb près, la mesure, déjà à 10 ppb, du rapport de masse muon-électron.
...
\sin \theta^2 - \sin \theta_{\tau}^2  \approx 1/\sqrt a_w \sin_{eff}{\theta} (m_Z) \approx 1 / P^{1/4}
 ...
 faisant apparaître le terme $\sin_{eff}{\theta} (m_Z)$ appelé "effective weak mixing angle"  (Table  \ref{tab:1:table1}).
...
Les masses des bosons W et Z sont reliées à la masse du boson scalaire massif (Brout-englert-Higgs), voir le triangle de Weinberg (Figure 4). Comme le groupe SU(2) ne concerne que les rotations dans un plan, il est naturel de compléter le triangle de Weinberg par un troisième axe portant l'unité. On obtient un vecteur de module directement lié à la constante holique ci-dessus, où $H$ est remplacé par la masse gravitationnelle ci-dessus $p_G$ du proton :
...
\section{La relation charge-couplage électrique}
...
Ces relations sont illustrées dans le « triangle électrique » figure 6.
...
\begin{equation}
  P^4  \approx E^{1/(a-1)^2} \approx (9/2)^a \approx (1/\sin \theta)^{ja}/d_e^4 
\end{equation}
...
\bibitem{Poincaré} Poincaré H., Sur la théorie des quanta, Journal de physique, vol 2, p. 5.(janvier 1912).
...
\bibitem{Poincare} Poincaré H., Dernières Pensées. “Conférence à l’Université de Londres”, pp. 102-103 (Flammarion, 1913).\\
...
\begin{table}
\caption[Table \ref{tab:1:table1}: Adimensional primary constants.]{Adimensional primary constants}
\label{tab:1:table1}
...
\begin{table}
\caption[Table \ref{tab:2:table2}:Physical constants]{Physical constants}
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{lllll}
    \toprule
    %\multicolumn{5}{c}{}                  \\
 ...
 %Rydbergh reduced wavelength $\lambdabar_e(2aH/p)^2$ & $\lambdabar_{Ryd}$  & m  & $1.45190673 \times 10^{-8} $ & 0.3   \\
  ...
  %Official CMB temperature & $T_{CMBoff}$ & K & $2.7255(6)$ & $2 \times 10^5$ \\    
   ...
   Cosmos (CMB) thermal reduced wavelegth & $\lambdabar_{CMB} = \hbar c/ k_B T_C $ & m & $8.400716621 \times 10^{-4} $ & \cite{Sanchez2} \\ 
   Cosmos (CMB) thermal Wien wavelegth & $\lambda_{Wn} = \hbar c/ w $ & m & $1.063082472 \times 10^{-3} $ & \cite{Sanchez2} \\
...
\begin{table}
\label{tab:8:table8}
\caption[Table \ref{tab:8:table8}: 15 Hubble radius formulas]{15 ppb precise formula for $R \approx 13.8119768$ Gly}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{3}{c}{}                   \\
    \cmidrule(r){1-3}

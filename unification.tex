...
In this article, the search for precision correlations uses the precisely defined values obtained by the "maximal correlation principle" (Tables 1 and 2) \cite{Sanchez3}. It is supposed that the scalar boson/electron mass ratio is $H^{(0)}=495^2$, as confirmed in this article to 0.1 ppm. The corresponding gauge coupling constants for the groups U(1) and SU(2) are, with $cos \theta = W/Z$ (the gauge coupling $g_3$ for SU(3), badly measured, will be induced in this article): 
...
\label{tab:2:table2}
  ...
  Optimized  neutral Pion/Electron mass ratio  & $\Pi_0$ & 264.143971  & ppb\cite{Sanchez2} $Z_{0mes}$: 264.14341(97)\\ 
...
%  $ \lambdabar_{CNB}^2 \lambdabar_{CMB}^4 (ln\Phi)^8/l_P^4 R_C H^0 $  & 13.812   & Confirms the scalar boson $H^{(0)}$ and the pertinence of $ln\Phi \approx \sin \theta_{Zeff} $ \\
 ...
 $\lambdabar_e (2/d_e^8)^{128}/(1-2g_3)$  & 13.815    &  Empirical, from the Combinatorial Hierarchy Lucas Large Number [12] \\ 
...
$ 2\hbar^2/Gm_em_pm_n $ & 13.80 & c-free dimensional analysis \cite{Sanchez4}  \\
 ...
 $ (\lambda_{CMB}/(j+1))^2/l_P$ & 13.80 & Central role of mammal temperature: $T_{mam}\approx j T_{CMB}$, with $j = 8\pi^2/ln2$ \\
  ...
  $ 2 \lambdabar_{e} (1/sin\theta)^{10d_e\sqrt {137}} $ & 13.80 & Corresponds to $(1/sin\theta) \approx 3/\sqrt2$   \\
...
This radius $R_e$ connects with $R$ by the following $10^{-7}$ precise relation, implying the "Weinberg-Sanchez" natural geometric extension of the Weinberg triangle \cite{Taylor}, $1/g_3 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2$, identified as the inverse SU3 coupling where $g_1 = W/H^{(0)}$ is the U(1) coupling and $g_2 = Z sin~\theta/H^{(0)}$ is the SU2 coupling, with $cos\theta = W/Z$ and the BE-Higgs scalar boson mass ratio, by respect to electron $H^{(0)} = 495^2$ (125.208 GeV)\cite{Sanchez3}. With $p_G = P/2^{127/2}$ (from the Combinatorial Hierarchy \cite{Bastin}), to $10^{-7}$, 
...
\begin{equation}
 \left\{
    \begin{array}{ll}
        ...
        1/g_3 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2\\
        R_e \approx 2R~H/g_3p_G
    \end{array}
\right.
\end{equation}

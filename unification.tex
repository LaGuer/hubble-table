...
\label{tab:2:table2}
  ...
  %This work "weak-mixing angle" & $(\sin \theta)^2(m_Z)$    & 0.231475 &  \\
   ...
   Optimized U(1) gauge coupling & g_1   & 0.3436256 & 100 this work \\
...
\section{Les paramètres-clefs du modèle électrofaible}
...
Le modèle standard de la physique des particules (table) a unifié les forces électriques et nucléaires faibles dans le modèle "électrofaible". La théorie de jauge invoquée, introduit les cuplages de jauge $g1$ (groupe U(1) et $g_2$ (groupe SU(2)), reliés à la constante électrique $a$ par \cite{Taylor}:
...
L'analyse sur ordinateur, basée sur les constantes principales : $\Gamma$ (constante d'Atiyah), $e^e$ terme "économique", $a/9$, voisin du précédent et $p_0 = 1831$, justifié plus loin, conduit à \textit{la triple ppb-relation} suivante, confirmant le couple $W,Z$ et définissant $sin\theta$ par son cube. 
 %\begin{equation}
...
\begin{equation}
(Z/W - W/Z)^{-1} \approx (H/p)^2\Gamma \sqrt 2/9 \approx p_0/a e^e (sin\theta)^2 \approx
 \end{equation}
 
Mais le $cos\theta$ correspondant présente un écart de 70 ppb avec $W/Z$, laissant présumer que ce modèle n'est pas complet. En effet l'interaction forte fait intervenir un couplage de jauge $g_3$ du groupe SU(3). Noter que $g_3 = 4\pi_q Z/W\sqrt a$ présente le même écart avec $g_1g_2\sqrt a_2$, où $\sqrta_2 = \qrt a sin\theta $ 

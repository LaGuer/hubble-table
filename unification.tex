...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 37 Hubble radius formulas]{37 formula for $R$}
\label{tab:6:table6}
...
\begin{table}
\caption[Table \ref{CosmosTable}: 33 Cosmos radius formula]{33 formula for $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ Rl_{nl}^2c^2a_w/R_NGm_{bc} $ & 9.07529 & Principe Holique, avec $m_{ph} = \hbar /cl_{nl}$, $m_{gr} = m_{ph}/a_w$, avec  $m_{bc} \approx 1838.419
 ~m_H$ \cite{Sanchez3}   \\
...
%$ \lambdabar_P (H~210^{210}/p\pi_q^2sin \theta)^{1/4}$ & 9.07578 & Confirme le nombre Holique $210^{210}$   \\
 ...
 $  \lambdabar_P ~ (\sqrt a_w (a_s/d_e)^2)^{16}$  & 9.07568 & Empirique  \\
  ...
  %$  \lambdabar_e ~ (a^2/137\pi \beta)^2(210^{210/ln a}/q \tau$  & 9.07579 & Empirique, utilisant le Nombre Holique $210^{210}$  \\
   ...
   %$  12^2d_e^{3/2}(\lambdabar_e^2/r_H) ~ exp(exp(exp(exp(exp(-g_2)))))  $ & 9.07588 & Liaison directe avec $g_2$, le couplage SU(2)  \\
  ...
  $ \lambdabar_W \pi{210 + 1/3}  \pi^{210} $ & 9.06 & Confirmation holique de la base $\pi$ \\
...
Carl Hermann \cite {Hermann} a calculé le nombre de symétries ponctuelles cristallographiques pour des espaces de dimensions entre de 1 à 8, voir la première ligne du tableau 6.7. C'est le nomnbre de polynomes dont le terme principal est $x^d$, à coefficients entiers dont toutes les racines sont sur le cercle unité. On remarque que \textit {le terme de degré 7 est 118, correspondant au numéro atomique de l'atome terminal, l'Organesson, dans le tableau périodique à 7 rangées et 32 colonnes}, voir ci-dessous l'analyse qui prouve que cela n'est pas dû au hasard. Cette analyse est basée sur les termes spectroscopiques de types $2 \times (2k +1)$, où $k$ est le nombre quantique orbital, qui s'identifie avec la série spéciale des cordes de l'Axe Topologique, en correspondance avec les bosons de jauge.
...
justifiant, à une unité près, justifiée ci-dessous, la somme canonique d'Atiyah, intégrant les octonions ($2^7$), et les quaternions ($2^3$), et la "séparation de jauge", rappelée ci-dessous, $136 = d_7 + d_{26} = 30 + 106$. Ce nombre primaire d'Eddington 136 est le nombre d'éléments indépendants dans la matrice symétrique $16\times16$.
...
Les dimensions 10 et 11 sont considérées dans la théorie des supercordes, et la dimension 12 connecte avec les 24 dimensions "transverses" de la théorie bosonique des cordes. De plus, la racine carrée du rapport de masse boson scalaire/électron, le nombre 495, central dans le présent article, est le coefficient binomial $C_{12}^4$, voir ci-dessous.

...
\label{tab:2:table2}
  ...
  %This work "weak-mixing angle" & $(\sin \theta)^2(m_Z)$    & 0.231475 &  \\
   ...
   "Squared Weak-mixing angle"  & $(\sin \theta)^2(m_Z)$    & 0.23122(4) & $1.7 \times 10^7$ \\
  
   Optimized "Squared Weak-mixing angle" $qR/R_N$  & $(\sin \theta)^2(m_Z)$    & 0.2311293 & $this work$ \\
   
  Effective "Squared Weak-mixing angle"  & $(\sin \theta_{eff})^2(m_Z)$    & 0.23148(4) & $1.7 \times 10^7$ \\
  
   Optimized "Squared Weak-mixing angle" $(ln\Phi)^2$  & $(\sin \theta)^2(m_Z)$    & 0.23156482 & $this work$ \\
...
La Table \ref{tab:6:table6} présente 23 formules simples qui confirment ce rayon de Hubble. Cette démarche rappelle celle de Jean Perrin qui, en présentant 14 formules impliquant  théories moléculaires en 1908, a finalement convaincu la communauté de l'existence réelle des atomes. Cette table devrait convaincre les indécis de \textit{l'inexistence de l'expansion de l'Univers}.



\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 23 Hubble radius formulas]{23 formula for $R$}
\label{tab:6:table6}
...
\begin{table}
\caption[Table \ref{CosmosTable}: 24 Cosmos radius formula]{24 formula for $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
...
$  \lambdabar_e ~ cotg\theta ~ 495^{\pi \sqrt a}  $ & 9.03 & Base 495, avec exposant $ \pi \sqrt a\approx \pi^{\pi}$  \\ 
  ...
  $  \lambdabar_e ~ exp(exp(exp(exp(exp(-q')))))  $ & 8.6 & Empirique, liaison avec la charge de jauge $q'$ \\
...
\begin{equation}
 \left\{
    \begin{array}{ll}
        E = exp(exp(e^e))\approx ((e^{3e})^{(e^{3e})})^{e^6/3} \approx e^{a(2e^e)^3}   \\
...
Le modèle standard des particules confirme la fonction topologique et la Cosmologie Permanente. Mais celle-ci va au-delà en affectant \textit{une masse aux gluons, photon et graviton}. Elle prévoit que la mission Euclide, consacrée à la variation, par tranche d'un milliards d'années-lumière, du taux de matière sombre, devrait constater \textit{une invariance du taux de matière sombre au lieu de la décroissance attendue}. Le futur télescope infra-rouge Webbs, consacré à la formation des galaxies lointaines, devrait constater \textit{une profusion inattendue de vieilles galaxies à longue distance.}

Indépendamment de la Cosmologie, les matrices circulantes de Koiré montrent aussi les limtes du modèle standard actuel.

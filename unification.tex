...
\label{tab:6:table6}
 ...
 $ R/\lambdabar_e  \approx 6H (2\pi_{hol} a^3)^5 $& 13.81197676    &  Holographique, avec     $\pi_{hol} = 3 + 1/(7+1/(16+2/e^{2e}))$\\
  ...
  $(\lambdabar_{e} 2^{128}/d_e^2(m_H/m_p)^6$  & 13.81197676   & Identification, à partir du grand nombre de Lucas, du moment magnétique $d_e$ \\
...
L'étude d'Hermann a été complétée et généralisée par l'équipe de Weigel \cite{Weigel}, (table 6.7). La suite fractionnaire ci-dessus de $\pi$ est un problème non résolu des mathématiques actuelles. Or, on note que la partie entière de $\pi_5$, le nombre $292$, déjà rencontré ci-dessus en connexion avec le nombre d'or $\Phi_a$, est la moyenne des nombres de symétries cristallines paires (419) et impaires (165) dans l'espace $10D$ des supercordes \cite{Weigel} \cite{Veysseyre} (Table \ref{tab:4:table4}).

On observe la relation suivante entre les trois paramètres de couplages électrique, électrofaible et gravitationnel $a, a_w, P$, à 1 ppb près: (Table \ref{tab:2:table2} ):

\begin{equation}
a_w^{5/2} \approx (419/417) P a^3 
 \end{equation}

$417 = 419 - 2$ est le nombre de symétries positives $10D$ non triviales. Le rapport $419/417$ est voisin de $2^{1/12^2}$, le limma musical repéré en physique des particules \cite{Sternheimer}. 

...
\begin{equation}
 \sum_{k=0}^{k=7} (d_k+1)= 2^7 + 2^3 = 136 = 118 + d_4 = d_7 + d_{26}
 \end{equation}

retrouvant la séparation $d_7 + d_{26} = 30 + 106$ et la somme canonique d'Atiyah, intégrant les octonions ($2^7$), et les quaternions ($2^3$). Ce nombre primaire d'Eddington 136 est le nombre d'éléments indépendants dans la matrice symétrique $16\times16$. Il correspond à la suite atomique ci-dessus, augmentée de la série $d_4$ = 18. Cette couche spectroscopique (appelée "g") est effectivement considérée par les chimistes comme devant prolonger la table des éléments.
...
\begin{equation}
 a (sin \theta)^2  = {a'}  \approx 30.505998 \approx 2e^e ,    ~~~~         a (cos \theta)^2  = {a''}  \approx 106.530011 \approx 4\pi a_s
 \end{equation}
...
où l'on reconnaît le terme $2e^e$ de l'Axe Topologique. On observe les \textit{approximations successives} suivantes, à 70, 20 ppm, 0.6 ppm et 15 ppb près: 
%la dernière provenant de $\tau/mu \approx 9 / \tg~\theta \approx \Gamma \sqrt2 \sin~\theta (H/p)^2$ :
\begin{equation}
 \left\{
    \begin{array}{ll}
        137 \approx {a'} a^3/\sqrt a_w \approx \tau/{a'} \sin(2\theta)\\
        2\times 30\times 30.5 /  a e^e \approx \cos\theta\\
        2\times 30\times 30.5 /  a e^e \approx \Gamma \sqrt 2 (H\sin\theta/3p)^2
    \end{array}
\right.
\end{equation}

\textit{La constante d'Atiyah $\Gamma$ confirme donc la valeur de $\sin~\theta$, donc de la masse $495^2$ du boson scalaire et la valeur de $G$ dans le domaine du ppb. Le programme d'Atiyah \cite{Atiyah} est ainsi en passe de se réaliser}.

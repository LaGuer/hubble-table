...
où $R$ est le rayon de Hubble et $ R_N/2 l_P^2/r_e^3$ est le demi-rayon \textit{holographique} du Cosmos, correspondant à l'élimination de $c$ entre la longueur de planck $l_P$ et le rayon classique de l'électron $ r_e = \hbar/am_ec$, ce qui introduit la masse de Nambu $m_N = am_e$, centrale en physique des particules\cite{Nambu}.

\textit{La cosmologie est liée au couple $G,\hbar$, laquantique au couple $\hbar,c$, et la gravitation au couple $G,c$, voir le trièdre des constantes} (fig).
...
\textit{Contrairement à la thèse du Multivers, basée sur une indépendance entre lois physuques et biologique, et sur une mauvaise interprétation du principe anthropique, la biologie s'intègre dans le Cosmos calculateur, la molécule d'ADN devant être un hologramme-ligne}.

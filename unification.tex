...
\begin{table}
%\label{tab:6:table6}
\caption[Table \ref{tab:6:table6}: 53 Hubble radius formulas]{53 formula for Hubble radius $R$}
\label{tab:6:table6}
...
This radius $R_e$ connects with $R$ by the following $10^{-7}$ precise relation, implying the "Weinberg-Sanchez" natural geometric extension of the Weinberg triangle \cite{Taylor}, $1/g_3 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2$, identified as the inverse SU3 coupling where $g_1 = W/H^{(0)}$ is the SU1 coupling and $g_2 = Z sin~\theta/H^{(0)}$ is the SU2 coupling, with $cos\theta = W/Z$ and the BE-Higgs scalar boson mass ratio, by respect to electron $H^{(0)} = 495^2$ (125.208 GeV)\cite{Sanchez3}. With $p_G = P/2^{127/2}$ (from the Combinatorial Hierarchy \cite{Bastin}), to $10^{-7}$:

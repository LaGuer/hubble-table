...
\begin{table}
\caption[Table \ref{tab:20:table20}: 8 Cosmos radius formula]{8 formula for $R_C$}
\label{tab:20:table20}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{3}{c}{}                   \\
    \cmidrule(r){1-3}
 ...
 %$\lambdabar_e  ((ln(M_N/m_e)^2 + (ln(R/\lambdabar_e)^2 + ((ln(t/t_e))^2)^{1/2} $ & 9.12 & couple Cosmos/c-Universe $(R/\lambdabar_e = t/t_e)$ , fig 3 %\cite{Sanchez2}   \\
 
 
 $ l_P(p/H) (R \Pi_{26}/R_N)^{1/3} $ & 9.07579 & Pertinence du produit des ordres des 26 groupes sporadiques \cite{Sanchez2}  \\ $\Pi_{26} \approx e^{841.2869278}$
...
In a 3D Super-space, logarithms of physical ratios are considered vectors. The Cosmos radius $ R_C$ appears as the norm of the vector using for length and time projections the same value $R/\lambdabar_e = t/t_e$. For the mass projection it is $M_N/m_e$ where
%that of the Hubble radius, and for mass ratio $M_N/m_e$.
$M_N$ is the critical mass in the Cosmos reduced spherical hologram. This is a dramatic geometrical confirmation (not dependant of the base for logarithms) of the Extended (2D-1D) Holographic Principle applied to the Bekenstein-Hawking Universe entropy. 
%The Universe is characterised by the c-equivalence $R/\lambdabar_e = t/t_e$ (fig.2). 
\end{figure} 

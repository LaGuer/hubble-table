...
La relation suivante définit le rapport $\tau$ dans \textit{une relation exhibant directement $sqrt a$ dans le développement de $\pi$}:

\begin{equation}
1-cos\theta  \approx  \tau/(4\pi_{\tau})^2 \sqrt a    ~~~~   \rightarrow ~~~~   \pi_{\tau} : 3, 7, \sqrt a
\end{equation}

La proximité des deux derniers rapports signifie que $1/sin\theta - 1/cos\theta$ est voisin de 1. En particulier, on constate que
...
En extrapolant la relation de jauge vue ci-dessus

\begin{equation}
 1/q^2 = 1/{q'}^2  + 1/{q''}^2 
 \end{equation}

à l'espace 3D, muni d'un troixième axe portant l'unité (Fig 3), on constate la connexion gavito-électrofaible:

\begin{equation}
 1 + 1/q^2 = 1 + 1/{q'}^2 + 1/{q''}^2 = (Z/495^2)^2 + 1 = m_Z/m_ \approx 2a^3/pp_G
 \end{equation}
 
 L'extrème précision de cette relation ($10^{-7})$ confirme la valeur ci-dessus du boson scalaire: 
 
 \begin{equation}
  m_Z/m_{BEH} \approx \sqrt(2a^3/pp_G)
 \end{equation}
  
 Rappelons que le terme $2a^3/pH \approx (R/\lambdabar_e)^{1/2\times 3\times 5\times 7}$ est la clef holique du rayon de Hubble. 

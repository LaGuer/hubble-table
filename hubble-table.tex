...
\title{Pi-quantzation and Permanent Cosmology}
...
\begin{abstract}
The Computational Hypothesis implies the quantization of the Archimedes constant. This validates the Wyler's theory and the Fermion Koide formula in the $10^{-9}$ domain. Also the Eddington's equation is fully rehabilitated, by connecting the four forces. The Permanent Oscillatory Cosmology is confirmed by 178 formula giving the invariant Hubble radius, with 14 correlating to $10^{-9}$, whose two are obtained by the computer using the Atiyah constant and the Eddington's electric constant, the whole number $137 \approx a $. This is conform with the Physics-Arithmetics unification predicted by Atiyah.   
\end{abstract}
...
\label{tab:1:table1}
 ...
 Third generation product $\approx e^{426.083498006426259 }$ & $\Pi_{3}$   &$ F_1\cdot F_2 \cdot F_{i24} \cdot F_{i23} \cdot F_{i22} \cdot F_3\cdot F_5 \cdot F_7$    & exact \\
  ...
  Happy Family order product $\approx e^{674.521028749613309} $   & $\Pi_{hap}$   &$2^{301} 3^{142} 5^{57} 7^{30} 11^{18} 13^{10}  17^5  19^4 23^9  31^3 \cdot 47 \cdot D $  & exact \\
      ...
      Pariah Family product $\approx e^{166.765899064651441}$ & $\Pi_{par}$   & $2^{62} 3^{23} 5^{13} 7^7 11^6 \cdot 13 \cdot 17\cdot 19^3 \cdot 23 \cdot29^2 \cdot 31^3 \cdot 37^2 \cdot 43 \cdot 67 $   & exact \\
...
\label{tab:6:table6}
 ...
 40 & $ 2 \lambdabar_{\Pi_0} 2^{a-1/d_e} $ & 13.81235 & confirms the computation basis $2$ \\
  41 & $  \lambdabar_{p} 26^{30} (a/\pi)^{-2/5} $ & 13.81209 & confirms the computation basis $26$ and Holic dimension 30 \\
   ...
   51 & $ \lambdabar_{e} ((a_0-a)/2\eta)^{210 \times 137^2-16^2/3}   $ & 13.81198 & $e^e$ confirms the Holic Principle and the symmetry $a_0 - a \approx 2\eta a $ \\

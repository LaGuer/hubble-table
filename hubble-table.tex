...
Note that with the well-known approximation $\pi_2 = 355/113$, corresponding to the fractionnal limited series $3,7,16$ one finds that $\pi_a$ corresponds to the series $3,7,16,-3\sqrt{137a}/2$, which in turn favors for $\pi_1$ the series  $3,7,16, 1-5a$ 
  ...
  A bridge was established between two very different mathematical domains : the group theory and the conformal field one, which is related to the string theory \cite{Frenkel}. The starting common point is the dimension of the Monster group $D = 196883$. In its treatment of the bosonic string theory, of dimension $d = 26$, Witten \cite{Witten} considers the corresponding entropy $\ln{D}$, and compare it with the natural term $4\pi$. But there are a $10 ppb$ precise relations implying $137$ and $\pi_1$ being the classical approximation 355/113 the electric constant, $ 6dlnD \approx (137/\pi_1)^2$ while the electrical constant  [3] $a ≈ 137.035999084(31)$ define a characteristic $\pi_a$  value:
\begin{equation}
...
\begin{equation}
\tau/\mu   \approx g(1) \approx  2a_s \approx \sqrt[3]{N_{ph}/n_{ph}}
\end{equation}

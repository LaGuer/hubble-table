...
The roots $x_1$ et $x_2$ of this equation are noted in Table 1: $x_1 approx 13.5926430$ and $1/x_2 = 10 x_1 \approx  135.926430$. Now, with the optimised value of the inverse strong coupling constant $a_s$ (Table 2\ref{tab:2:table2}):
...
This Lucas number appears in the ppb precise formula of Table 3\ref{tab:3:table3}, in liaison with 137. Moreover $32^2 - N_{32} =  496$ is the above dimension of the superstring gauge group SO(32), and \textit {the third perfect number}, see below the paramount importance of this fact, unrecognized up to this day.
...
Among the 30 or so free parameter of the present standard model, the Nature seems to favor some ones (Hierarchy Principle \cite{Sanchez}). They distinguish themselves as being measured with high precision, so the Table 2\ref{tab:2:table2} does not include the quarks, neither the neutrinos. 
...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{lllll}
    \toprule
    \multicolumn{5}{c}{physical constants}                  \\
    \cmidrule(r){1-5}
...
\label{tab:3:table3}
  \hskip-2.0cm\begin{tabular}{lllll}
    \toprule
    \multicolumn{5}{c}{cosmic constants}                   \\ 
      \cmidrule(r){1-5}
     name & Symbol   & unit   & Value & imp (ppb) \\
 \midrule
...
\label{tab:4:table4}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{4}{c}{44 formulas for the Hubble radius, with better precision than 1 \%}                   \\
    \cmidrule(r){1-4}
...
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{4}{c}{39 formulas for Hubble radius, with better precision than $2 \times 10^{-4}$}                   \\
    \cmidrule(r){1-4}

...
\draw[step=1cm,gray,very thin] (-6,-6) grid (6,6);
\draw[color=red] (-0.42,0.) -- (6.,0.);
\draw[color=red] (6.,0.) -- (6.,3.43);
\draw[color=red] (-0.42,0.) -- (6.,3.43);
\draw[color=blue] (6.,0.) -- (4.3,2.5);
\node [anchor = east] at (5.,1.5) {$q$};
\node [anchor = east] at (6.,2.5) {$q''$};
\node [anchor = east] at (3.5,-0.5) {$q'$};
\clip(-6.,-0.4) rectangle (0.4,3.4);
...
%\draw (6.,5.52)-- (-4.32,0.);
%\draw (6.,0.)-- (6.,5.52);
\draw[color=gray] (0.,0.4) node {\large $\theta$};
\draw[color=gray] (0.34,0.33);
%\tkzMarkRightAngle[fill=blue!20](B,F,C)
\end{tikzpicture}
...
\node [anchor = east] at (2.,-0.25) {$\omega$};
\node [anchor = east] at (-1.,-0.25) {$z$};
\clip(-6.,-0.4) rectangle (0.4,3.4);
...
%\tkzDefPoint(0.0,0.0){A} \tkzDefPoint(6.4,0.0){B} \tkzDefPoint(5.1,-1.3){C} \tkzDefPoint(4.0,-1.0){D}
%\tkzDrawPoints(A,B,C,D)
%\tkzMarkRightAngle(A,B,C)
%\tkzMarkRightAngle(B,D,C)
%\tkzLabelPoints[below](A,B)
%\tkzMarkAngle[fill=blue!40,size=1.4cm,opacity=.5](C,A,B)
%\tkzLabelAngle[pos=0.8](C,A,B){$\theta$}
%  \draw [important line] (0.0,0.) coordinate (A) -- (5.1,-1.3) coordinate (C) node[below, text width=5em] {$C$}
%  \draw [important line] (6.4,0.) coordinate (B) -- (4.,-1.0) coordinate (D) node[below, text width=5em] {$q$}
  ...
  \draw [decoration={markings,mark=at position 1 with
    {\arrow[scale=2,>=stealth]{>}}},postaction={decorate}] (0,0,0) -- (6.4,0,3.4);
...
\end{tikzpicture}
\caption[The Weinberg-Sanchez cuboid properties]{Weinberg Sanchez cuboid: Electro-weak / gravitation connection}
\end{figure}

...
\tkzLabelSegment[sloped,text=red](q,B){$q$}
...
\tkzLabelAngle[pos=1.6](C,A,B){$\theta$}
  ...
  \draw [very thin,black,line join=round] (0,0,0)  -- node [sloped] {$\sqrt{2a^3/pp_G}$} (6.4,10,3.4);
...
\begin{tikzpicture}
%\tkzInit[xmax=6.4, zmax=3.4]
\tkzDefPoint(0.0,0.0){A} \tkzDefPoint(4.9,0.0){B} \tkzDefPoint(3.12,-1.74){C} \tkzDefPoint(1.8,-1.0){q}
\tkzDrawPoints(A,B,C,q)
\tkzDefMidPoint(B,q) \tkzGetPoint{D}
...
\tkzLabelSegment[sloped,text=red](q,B){$\sqrt{\omega z}$}
\tkzMarkRightAngle(A,B,C)
\tkzMarkRightAngle(B,q,C)
\tkzLabelPoints[below](A,B,C)
...
%\tkzLabelAngle[pos=0.6](A,q,B){$\sqrt{\omega z}$}
  ...
  \draw [very thin,black,line join=round] (0,0,0)  -- node [sloped] {$4\sqrt{(3H\beta/n)}$} (4.9,1.,4.6);
  ...
  %\draw[red]   (0,0,0) -- (.95,0,0)    node[red,left=6pt]    {$x$};
\end{tikzpicture}
\caption[The Sanchez cuboid properties]{3D perspective of the "Sanchez" cuboid: Electro-weak / Eddington matrix connection}
\end{figure}

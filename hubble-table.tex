...
45,x=1.0cm,y=1.0cm]
%\label{tab:15:table15}
\tkzDefPoint(-0.118,0.0){A} \tkzDefPoint(0.0,0.472){B} \tkzDefPoint(1.882,0.){C} \tkzDefPoint(0.0,0.0){D}
\tkzDrawPoints(A,B,C,D)
\tkzMarkRightAngle(A,B,C)
\tkzMarkRightAngle(B,D,C)
\tkzLabelPoints[below](A,C,D)
...
\draw[color=red] (-0.118,0.) -- (0.,0.);
\draw[color=red] (0.472,0.) -- (0.,0.0);
\draw[color=red] (-0.118,0.) -- (0.,1.882);
\draw[color=red] (0.,0.472) -- (-0.118,0.);
\draw[color=blue] (0.,0.) -- (0.,0.472);
\node [anchor = east] at (0.,1.5) {$h$};
%\node [anchor = east] at (2.,2.0) {$\approx 5$};
%\node [anchor = east] at (-1.,2.0) {$\approx \sqrt{22}$};
%\node [anchor = north] at (0.,-0.7) {$\approx \phi^{4}$};
...
%\tkzMarkRightAngle[fill=blue!20](B,F,C)
\end{tikzpicture}
\caption[The holographic Triangle properties.]{Holographic triangle is caracterized by the use of $S=h=\sin\theta$ in its height h its area S.}
\end{figure}

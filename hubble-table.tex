...
The starting observation is that $p \approx (a^2/10)$. Now, $10 \approx \pi^2$. Indeed, to 90 ppb:

\begin{equation}
(i^{\pi/i}/\pi)^4 \approx e^{e^e} (2p_G/a^{2/3})^{1/137}
\end{equation}

implying a role of the economic number $e^{e^e}$, while the deviation term is very close to $p_K/p_G$ and $p_{hol}/p_K$, where $p_G = P/2^{127/2}$ is the gravity-dependant approximation for $p$, $ p_{hol} = (4a^3/3)^{1/2} $ its holographic form and $p_K$ the Koyde factor introduced further.
...
The roots $x_1$ et $x_2$ of this equation are noted in Table \ref{tab:1:table1}: $x_1 \approx 13.5926430$ and $1/x_2 = 10 x_1 \approx  135.926430$. There ratio is $ 10 x_1^2 = p_E \approx 1847.599459$. 

Comparing $ p_{hol} = (4a^3/3)^{1/2} $  with $p_G = P/2^{127/2}$
...
\begin{equation}
p_E^2 - pH \approx \pi a^2     
\end{equation}

presenting an holographic character. Eliminating $2a^2$, the Rayleigh factor, between the two last relations, this defines an approximation for $\pi - 1/2$,  furnishing an operational definition for $\sqrt a$ when compared with its natural value $4\pi$:

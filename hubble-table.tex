...
\begin{abstract}
The Computational Hypothesis implies the quantization of the Archimedes constant. This validates the Wyler's theory and the Fermion Koide formula in the $10^{-9}$ domain. Also the Eddington's equation is fully rehabilitated, by connecting the four forces. The Permanent Oscillatory Cosmology is confirmed by 174 formula giving the invariant Hubble radius, with 14 correlating to $10^{-9}$, whose two are obtained by the computer using the Atiyah constant and the Eddington's electric constant, the whole number $137 \approx a $. This is conform with the Physics-Arithmetics unification predicted by Atiyah.   
\end{abstract}
...
\label{tab:6:table6}
 ...
 38 & $ 2 \lambdabar_{\Pi_0} 2^{a-1/d_e} $ & 13.81235 & confirms the computation basis $2$ \\
   ...
   39 & $ 2^{125/2} \lambdabar_p l_P (\lambda_C \lambdabar_C/ r_e^2)^2/137\beta  $ & 13.81193 & confirms the Cosmos (CMB) temperature\\
    ...
    45 & $  \lambdabar_{e} (e^{16}\pi^4/60 \zeta(3))^{a/\Gamma} /d_e  $ & 13.811201 & $e^e$ calculation basis \\
   
    
    
   
   46 & $ d_e \lambdabar_{e} (R_N d_e/r_e)^{1/4} e^{24e}   $ & 13.81198 & $e^e$ calculation basis \\
   
   
   
   
   47 & $ \lambdabar_{e} (d_e^5 pH/a^2)^{1/3} e^{32e}   $ & 13.81198 & $e^e$ calculation basis \\
   
   
   
   48 & $ \lambdabar_{e} ((a_0-a)/2\eta)^{210 \times 137^2-16^2/3}   $ & 13.81198 & $e^e$ confirms the Holic Principle and the symmetry $a_0 - a \approx 2\eta a $ \\
   
   
   
   
   
   49 & $ (10/3d_e^{e^3})\lambdabar_{e}  (P/a_w)^{7/2} $ & 13.81198 & confirms the power 7 of Holic Principle \\
   ...
   51 & $\lambdabar_{e}^2/l_K  a^{31} (3/8\pi_0^2)^2 $ & 13.81198(3) & with $\pi_0 = 3 + 1/ (7+1/\pi_0^2)$ \\
  
    
     
    
   52 & $2(l_K/F)^2/\lambdabar_{e}$ & 13.81198(3) & from elimination of $c$ between gravitational and electroweak couplings \\
...
\label{tab:6:table6}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    \midrule
     ...
     14 & $ \lambdabar_{e} (3^3)^{3^3} e^{\pi_1^2/2}  $ & from the Cosmos holographic radius $R_N$ with $\pi_1 = 3+1/(7 + \beta a/137\times 16$  \\

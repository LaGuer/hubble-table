...
\begin{abstract}
The Permanent Oscillatory Cosmology is confirmed by 160 formula giving the Hubble radius, with 8 correlating to $10^{-9}$, whose two are obtained by the computer using the Atiyah constant and the Eddington's electric constant, the whole number $137 \approx a $. This is conform with the Physics-Arithmetics unification predicted by Atiyah. The identification with the Eddington statistical formula gives $G$, compatible with the $10^{-5}$ precise BIPM measurement, the $10^{-6}$ precise sun-quasar non-Doppler Kotov period and the $10^{-8}$ precise relation to the muon mass. The additive hypothesis of a computing Cosmos implies leads to a formula of the electric constant $a$ implying $\pi + \gamma - e $, and a $\pi$ rationalization process which validates the Wyler's theory and the Fermion Koide formula in the $10^{-9}$ domain. Also the Eddington's equation is fully rehabilitated, by connecting the four forces. The Ramanujan number and the computing term $a^a$ makes a bridge between octonion algebra and sporadic groups, as predicted by Atiyah.   
\end{abstract}
...
Feynman simply forgets here the science history. Looking for whole values in experimental data was very useful for Dalton, Balmer, Mendeleiev, opening respectively chemistry, atomic physics and nuclear physics.

Atiyah inisted on a parallel between the Archimedes constant $\pi$, and he remarked that the couple $137,a$ shows $a^2 - 137^2 \approx \pi^2$, and that his constant involves $a/\pi$,
...
Starting with $\pi_1$ is equal the hypothetical $\pi$ this leds to the following development for $\pi_2 : 3,7,16,5\times 137/d_e$, where $d_e$ is the excess electron magnetic moment, tighty tied to $\sqrt a$ in quantum eectrodynamics. Adopting precisely this value, this induces a slighty varation of the development, characteristic of the isospin symmetry neutron-Hydrogene, leading to a value compatible with experiment:
...
Note that with the well-known approximation $\pi_2 = 355/113$, corresponding to the fractionnal limited series $3,7,16$ one finds that $\pi_a$ corresponds to the series $3,7,16,-3\sqrt{137a}/2$, which in turn favors for $\pi_1$ the series  $3,7,16, 1-5a$ 
  ...
  A bridge was established between two very different mathematical domains : the group theory and the conformal field one, which is related to the string theory [Frenkel]. The starting common point is the dimension of the Monster group $D = 196883$. In its treatment of the bosonic string theory, of dimension $d = 26$, Witten [2] considers the corresponding entropy $lnD$, and compare it with the natural term $4\pi$. But there are a $10 ppb$ precise relations implying $137$ and $\pi_1$ being the classical approximation 355/113 the electric constant, $ 6dlnD \approx (137/\pi_1)^2$ while the electrical constant  [3] $a ≈ 137.035999084(31)$ define a characteristic $\pi_a$  value:
\begin{equation}
 6dlnD \approx (a/\pi_a)² - 1 ~~~~\rightarrow  ~~~~ \pi_a : 3,7,16,-3\sqrt{137a}/2
\end{equation}  
...
, confirming the arithmetical character of Physics revealed by the Monster Group, Topological Axis [5], and the Eddington number 137 [6][7]. As predicted [8] the physical constants are tied to rational approximations of p
   ...
   \section{The computational signifiance of $a$}
         ...
         The present standard model of physics is incomplete, since 30 or so parameters are not calculated from the theory and so appear as 'free parameters'. As these parameters are not reckognised by matematicians, this would mean either that they are arbitrary, or they belongs to an unknown domain or mathematics. 
...
Making the substition $e \to i$, one considers the term $i^{\pi/i} = e^{\pi^2/2} = a_0$ which shows direct connection with $a$
...
Now, the powers of $\pi $ appear in the even Rieman series, indicating that $\pi$ must be seen as a computation basis. Thus the difference $\pi - e$ is examined, which shows in the first approximation $ 1-\gamma$. The second approximation shows unambigously td ratio $(a/137)^2$, leading to the following expression:
...
Moreover, $X$ is unambigously tied to the Atiyah's constant $\Gamma$, even showing a bridge with $\gamma$:

\begin{equation}
X \approx \Gamma ^{1/sin^2(\theta_{eff})} \approx (1/\gamma)^{\Gamma (1+1/a_0)}
\end{equation}  
:17
corresponding to $ sin^2(\theta_{eff})\approx 0.231490 \approx (sqrt{(a_0 -3)/d_e})/2\Gamma \approx (sqrt{(\Delta a/137})/2\Gamma$, compatible with the measurement \cite{Tanabashi}, and making the laison wit the Eddington discriminant $\Delta$ (see below). The pertinence of the Atiyah constant is fully confirmed.
...
\label{tab:1:table1}
  \hskip-2.5cm\begin{tabular}{llll}
    ...
    Feigenbaum bifurcation relative velocity & $f$    & 4.66920160910299 & 'exact' \\ 
   ...
   de Vries Electric constant  & $a_{deV} $   & 137.03599909582970048 & 'exact' \\
    ...
    Modular number & $j_0$    & 744  & exact \\ 
     ...
     Monster group order $\approx e^{ 124.12642336632464}$  & $O_M$    & $2^{46}\cdot 3^{20} \cdot 5^9 \cdot 7^6 \cdot 11^2 \cdot 13^3 \cdot 17\cdot 19 \cdot 23 \cdot29 \cdot 31 \cdot 41 \cdot 47 \cdot 59 \cdot 71$  & exact \\   
     
     Monster dimension & $D$    & $47 \cdot 59 \cdot 71 = 196883$   & exact \\
     
     Baby-Monster group order $\approx e^{ 77.4095679039653309}$  & $O_B$    & $2^{41}\cdot 3^{13} \cdot 5^6 \cdot 7^2 \cdot 11 \cdot 13 \cdot 17\cdot 19 \cdot 23 \cdot 31 \cdot 47$  & exact \\
     
    
 First generation (Mathieu) product $\approx e^{68.8957364082420507}$ & $\Pi_{1}$   & $ M_{11}\cdot M_{12}\cdot M_{21} \cdot M_{22}\cdot M_{23}$    & exact \\
 ...
 Third generation product $\approx e^{426.083498006426259 }$ & $\Pi_{3}$   &$ F_1\cdot F_2 \cdot F_{i24} \cdot F_{i23} \cdot F_{i22} \cdot F_3\cdot F_5 \cdot F_7$    & exact \\
  ...
  Happy Family order product $\approx e^{674.521028749613309} $   & $\Pi_{hap}$   &$2^{301}\cdot 3^{142} \cdot 5^{57} \cdot 7^{30} \cdot 11^{18} \cdot 13^{10} \cdot 17^5 \cdot 19^4 \cdot 23^9 \cdot 31^3 \cdot 47\cdot D $  & exact \\
     
      Pariah Family product $\approx e^{166.765899064651441}$ & $\Pi_{par}$   & $2^{62}\cdot 3^{23} \cdot 5^{13} \cdot 7^7 \cdot 11^6 \cdot 13 \cdot 17\cdot 19^3 \cdot 23 \cdot29^2 \cdot 31^3 \cdot 37^2 \cdot 43 \cdot 67 $   & exact \\
...
\begin{table}
\caption{40 formulas for the Hubble radius, with better precision than 1 \%}
\label{tab:4:table4}
...
\begin{table}
\caption{37 formulas for the Hubble radius, with better precision than 0.1 \%}
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    %\multicolumn{4}{c}{}                   \\
  ...
  10 & $R_1 (8/\sqrt{3a})^{1/7}$ & 13.812 & from relations between photon numbers \\
   ...
   15 & $O_M^{d_e pH\sqrt\beta / 24D}$ & 13.811 & confirms the monster and its dimension D\\
    ...
    24 & $\lambdabar_{e} e^a/p^6\Gamma $ & 13.815 & confirms the natural calculation basis \\
    25 & $  \sqrt{\lambdabar_{p} \lambdabar_{H}} (WZ)^4 \approx \sqrt{\lambdabar_{p} \lambdabar_{H}}  (HK_+/p)^{14} $ & 13.818 & confirms the Holic Principle \\

26 & $ (3/\pi d_e)\lambdabar_{n} (K_0 K_+)  $ & 13.813 & confirms the Holic Principle \\

    27 & $ (20/9)\lambdabar_{e} P^{\sqrt a - 10 }  $ & 13.810 & from $ N_H approx P^{\sqrt a} $  \\
    
    28 & $ (\sqrt{5/2}/3) ( \Pi_0^3 \Pi_+)^4 $ & 13.812 & confirms the topological axis  \\
     29 & $ R_N (1834^2/pH) ( \Pi_0 /\Pi_+)^8 $ & 13.812 & confirms the Cosmos holographic radius $R_N $  \\
     
     
     
     
     
  30 & $\lambdabar_{e} a^{a/16} /Z^4$ & 13.81111 & confirms the cenral role of $Z$ and $a^a$\\
   31 & $(R_N n/ (n/p^2) (a_s^{2\pi/z\sqrt a}$ & 13.81113 & confirms strong coupling constant $a_s$\\
  32 & $ \lambdabar_{e} \sqrt{a/137} j_0^{2ep\beta/j_0}$ & 13.81139 & confirms the modular number as calculation basis\\ 
   33 & $\lambdabar_{e} a^{a_s} 2P/e^5d_e^3$ & 13.81189 & confirms the computing character of $a$ and $a_s$ \\
   34 & $\lambdabar_{e} j_0^{n/a}$ & 13.81189 & confirms the modular number as calculation basis\\
   35 & $(2-1/11)^{2\times 7^3/5}$ & 13.81187 & confirms the Archimedes $\pi_{Arch} = 22/7~~~~ 6/\pi_{Arch} =2-1/11$\\
   ...
   36 & $ (W^2/3^6)\lambdabar_{e} (\lambdabar_{e}^4 l_{ph}^3 (\lambdabar_{CMB})/l_P^8)^{1/7}$ & 13.81224 & comes from $\sqrt{2n_{ph}/n_n} \approx (u^U)/(u_{CMB}+u_{CNB})$\\ 
   37 & $\lambdabar_{e} x_1^{34} /(1+1/e^5)$ & 13.81206 & confirms the Eddington's equation\\
   38 & $R_N /\sqrt {e-1}$ & 13.81207 & empiric\\
    39 & $\lambdabar_{e} a^{2a_s} \sqrt{nH}/6d_e^6 $ & 13.81253 & confirms the computing character of $a$ and $a_s$\\
    40 & $\lambdabar_{e} a^{\tau / \mu} (137/2\sqrt a)^4 /3 $ & 13.81115 & confirms the computing character of $a$, $\tau$ and $\mu$\\
   41 & $R_N exp(-2/e^2)$ & 13.81195 & empiric\\
   42 & $(R_N/2)(n/H)(a/137)^2 \pi^{1/e}$ & 13.81201 & empiric\\ 
   43 & $(R_N/2)(n^2/pH)a^{1/sqrt a}$ & 13.81255 & from central relation\\
  44 & $(4\beta^2 \pi_{Arch}^4 /3) (R_C)/P) (O_B/O_M)^2$ & 13.81196 & implies the 2 largest sporadic groups, with $\pi_{Arch} = 22/7$\\
  
   45 & $(\lambdabar_{e} ^2/l_K)(O_B \pi^4 a_s^2 p/FH)^2$ & 13.81114 & implies the Baby Monster group\\
   46 & $N_{Ed}(l_P^2/ \lambdabar_n) a^{11}/137^8 (2\pi)^7   $ & 13.81294 & confirms 137, $a$ and $2\pi$ as computation basis\\
  47 & $2\beta \lambdabar_{e} j^{17} (4\pi)^2 \sqrt{137}$ & 13.81198 & j calculation basis \\
  48 & $ 2^{136}\lambdabar_{F}137 a/a_s $ & 13.81200 & symmetry among the coupling constants \\
  49 & $ (4/3)\lambdabar_e \phi^{\sqrt {n\sqrt{pH}/\beta}} $ & 13.81200 & confirms the golden ratio as calculation basis \\ 
   50 & $N_{Ed}(l_P^2/\sqrt(\lambdabar_p \lambdabar_n)) (r_H/\lambdabar_e)^3 (H/p)^2 / (2\pi)^8  $ & 13.81200 & confirms the computational role of $2\pi$\\ 
   51 & $2N_{Ed}(l_P^2/\lambdabar_{\tau}) (e/(\pi-e)) (2\pi)^3 136/d_e(a-1) $ & 13.81200 & confirms the role of $\pi - e$\\
    52 & $2N_{Ed}(l_P^2/\lambdabar_e) (W/F) (an/\pi p)^2 \sqrt{137} $ & 13.81206 & confirms the Atiyah ratio $a/\pi$\\
    53 & $ \lambdabar_e (1/ln\phi)^{2sqrt {2(H/p)n\sqrt{nH/\sqrt\beta}}} $ & 13.81209 & confirms the golden ratio logarithm as calculation basis \\ 
   54 & $2N_{Ed}(l_P^2/\lambdabar_n) (r_H/lambdabar_e)^3/ \pi 496^2 $ & 13.81207 & confirms the superstring 496\\ 
...
\begin{table}
\caption{35 formulas for Hubble radius, with better precision than $6 \times 10^{-5}$}
\label{tab:6:table6}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    %\multicolumn{4}{c}{}                   \\
  ...
  1 & $2N_{Ed}(l_P^2/\lambdabar_e) (F/Z) (a/\pi)^2 d_e^{\sqrt{10}} $ & 13.81186 & confirms the Atiyah ratio $a/\pi$\\ 
   2 & $\lambda_{e} (e^{2a}2H/3p^2)^{1/3}$ & 13.81145 & natural base e\\
   3 & $4r_e (p_W/p) (2j^2 7^{137})^{1/3} $ & 13.81120 & unambigous base 7 \\
   
   4 & $ \sqrt{R_N lambda_{e} /2d_e} e^{P/2AD} $ & 13.81120 & pertinence of the Monster dimension $D$ \\
   ...
   8 & $\lambda_{e} (3j^j/2H)^{1/6}$ & 13.81199 & $j$ and $a$ : related computation bases \\
   9 & $\beta F P^{3/2} (n/p)^{7/2} 2 \pi$ & 13.81198 & proton-neutron symmetry  \\ 
   10 & $(45\lambdabar_{CMB}^7/4(p+5)/\lambda_{CNB}^3)^{1/2}/l_P$ & 13.81197 & confirms $T_{CMB} and p+5 \approx n^2/p \approx H^5/p^4$\\
   11 & $4l_Kp^4 \sqrt{p/H}/\beta d_e$ & 13.81198 & confirms the non-Doppler sun-quasar period \\
   12 & $\lambdabar_{e} e^{(4)}(1/ln{\sqrt a}) (a^3/pH) (a/\pi)^{-2/p}   $ & 13.81199 & confirms $R_N = R pH/a^3$ and the economic function $e^{(4)}(x)= e(e(e((x))))$\\
   13 & $g(6)^{1/(1-1/\pi e)}/N_L^{1/(\pi e-1)}$ & 13.81198 & confirms the topogical term g(6) \\
    14 & $(n/p^{1/4)}  \lambdabar_{e} \phi ^{3\times 64}/(2\pi)^2   $ & 13.81206 & confirms the golden ratio as computation basis \\
    
    15 & $2 N_L r_e (16\pi e)^2/137  $ & 13.81193 & confirms the Combinatorial Hierarchy \\
    
    16 & $\lambdabar_{e} e^{a/ln\delta} n/\sqrt{2a^3}  $ & 13.81155 & confirms the Feigenbaum constant $\delta$ \\
    
    
    17 & $ 1834\sqrt 2 \lambdabar_{e} (exp(e^{\pi})/P^10)^4  $ & 13.81155 & confirms the large term $exp(e^{\pi})$ \\
    
    18 & $ (n/p)^2\sqrt {2R_N \lambdabar_{e}} ((a/e)^a/P^10)^2  $ & 13.81155 & liaison between the basis $a$ and $e$ \\
    
    19 & $ \lambdabar_{e} (d_e^8(a/e)^a/a)^{1/6}  $ & 13.81200 & liaison between the basis $a$ and $e$ \\
    
    
     20 & $ (p/H)^2\lambdabar_{e}(75/64)^{2\sqrt{137 a}}exp(2^{22/4})  $ & 13.81168 & confirms the Topological function \\
    
    
    
    21 & $2 N_L \lambdabar_{e} (3p_K/2)^2 \times 137/Fsqrt{pH\beta}  $ & 13.81198 & confirms the Koide factor \\
    
    22 & $2 N_L \lambdabar_{e} (1-a^{7/2}/137^{3/2}pH  $ & 13.81198 & both $a$ and 137 are computional basis \\
    
    23 & $2 N_L \lambdabar_{e} (p_W/n)^4  $ & 13.81197 & both $n$ and $p_W$ are computional basis \\
    
    24 & $ 2R_Np(\beta^4/ln\pi)^2/H $ & 13.81197 & confirms cosmos hologram radius $R_N$ \\
    
    
    25 & $4 \lambdabar_{e} e^{139\pi /5} (2(a-136))^{-1/a}  $ & 13.81197 & confirms the basis $e^\pi$ \\
    
   26 & $ \lambdabar_{e} ((R_C/l_{ph})^3 \sqrt{n(1+sqrt3)/p})^{1/7} $ & 13.81198 & confirms the Universe as Cosmos gauge boson \\
   
   
   
   
   
   27 & $ (137/a)\lambdabar_{e} (e^{3\mu}d_e^{p_{hol}/3} )^{1/7} $ & 13.81197 & confirms the Universe as Cosmos gauge boson \\
   
   
   
    28 & $ (p/H)^4 \lambdabar_{e} (e^{3\times 210}/O_1)^{1/7} $ & 13.81177 & confirms the role of the Matthieu group$ O_1$ \\
    
    
    
    
   29 & $ \lambdabar_{p} g(6) 2^{256} p_W /e^{e^{e}/2} p_{hol}^2 O_B^2 $ & 13.81198 & confirms the role of the Monster groups \\ 
   
   
   
   
   
   30 & $ \lambdabar_{p} O_M p_W /a \sqrt{2p^3/H} e^{e^{e}/2} p_{hol}^2 O_B^2 $ & 13.81197 & confirms the role of the Monster groups \\ 
   ...
   31 & $ 4 \lambdabar_{p} (c/PC) p_{Ed} \sqrt{a/137} O_M O_B^2 $ & 13.81188 & confirms the Cosmos and Monster groups \\   
   ...
   32 & $ 2^{125/2} \lambdabar_p l_P (\lambda_C \lambdabar_C/ r_e^2)^2/137\beta  $ & 13.81193 & confirms the Cosmos (CMB) temperature\\
   
   
    
    33 & $ 4 \lambdabar_{e} ((\pi^2 a/2 \times 137^{3/2})\sqrt{N_H N_{ph}})^{1/7} $ & 13.81197 & confirms the Universe as Cosmos gauge boson \\
    
    
    
    34 & $ (a/137)^{1/4} \lambdabar_{e} (H/2\pi)^{}16/a_s  $ & 13.81201 & confirms the strong coupling $a_s$  \\
    
    
    
    
    35 & $ (ln(\sqrt3+d_e^2 H/p)lnP) \lambdabar_{e} ((5^4-137)/(4^4-137))^{2^6}  $ & 13.81197 & confirms the transition 4D-5D  \\
    
    
    
   36 & $2(l_K/F)^2/\lambdabar_{e}$ & 13.81198(3) & from elimination of $c$ between gravitational and electroweak couplings \\
...
\begin{table}
\caption{9 ppb precise formula for $R \approx 13.8119768$ Glyr}
\label{tab:6:table6}
  \hskip-2.0cm\begin{tabular}{llll}
    \toprule
    \multicolumn{3}{c}{}                   \\

...
Now $p_{hol}$ is very close to $ \sqrt {6F}$, so that there mean value introduces the volume $V_{2r_H}$ of the cube of radius $2aH/p$, circonscrit to the sphere of the electronic coud in the Hydrogen atom. This is analog to the Archimedes method for calculatig $\pi$, as noteiced by Atiyah. Replacing $\Delta_E+1$ by $ \Delta_{Ea} = a^2 - 40 $, which introduces the transformation $136 \rigtharrow a$, one gets \taxtit{a ppb correlation}:

\begin{equation}
(FV_{2r_H})^{1/2}  \approx  p_E^2 + (4/\pi_{EFV})\Delta_{Ea}   ~~~~~~ \Rightarrow    6\pi_{EFV}^5 \approx 2(F/137\times a\beta)^2 
\end{equation}

Moreover, the fractional development of this critical value \pi_{EFV} is : 3, 2i^{\pi/i}/(1.6)^16, where the 16D space is flagrant. 
...
With the product $pH$, the gravitationnal quadratic deviation is simpler:

\begin{equation}
pH - p_G^2 \approx a^2     
\end{equation}

 Eliminating $2a^2$, the Rayleigh factor, between the two last relations, this defines an approximation for $\pi - 1/2$,  furnishing an operational definition for $\sqrt a$ when compared, as seen above, with its natural value $4\pi$:
...
With $\pi_1 = 3+2/x_1$, the coresponding Lenz-Wyler value comes back to the holographic value:
...
\textit {So the four force couling are impled in the Eddington's equation}

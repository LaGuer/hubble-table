...
implying a role of the economic number $e^{e^e}$, while the deviation term is very close to $(2p\G/a^{2/3})) {1/137}$, $p_K/p_G$ and $p_{hol}/p_K$, where $p_G = P/2^{127/2}$ is the gravity-dependant approximation for $p$, $ p_{hol} = (4a^3/3)^{1/2} $ its holographic form and $p_K$ the Koyde factor introduced further.


Eddington identified 10 to the number of independant parameters in the space-time symmetric matrix $4 \times 4$, and he introduced the number $136$ for the matrix $16\times 16$. The basic idea is to connect two spaces, with different dimensions  16D and 4D, which is the general holographic concept. 
...
The roots $x_1$ et $x_2$ are $x_1 \approx 13.5926430$ and $1/x_2 = 10 x_1 \approx  135.926430$. Theire ratio is $ 10 x_1^2 = p_E \approx 1847.599459$.

This value must be compared  with $p_G = P/2^{127/2}$ the $G$-dependant parameter, using the Lucas Number:

\begin{equation}
p_E^2 - p_G^2 \approx \pi a^2     
\end{equation}

presenting an evident holographic character : the area of the disk of radius $a$, where $a$ is the principal value of the ratio hydrogene radius/electron reduced wavelength. 


Considering now the sphere of radius $a$, this introduces the 3D holographic parameter $ p_{hol} = (4a^3/3)^{1/2} $ 
...
Now for $pH$, the electric term 


while for $p_E$ there is an evident symmetry:

\begin{equation}
pH - p_G^2 \approx a^2     
\end{equation}

tying directly gravitation and electricity with $pH$

Now, for $p_E$:

 Eliminating $2a^2$, the Rayleigh factor, between the two last relations, this defines an approximation for $\pi - 1/2$,  furnishing an operational definition for $\sqrt a$ when compared with its natural value $4\pi$:

...
\textit{The number $a^a$ is of central importance in the following, implying that $a$ is an optimal valculation basis}
...
\textit{The pertinence of the Atiyah constant is fully confirmed.}
  ...
  The golden ratio $\Phi = (1+sqrt 5)/2$ shows singular arithmetical and geometrical properties. The connection with $a$ and the whole value $E(p) = 1836$ and with the computational term $e^{1/e}$  is direct :
...
implying the above canonical factor $a/20$. The analysis leads to:
 ...
 \begin{equation}
a^2/137 \approx 20 ~ (\Phi p_W/p)^4   ~~~~~ p_W = 6\pi^5  ~~ \pi : 3,7,16,a_s^2\tau/\mu  
\end{equation}  
 
implying the Lenz-Wyler formula, recalled below.
...
\begin{equation}
f(\sqrt\pi_4) \approx (g(\sqrt\pi_4))^{4\pi/\sqrt a}  ~~~~ \pi : 3,7,\sqrt{a+1}
\end{equation} 

This implies the Cosmos radius:

\begin{equation}
R_C/\lambdabar_e \approx (g(\sqrt\pi_4))^{4\pi}  ~~~~ \pi : 3,(1+5\sqrt 5)/2
\end{equation} 
...
The proton-electron mass ratio of Wyler \cite{Wyler} is the simple formula, which was piblished by Lenz in the shortest of al articles: 
...
\subsubsection {the Lenz-Wyler monster grou connection}
...
\subsubsection {the Lenz-Wyler golden number connection}

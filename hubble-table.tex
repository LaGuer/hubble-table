...
%Note that with the well-known approximation $\pi_{137} = 355/113$, corresponding to the fractionnal limited series $3,7,16$ %one finds that $\pi_a$ corresponds to the series $3,7,16,-3\sqrt{137a}/2$, which in turn favors for $\pi_1$ the series  %$3,7,16, 1-5a$ 
 ...
 The fractional development of $e$ shows unambigously the central gravitational parameter, tied to the Lucas numner \cite{Sanchez}  $p_G = P/2^{127/2}$ 
...
The number $(4\pi_{D,a})^2 \sqrt a$ is close to the Eddington's proton/electron mass ratio, as recalled below. This connects $a$ and $D$ in the ppb range. 
...
\textit{The $G$ value is again confirmed by optimising correlations, so the the 180 ppm-distant official value of $G$ appears to be erroneous by $7.5 \sigma$}.
...
With the third "subperfect" number $n_3 = 495 = M_{11}/16$, tied to the supersymmetric number 496 (third perfect number), and whose square is compatible with the measured Higgs/electron mass ratio, one proposes the neutral pion mass factor:  

\begin{equation}
e^{1/613} \approx (n_3/p)(\Pi_0/a)^2  
\end{equation}

Then the computer analysis shows up the following otimales correlations

...
\section{The scope and method}
   ...
   \section{The computational signifiance of $a$}
         ...
         %Our main hypothesis is that \texit {the Cosmos is a computer using these parameters as optimal calculation basis},
...
Making the substition $e \to i$, one considers the term $i^{1/i} = e^{\pi^2/2}$ which shows direct connection with $a$
   
 \begin{equation}
i^{1/i} \approx (1+d_e/a)^2 
\end{equation}
 
where $d_e$ is the excess magnetic momment of the electron. This is a quantity connected to $a$ trough the quantum electrodynamics, the most precise theory, checked to 0.01 ppb. The first term of its development is $d_e \approx 1 + 1/2\pi a$. This leads to

\begin{equation}
\sqrt {a i^{1/i}} \approx a + 1 + 1/ 2\pi a
\end{equation}

giving $a$ to 3 ppb. Quite independently, Hans de Vries \cite{deVries} gave a formula expliciting the other terms in the development.
...
this has the zdvantage of starting from the constant Eddington 137. With $x = 136 \times 137 \sqrt{2p_G}$, where $p_G = p/\sqrt N_L$ (see table 1) is tied to the $G$ value presented in this article (table 6), one gets $a \approx 137.03599909275 $, compatible with the 0.15 ppb precise measured value and at 0.02 ppb from the de Vries value.

\begin{equation}
x \approx  
\end{equation}

Where $p_G$ is the constant that appears in the development of $e = 2 + 1/(7/5 -1/(3\sqrt {p_G}))$. 


This leads to 2 decimal more than the measured value : $a \approx 137.03599909314(13)$. So the Eddington number 137 is fully rehabilitated as well as the central role of $a$.

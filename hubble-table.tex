...
Now, the powers of $\pi $ appear in the even Riemann series, indicating that $\pi$ must be seen as a computation basis. Thus the difference $\pi - e$ is examined, which shows in the first approximation $ 1-\gamma$. The second approximation shows unambigously td ratio $(a/137)^2$, leading to the following expression:
...
\end{equation}  
:17
corresponding to $ sin^2(\theta_{eff})\approx 0.231490 \approx (sqrt{(a_0 -3)/d_e})/2\Gamma \approx \bigg(\sqrt{(\Delta a/137})/2\Gamma$, compatible with the measurement \cite{Tanabashi}, and making the laison wit the Eddington discriminant $\Delta$ (see below). The pertinence of the Atiyah constant is fully confirmed.

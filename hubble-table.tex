...
\begin{abstract}
The Permanent Oscillatory Cosmology is confirmed by 116 formula giving the Hubble radius, with 7 correlating to $10^{-9}$, whose two are obtained by the computer using the Atiyah constant and the Eddington's electric constant, the whole number $137 \approx a $. This is conform with the Physics-Arithmetics unification predicted by Atiyah. The identification with the Eddington statistical formula gives $G$, compatible with the $10^{-5}$ precise BIPM measurement, the $10^{-6}$ precise sun-quasar non-Doppler Kotov period andthe .  $10^{-8}$ precise relation to the muon mass. The additive hypothesis of a computing Cosmos implies leads to a formula of the electric constant $a$ impying $\pi_ + \gamma - e $, and a $\pi$ rationalization process which validates the Wyler's theory and the Fermion Koide formula in the $10^{-9}$ domain. Also the Eddington's equation is fully rehabilitated, by connecting the four forces. The Ramanujan number and the computing term $a^a$ makes a bridge between octonion algebra and sporadic groups, as predicted by Atiyah.   
\end{abstract}

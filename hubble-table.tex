...
\begin{abstract}
The hypothesis that the Cosmos is a computer leads to a reunification of Arithmetics and Physics. The 30 or so parameters of the standard model are then supposed to be an optimal calculation basis. The extrapolation of the operational definition of the Euler-Mascheroni constant $e$ leads to a relation defining the electric constant $a$. This complies with the Physics-Arithmetics unification predicted by Atiyah, who suggested that $a$ was a renormalization of the Archimedes constant $\pi$. But the calculation hypothesis implies the rationalisation of $\pi$. This rehabilitates the Wyler's theory,the Eddington's equation and the Fermion Koide formula in the $10^{-9}$ domain. Analysis shows that the 26 sporadic groups are involved, with special importance of the Matthieu family, leading to a confirmation that the Higgs boson is a central particle. The Permanent Oscillatory Cosmology is confirmed by 223 formulas for the invariant Hubble radius, with 18 correlating to $10^{-9}$, whose two are obtained by the computer using the Atiyah constant and the Eddington's electric constant, the whole number $137 $. 
\end{abstract}
...
confirming the arithmetical character of Physics revealed by the Monster Group, the Topological Axis \ref{tab:9:table9}, and the Eddington number 137 \textcolor{red}{cite{ref missing for [6][7]}}. 


\textit{As predicted \textcolor{red}{cite{ref missing for [8]}}, it is now proven that the physical constants are tied to rational approximations of $\pi$.}
...
Note that this decomposition is the rationalization of the $10^{-4} $ precise connection \cite{Sanchez}:
...
\section{The connection with the 11 main parameters}
...
where $\Pi_{11}$ is the product of the 11 main mass ratioes relative to electron in particle physics (table 1): Nambu ($a$), proton, neutron, muon, tau, Ions, Kaons, weak bosons.  

\begin{equation}
\Pi_{11} = a \cdot p \cdot  n \cdot  \mu \cdot  \tau \cdot  \Pi_0 \cdot  \Pi \cdot  K_0  \cdot K \cdot Z \cdot  W
\end{equation}

The number 495 doubly connects with this series in the search for solutions limited to 4 terms with powers limted to 2 and precision $5 \times 10^{-4} \approx (a/137)^2$:

\begin{equation}
495 \approx a (p/K)^2 \approx n (a/\Pi_0)^2
\end{equation}

This shows how the charged Kaon $K$, belonging to the so-called 'strange particles' appears to be important. 


Also, the gravitational term $p_G^2 = P^2/2^{127}$ verifies, to 10 ppm:

\begin{equation}
(a/137)^{1/2} p_G^2 \approx KK_0^2/\Pi \approx n^2 K / K_0
\end{equation}


Now it becomes clear what is the signifiance of the product of terms in a series. Indeed, looking for an insertion in the series of an additive term, this implies the consideration of this product, which is the maximum to be considered, and its geometrical mean, which is the most probable term to try.
...
\begin{equation}
\tau \approx n^2/K_0 \approx K_o^2/\Pi
\end{equation}


The optimal basis $e$ cheks:

\begin{equation}
e \approx p\Pi_0/Z \approx W/a\sqrt Z
\end{equation}

while :

\begin{equation}
\gamma \approx (n/\mu)^2/137
\end{equation}

The central Pythagoras number $6^3$ (which is the sum of the cubes of 3, 4 and 5) enters the connection, with a direct relation with the Cosmos temperature:

\begin{equation}
\sqrt (\lambdabar_{CMB}/\lambdabar_e \approx 6^6  \approx n\tau/a
\end{equation}

The Pell-Fermat generator $g_1 = 1 + \sqrt 2$ shows a symmetric correlation:

\begin{equation}
g_1 \approx \sqrt\Pi_0  (\Pi/n)
\end{equation}

The second generator $g_2 = \sqrt 2 + \sqrt 3$ verifies:

\begin{equation}
g_2 \approx W/\tau \sqrt \mu
\end{equation}

\begin{equation}
ln O_M \approx \sqrt (WH(n/p)^{1/2}) / 137
\end{equation}


\section{ppb connections involving the Pions and Kaons}

With the third "subperfect" number $p_3 = 495 = M_{11}/16$, tied to the supersymmetric number 496 (third perfect number), and whose square is compatible with the measured Higgs/electron mass ratio, one gets the neutral pion mass factor:  

\begin{equation}
e^{1/613} \approx (p_3/p)(\Pi_0/a)^2  
\end{equation}

Then the computer analysis shows up the following relations

\begin{equation}
W/\mu^2 \approx (p/p_3)(K/K_0)  
\end{equation}
...
The value of the weak neutral boson Z is thus confirmed in the ppb range \cite{Sanchez}, and the ppb values for the Pions and Kaons are resumed in the table 1.  
...
this ratio is $p_E \approx 1847.599459$. Now it enters a triple correlation, with the above main series of parameters, limited to cubic terms:
...
leading to a large number which correlates with the main gravtational parameter $P = m_P/m_e$:

\begin{equation}
  p^7/x_2 \approx (1/x_2)^{3/2}/4   
\end{equation}

The power 14 of $p$ replaces $1/16x_2$ by the strong coupling $a_s$.  The study of deviation shows up the electric and weak nuclear couplings in the corective factor, leading to (23 ppm):
...
The fine-tuning was invoked to justify the existence of Life in our Universe, considered as special among a series of sterile universe in a disparate Multiverse \cite{Carr}. This application of the so-called 'anthropic principle' is misleading since it has been shown that the physical parameters are present in the DNA nucleotide cite{Sanchez}. Indeed, the massses of the couple AT and GC are equal, within one Hydrogen precision, to 1839/3 = 613 hydrogene masses, so that the mean mass of a complete codon and an electron is about an Hydrogen mass. Moreover this bi-codon mass is directly tied to the non-Doppler quasar Kotov period \cite{Sanchez3}. So the DNA molecule would be a linear hologram, tied to optimal calculation directly with the Cosmos. \textit{This could be the prefiguration of quantum computer in the future.}
...
\label{tab:1:table1}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Third Subperfect number  & $p_{3}$    & 495 & exact \\ 
